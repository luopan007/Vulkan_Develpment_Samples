...
#include "MyVulkanManager.h"
#include "ShaderCompileUtil.h"
void ShaderQueueSuit_XRay::create_uniform_buffer(VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
    bufferByteCount=sizeof(float)*6;
...
void ShaderQueueSuit_XRay::destroy_uniform_buffer(VkDevice& device)
{
    vk::vkDestroyBuffer(device, uniformBuf, NULL);
    vk::vkFreeMemory(device, memUniformBuf, NULL);
}
void ShaderQueueSuit_XRay::create_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_XRay::destroy_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_XRay::init_descriptor_set(VkDevice& device)
{
...
void ShaderQueueSuit_XRay::create_shader(VkDevice& device)
{
    std::string vertStr= FileUtil::loadAssetStr("shader/XRay.vert");
    std::string fragStr= FileUtil::loadAssetStr("shader/XRay.frag");
...
void ShaderQueueSuit_XRay::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
...
void ShaderQueueSuit_XRay::destroy_pipe_line(VkDevice& device)
{
    vk::vkDestroyPipeline(device, pipeline, NULL);
    vk::vkDestroyPipelineCache(device, pipelineCache, NULL);
}
ShaderQueueSuit_XRay::ShaderQueueSuit_XRay(VkDevice* deviceIn,VkRenderPass& renderPass,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ShaderQueueSuit_XRay::~ShaderQueueSuit_XRay()
{
