...
VkDescriptorImageInfo MyVulkanManager::depthImageInfo;
...
VkFormat MyVulkanManager::colorFormat[3];
VkFormatProperties MyVulkanManager::colorFormatProps[3];
VkImage MyVulkanManager::colorImage[3];
VkDeviceMemory MyVulkanManager::memColor[3];
VkImageView MyVulkanManager::colorImageView[3];
VkDescriptorImageInfo MyVulkanManager::colorImageInfo[3];
VkFramebuffer*  MyVulkanManager::framebuffers;
VkFramebuffer MyVulkanManager::selfTexFramebuffer;
VkFramebuffer MyVulkanManager::selfTexFramebufferPeople;
ShaderQueueSuit_CommonTexLight_Depth* MyVulkanManager::sqsCTLD;
ShaderQueueSuit_CommonTexLight* MyVulkanManager::sqsCTL;
ShaderQueueSuit_XRay* MyVulkanManager::sqsXRay;
ShaderQueueSuit_TexRect* MyVulkanManager::sqsD2D;
ObjObject* MyVulkanManager::floor;
ObjObject* MyVulkanManager::wall;
ObjObject* MyVulkanManager::people;
TexDrawableObject* MyVulkanManager::d2dA;
float MyVulkanManager::objScale=0.0241f;
float MyVulkanManager::peopleScale=8.0f;
float MyVulkanManager::wallDis=85;
float MyVulkanManager::cx=0;
float MyVulkanManager::cy=20;
float MyVulkanManager::cz=35;
float MyVulkanManager::cxPrevious=0;
float MyVulkanManager::cyPrevious=0;
float MyVulkanManager::czPrevious=0;
...
void MyVulkanManager::create_render_pass_self()
{
    ...
    assert(result == VK_SUCCESS);
    clear_values_self[0].color.float32[0] = 0.2f;
    clear_values_self[0].color.float32[1] = 0.2f;
    clear_values_self[0].color.float32[2] = 0.2f;
    clear_values_self[0].color.float32[3] = 0.2f;
    clear_values_self[1].color.float32[0] = 0.2f;
    clear_values_self[1].color.float32[1] = 0.2f;
    clear_values_self[1].color.float32[2] = 0.2f;
    clear_values_self[1].color.float32[3] = 0.2f;
...
void MyVulkanManager::create_frame_buffer_screen()
{
    ...
    attachments[0] = colorImageView[1];
    VkResult result = vk::vkCreateFramebuffer(device, &fb_info, NULL, &selfTexFramebufferPeople);
    assert(result == VK_SUCCESS);
}
void MyVulkanManager::create_frame_buffer_self()
{
    VkImageView Selfattachments[3];
    VkFramebufferCreateInfo Selffb_info = {};
    Selffb_info.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
    Selffb_info.pNext = NULL;
    Selffb_info.renderPass = renderPassSelf;
    Selffb_info.attachmentCount = 3;
    Selffb_info.pAttachments = Selfattachments;
    Selffb_info.width = screenWidth;
    Selffb_info.height = screenHeight;
    Selffb_info.layers = 1;
    Selfattachments[0] = colorImageView[0];
    Selfattachments[1] = colorImageView[2];
    Selfattachments[2] = depthImageView;
    VkResult result = vk::vkCreateFramebuffer(device, &Selffb_info, NULL, &selfTexFramebuffer);
    assert(result == VK_SUCCESS);
}
...
void MyVulkanManager::createDrawableObject()
{
    ...
    people=LoadUtil::loadFromFile("model/people.obj",device, memoryroperties);
    ...
    d2dA=new TexDrawableObject(vdataIn,30*4,6,device, memoryroperties);
}
...
void MyVulkanManager::initMatrixAndLight()
{
    ...
    MatrixState3D::setCamera(cx,cy,cz, 0,20,35, 0,1,0);
    float ratio=(float)screenWidth/(float)screenHeight;
    MatrixState3D::setProjectFrustum(-ratio,ratio,-1,1,1.6f,3000);
    LightManager::setLightPosition(0,20,0);
    LightManager::setlightAmbient(0.9f,0.9f,0.9f,0.9f);
...
void MyVulkanManager::flushUniformBufferForToTex()
{
    ...
    uint8_t *pData;
    VkResult result = vk::vkMapMemory(device, sqsCTLD->memUniformBuf, 0, sqsCTLD->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCTLD->bufferByteCount);
    vk::vkUnmapMemory(device,sqsCTLD->memUniformBuf);
    result = vk::vkMapMemory(device, sqsCTL->memUniformBuf, 0, sqsCTL->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCTL->bufferByteCount);
    vk::vkUnmapMemory(device,sqsCTL->memUniformBuf);
    float vertexXRayUniformData[6]=
            {
                    CameraUtil::camera9Para[0],CameraUtil::camera9Para[1],CameraUtil::camera9Para[2],1.0,
                    (float)screenWidth,(float)screenHeight
            };
    result = vk::vkMapMemory(device, sqsXRay->memUniformBuf, 0, sqsXRay->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, vertexXRayUniformData, sqsXRay->bufferByteCount);
    vk::vkUnmapMemory(device,sqsXRay->memUniformBuf);
}
void MyVulkanManager::flushTexToDesSetForToTex()
{
    for(int i=0;i<TextureManager::texNamesForCommonTexLight.size();i++)
    {
        sqsCTLD->writes[0].dstSet = sqsCTLD->descSet[i];
        sqsCTLD->writes[1].dstSet = sqsCTLD->descSet[i];
        sqsCTLD->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForCommonTexLight[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsCTLD->writes, 0, NULL);
    }
    for(int i=0;i<TextureManager::texNamesForCommonTexLight.size();i++)
    {
    ...
    sqsXRay->writes[0].dstSet = sqsXRay->descSet[0];
    sqsXRay->writes[1].dstSet = sqsXRay->descSet[0];
    sqsXRay->writes[1].pImageInfo = &colorImageInfo[0];
    sqsXRay->writes[2].dstSet = sqsXRay->descSet[0];
    sqsXRay->writes[2].pImageInfo = &colorImageInfo[2];
    sqsXRay->writes[3].dstSet = sqsXRay->descSet[0];
    sqsXRay->writes[3].pImageInfo = &colorImageInfo[1];
    vk::vkUpdateDescriptorSets(device, 4, sqsXRay->writes, 0, NULL);
}
void MyVulkanManager::drawSceneToTex()//用于完成第一轮绘制的方法
{
    ...
    float ratio=(float)screenWidth/(float)screenHeight;
    MatrixState3D::setProjectFrustum(-ratio, ratio, -1, 1, 1.6f, 3000);
    MatrixState3D::setCamera(cx,cy,cz, 0,20,0, 0,1,0);
    MatrixState3D::pushMatrix();//保护现场
    MatrixState3D::scale(objScale,objScale,objScale);//进行缩放变换
    wall->drawSelf(cmdBuffer,sqsCTLD->pipelineLayout,sqsCTLD->pipeline,//绘制墙壁
                    &(sqsCTLD->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/brick.bntex")]));
    floor->drawSelf(cmdBuffer,sqsCTLD->pipelineLayout,sqsCTLD->pipeline,//绘制地板
                    &(sqsCTLD->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/floor.bntex")]));
    MatrixState3D::popMatrix();//恢复现场
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(wallDis,80,0);
    ...
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(0,80,wallDis);
...
void MyVulkanManager::drawPeopleToTex()//用于完成第二轮绘制的方法
{
    rp_begin_screen.framebuffer = selfTexFramebufferPeople;
    vk::vkResetCommandBuffer(cmdBuffer, 0);
    VkResult result = vk::vkBeginCommandBuffer(cmdBuffer, &cmd_buf_info);
    MyVulkanManager::flushUniformBufferForToTex();
    MyVulkanManager::flushTexToDesSetForToTex();
    ...
    czPrevious=cz;
    MatrixState3D::pushMatrix();//保护现场
    MatrixState3D::translate(10,0,0);//进行平移变换
    MatrixState3D::rotate(90,0,1,0);//进行旋转变换
    MatrixState3D::scale(peopleScale,peopleScale,peopleScale);//进行缩放变换
    people->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,//绘制人物
                     &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/people.bntex")]));
    MatrixState3D::popMatrix();//恢复现场
    vk::vkCmdEndRenderPass(cmdBuffer);//结束渲染通道
    result = vk::vkEndCommandBuffer(cmdBuffer);
    submit_info[0].waitSemaphoreCount = 0;
...
void MyVulkanManager::drawSceneToScreen()//用于完成第三轮及第四轮绘制的方法
{
    ...
    MatrixState3D::pushMatrix();//保护现场
    d2dA->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));//基于纹理矩形绘制场景到屏幕(不含人物)
    MatrixState3D::popMatrix();//恢复现场
    MatrixState3D::setProjectFrustum(-ratio, ratio, -1, 1, 1.6f, 3000);
    MatrixState3D::setCamera(cxPrevious,cyPrevious,czPrevious, 0,20,0, 0,1,0);
    MatrixState3D::pushMatrix();//保护现场
    MatrixState3D::translate(10,0,0);//进行平移变换
    MatrixState3D::rotate(90,0,1,0);//进行旋转变换
    MatrixState3D::scale(peopleScale,peopleScale,peopleScale);//进行缩放变换
...
{
    sqsCTLD=new ShaderQueueSuit_CommonTexLight_Depth(&device,renderPassSelf,memoryroperties);
    sqsCTL=new ShaderQueueSuit_CommonTexLight(&device,renderPassScreen,memoryroperties);
    sqsXRay=new ShaderQueueSuit_XRay(&device,renderPassScreen,memoryroperties);
    sqsD2D=new ShaderQueueSuit_TexRect(&device,renderPassScreen,memoryroperties);
}
...
void MyVulkanManager::create_vulkan_SelfColorBuffer() {
    VkFormat colorFormat[3] = { VK_FORMAT_R8G8B8A8_UNORM,VK_FORMAT_R8G8B8A8_UNORM,VK_FORMAT_R16_SFLOAT};
    for(int i=0;i<3;i++)
    {
