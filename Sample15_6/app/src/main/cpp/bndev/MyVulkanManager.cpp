...
ObjObject*  MyVulkanManager::base;
ObjObject*  MyVulkanManager::airship;
Dashboard2DObject* MyVulkanManager::d2dA;
float MyVulkanManager::isBloom=0;
float MyVulkanManager::rotateIndex;
...
{
    VkFormat colorFormat[2] = { VK_FORMAT_R8G8B8A8_UNORM,VK_FORMAT_R8G8B8A8_UNORM};
...
{
    base=LoadUtil::loadFromFile("model/base.obj",device,memoryroperties);
    airship=LoadUtil::loadFromFile("model/airship.obj",device,memoryroperties);
...
void MyVulkanManager::initMatrixAndLight()
{
    ...
    MatrixState2D::setProjectOrtho(-ratio,ratio,-1,1,1.0f,1000);
    MatrixState2D::setCamera(0,0,5,0,0,0,0,1,0);
    LightManager::setLightPosition(CameraUtil::camera9Para[0],CameraUtil::camera9Para[1],CameraUtil::camera9Para[2]);
...
{
    for(int i=0;i<TextureManager::texNamesForCommonTexLight.size()/2;i++)
    {
        ...
        sqsCTL->writes[2].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForCommonTexLight[i+2]]);
        vk::vkUpdateDescriptorSets(device, 3, sqsCTL->writes, 0, NULL);
    }
}
void MyVulkanManager::drawSceneToTex(){ 
    ...
    MatrixState3D::scale(40,40,40);
    base->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                    &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/base.bntex")]));
    MatrixState3D::popMatrix(); 
    MatrixState3D::pushMatrix(); 
    MatrixState3D::translate(0,50,0);
    MatrixState3D::rotate(rotateIndex,0,1,0);
    MatrixState3D::scale(0.06,0.06,0.06);
    airship->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                 &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/airship.bntex")]));
    MatrixState3D::popMatrix(); 
...
void MyVulkanManager::drawSceneToScreen()
{
    ...
    d2dA->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,
                   &(sqsD2D->descSet[0])); 
...
void MyVulkanManager::create_vulkan_SelfColorBuffer() {
    VkFormat colorFormat[3] = { VK_FORMAT_R8G8B8A8_UNORM,VK_FORMAT_R8G8B8A8_UNORM};
    for(int i=0;i<2;i++)
    {
...
{
    float fragmentUniformData[1]={isBloom};
    uint8_t *pData;
