...
ObjObjectQiu::ObjObjectQiu(float* vdataIn,int dataByteCount,int vCountIn,VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ObjObjectQiu::~ObjObjectQiu()
{
    delete [] vdata;
    vk::vkDestroyBuffer(*devicePointer, vertexDatabuf, NULL);
    vk::vkFreeMemory(*devicePointer, vertexDataMem, NULL);
}
void ObjObjectQiu::drawSelf(VkCommandBuffer& cmd,VkPipelineLayout& pipelineLayout,VkPipeline& pipeline,VkDescriptorSet* desSetPointer)
{
    ...
    float* vp=MatrixState3D::getVPMatrix();
    memcpy(pushConstantData, vp, sizeof(float)*16);
    vk::vkCmdPushConstants(cmd, pipelineLayout,VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(float)*16,pushConstantData);
    vk::vkCmdDraw(cmd, vCount, PlanetConstant::paiNumber, 0, 0);//采用实例渲染绘制海量小星体
}