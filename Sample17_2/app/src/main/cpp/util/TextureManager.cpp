...
std::vector<std::string> TextureManager::texNames={"texture/qiu.bntex","texture/stone.bntex","texture/stone2.bntex","texture/stone3.bntex","texture/noise2.bntex","texture/noise1.bntex","texture/noise3.bntex"};
std::vector<std::string> TextureManager::doubleStone={"texture/stone.bntex","texture/noise1.bntex","texture/stone2.bntex","texture/noise2.bntex","texture/stone3.bntex","texture/noise3.bntex"};
...
{
    int result=-1;
    for(int i=0;i<doubleStone.size();i++)
    {
        if(doubleStone[i].compare(texName.c_str())==0)//与内容字符串进行比较
        {
            result=i;
            break;
        }
