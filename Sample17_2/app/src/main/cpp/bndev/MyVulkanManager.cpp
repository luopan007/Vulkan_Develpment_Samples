...
ShaderQueueSuit_CommonQiu* MyVulkanManager::sqsQiuTwo;
...
void MyVulkanManager::flushUniformBuffer()
{
    ...
    if(PlanetConstant::perAngle>=360){
        PlanetConstant::perAngle=0;
    }
    PlanetConstant::perAngle+=0.8;

    if(PlanetConstant::rotateAngle>=360){
        PlanetConstant::rotateAngle=0.0f;
    }
    PlanetConstant::rotateAngle+=0.05;
    float qiuUniformData[27]=
            {
                    MatrixState3D::cx,MatrixState3D::cy,MatrixState3D::cz,1.0,
                    LightManager::lx,LightManager::ly,LightManager::lz,1.0,
                    LightManager::lightAmbientR,LightManager::lightAmbientG,LightManager::lightAmbientB,LightManager::lightAmbientA,
                    LightManager::lightDiffuseR,LightManager::lightDiffuseG,LightManager::lightDiffuseB,LightManager::lightDiffuseA,
                    LightManager::lightSpecularR,LightManager::lightSpecularG,LightManager::lightSpecularB,LightManager::lightSpecularA,
                    PlanetConstant::perAngle,PlanetConstant::rotateAngle,PlanetConstant::groupNumber,PlanetConstant::oneGroupNumber,PlanetConstant::distanceDai,PlanetConstant::linWidthSpan,
                    PlanetConstant::angleSpan
            };
    result = vk::vkMapMemory(device, sqsQiu->memUniformBuf,0, sqsQiu->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, qiuUniformData, sqsQiu->bufferByteCount);
    vk::vkUnmapMemory(device,sqsQiu->memUniformBuf);

    qiuUniformData[24]=PlanetConstant::distanceDaiBig;

    result = vk::vkMapMemory(device, sqsQiuTwo->memUniformBuf,0, sqsQiuTwo->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, qiuUniformData, sqsQiuTwo->bufferByteCount);
    vk::vkUnmapMemory(device,sqsQiuTwo->memUniformBuf);
...
void MyVulkanManager::flushTexToDesSet()
{
    ...
    for(int i=0;i<TextureManager::doubleStone.size()/2;i++)
    {
        sqsQiu->writes[0].dstSet = sqsQiu->descSet[i];
        sqsQiu->writes[1].dstSet = sqsQiu->descSet[i];
        sqsQiu->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::doubleStone[i*2+0]]);//加载第一个纹理
        sqsQiu->writes[2].dstSet = sqsQiu->descSet[i];
        sqsQiu->writes[2].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::doubleStone[i*2+1]]);//加载第二个纹理
        vk::vkUpdateDescriptorSets(device, 3, sqsQiu->writes, 0, NULL);

        sqsQiuTwo->writes[0].dstSet = sqsQiuTwo->descSet[i];
        sqsQiuTwo->writes[1].dstSet = sqsQiuTwo->descSet[i];
        sqsQiuTwo->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::doubleStone[i*2+0]]);//加载第一个纹理
        sqsQiuTwo->writes[2].dstSet = sqsQiuTwo->descSet[i];
        sqsQiuTwo->writes[2].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::doubleStone[i*2+1]]);//加载第二个纹理
        vk::vkUpdateDescriptorSets(device, 3, sqsQiuTwo->writes, 0, NULL);
    }
}
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        MatrixState3D::pushMatrix();
        objQiu->drawSelf(cmdBuffer,sqsQiu->pipelineLayout,sqsQiu->pipeline,
                         &(sqsQiu->descSet[0]));
        objQiuTwo->drawSelf(cmdBuffer,sqsQiu->pipelineLayout,sqsQiu->pipeline,
                          &(sqsQiu->descSet[1]));
        objQiuThree->drawSelf(cmdBuffer,sqsQiu->pipelineLayout,sqsQiu->pipeline,
                            &(sqsQiu->descSet[2]));
        MatrixState3D::popMatrix();

        MatrixState3D::pushMatrix();
        objQiu->drawSelf(cmdBuffer,sqsQiuTwo->pipelineLayout,sqsQiuTwo->pipeline,
                         &(sqsQiuTwo->descSet[0]));
        objQiuTwo->drawSelf(cmdBuffer,sqsQiuTwo->pipelineLayout,sqsQiuTwo->pipeline,
                            &(sqsQiuTwo->descSet[1]));
        objQiuThree->drawSelf(cmdBuffer,sqsQiuTwo->pipelineLayout,sqsQiuTwo->pipeline,
                              &(sqsQiuTwo->descSet[2]));
        MatrixState3D::popMatrix();
...
void MyVulkanManager::initPipeline()
{
    ...
    sqsQiuTwo=new ShaderQueueSuit_CommonQiu(&device,renderPass,memoryroperties);
}
