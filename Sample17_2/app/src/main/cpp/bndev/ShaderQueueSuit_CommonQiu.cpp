...
#include <regex>
void ShaderQueueSuit_CommonQiu::create_uniform_buffer(VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
    bufferByteCount=27*sizeof(float);
...
void ShaderQueueSuit_CommonQiu::destroy_uniform_buffer(VkDevice& device)
{
    vk::vkDestroyBuffer(device, uniformBuf, NULL);
    vk::vkFreeMemory(device, memUniformBuf, NULL);
}
void ShaderQueueSuit_CommonQiu::create_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonQiu::destroy_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonQiu::init_descriptor_set(VkDevice& device)
{
    VkDescriptorPoolSize type_count[3];
    type_count[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    type_count[0].descriptorCount = TextureManager::doubleStone.size()/2;
    type_count[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[1].descriptorCount = TextureManager::doubleStone.size()/2;
    type_count[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[1].descriptorCount = TextureManager::doubleStone.size()/2;
    VkDescriptorPoolCreateInfo descriptor_pool = {};
    descriptor_pool.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    descriptor_pool.pNext = NULL;
    descriptor_pool.maxSets = TextureManager::doubleStone.size()/2;
    ...
    alloc_info[0].descriptorSetCount = TextureManager::doubleStone.size()/2;
    alloc_info[0].pSetLayouts = layouts.data();
    descSet.resize(TextureManager::doubleStone.size()/2);
...
void ShaderQueueSuit_CommonQiu::create_shader(VkDevice& device)
{
    std::string vertStr= FileUtil::loadAssetStr("shader/commonTexQiu.vert");
    std::string fragStr= FileUtil::loadAssetStr("shader/commonTexQiu.frag");
...
void ShaderQueueSuit_CommonQiu::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
...
void ShaderQueueSuit_CommonQiu::destroy_pipe_line(VkDevice& device)
{
    vk::vkDestroyPipeline(device, pipeline, NULL);
    vk::vkDestroyPipelineCache(device, pipelineCache, NULL);
}
ShaderQueueSuit_CommonQiu::ShaderQueueSuit_CommonQiu(VkDevice* deviceIn,VkRenderPass& renderPass,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ShaderQueueSuit_CommonQiu::~ShaderQueueSuit_CommonQiu()
{
