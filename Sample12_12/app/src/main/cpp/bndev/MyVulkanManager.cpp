...
ObjObject*  MyVulkanManager::objectDraw;
...
{
    objectDraw=LoadUtil::loadFromFileVertexAndNormal("model/ch.obj",device, memoryroperties);
}
...
void MyVulkanManager::initMatrixAndLight()
{
    ...
    LightManager::setLightPosition(0,0,10);
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        MatrixState3D::scale(5.0f,5.0f,5.0f);
        objectDraw->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/3dNoise.bn3dtex")]));
