...
ObjObject* LoadUtil::loadFromFileVertex(const string& vname,VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties) {
    ObjObject *lo;
    vector<float> alv;
    vector<float> alvResult;
    std::string resultStr = FileUtil::loadAssetStr(vname);
...
ObjObject* LoadUtil::loadFromFileVertexAndTex(const string& vname,VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties) {
    ObjObject *lo;
    vector<float> alv;
    vector<float> alvResult;
    vector<float> alt;
    ...
    int vCount = (int) alvResult.size() / 3;
    int dataByteCount = vCount * 5 * sizeof(float);
    float *vdataIn = new float[vCount * 5];
    int indexTemp = 0;
    for (int i = 0; i < vCount; i++) {
        vdataIn[indexTemp++] = alvResult[i * 3 + 0];
        vdataIn[indexTemp++] = alvResult[i * 3 + 1];
...
ObjObject* LoadUtil::loadFromFileVertexAndNormal(const string& vname,VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties) {
    ObjObject *lo;
    vector<float> alv;
    vector<float> alvResult;
    vector<float> aln;
