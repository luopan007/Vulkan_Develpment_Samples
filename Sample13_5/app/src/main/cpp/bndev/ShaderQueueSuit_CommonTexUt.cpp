#include "ShaderQueueSuit_CommonTexUt.h"
#include <assert.h>
...
void ShaderQueueSuit_CommonTexUt::create_uniform_buffer(VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
    bufferByteCount=20*sizeof(float);
...
void ShaderQueueSuit_CommonTexUt::destroy_uniform_buffer(VkDevice& device)
{
...
void ShaderQueueSuit_CommonTexUt::create_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonTexUt::destroy_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonTexUt::init_descriptor_set(VkDevice& device)
{
    ...
    type_count[2].descriptorCount = TextureManager::texNames.size()/2;
...
void ShaderQueueSuit_CommonTexUt::create_shader(VkDevice& device)
{
    std::string vertStr= FileUtil::loadAssetStr("shader/vertex_ut.vert");
    std::string fragStr= FileUtil::loadAssetStr("shader/frag_ut.frag");
...
void ShaderQueueSuit_CommonTexUt::destroy_shader(VkDevice& device)
{
    vk::vkDestroyShaderModule(device, shaderStages[0].module, NULL);
    vk::vkDestroyShaderModule(device, shaderStages[1].module, NULL);
}

void ShaderQueueSuit_CommonTexUt::initVertexAttributeInfo()
{
    ...
    vertexAttribs[3].format = VK_FORMAT_R32G32B32_SFLOAT;
    vertexAttribs[3].offset = 32;
}

void ShaderQueueSuit_CommonTexUt::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
...
void ShaderQueueSuit_CommonTexUt::destroy_pipe_line(VkDevice& device)
{
...
ShaderQueueSuit_CommonTexUt::ShaderQueueSuit_CommonTexUt(VkDevice* deviceIn,VkRenderPass& renderPass,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ShaderQueueSuit_CommonTexUt::~ShaderQueueSuit_CommonTexUt()
{
