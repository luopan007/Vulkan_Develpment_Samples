...
void setImageLayoutCube(VkCommandBuffer cmd, VkImage image,
                    ...
                    VkImageLayout new_image_layout)//设置立方图纹理布局
{
    ...
    image_memory_barrier.subresourceRange.layerCount = 6;//数组层的数量
...
std::vector<std::string> TextureManager::texNames={"texture/skycubemap_left.bntex","texture/skycubemap_right.bntex","texture/skycubemap_down.bntex","texture/skycubemap_up.bntex","texture/skycubemap_back.bntex","texture/skycubemap_front.bntex","texture/cube.bntexcube"};
std::vector<std::string> TextureManager::texNamesForCubeMap={"texture/cube.bntexcube"};
std::vector<std::string> TextureManager::texNamesForSkyBox={"texture/skycubemap_left.bntex","texture/skycubemap_right.bntex","texture/skycubemap_down.bntex","texture/skycubemap_up.bntex","texture/skycubemap_back.bntex","texture/skycubemap_front.bntex","texture/cube.bntexcube"};
...
void TextureManager::initCubemapTextures(std::string texName, VkDevice& device, VkPhysicalDevice& gpu, VkPhysicalDeviceMemoryProperties& memoryroperties, VkCommandBuffer& cmdBuffer, VkQueue& queueGraphics, VkFormat format, TexDataObject* ctdo)
{
    ...
    image_create_info.extent.depth = 1;
    image_create_info.mipLevels = 1;
    image_create_info.arrayLayers = 6;//图像数组层数量
    image_create_info.samples = VK_SAMPLE_COUNT_1_BIT;
    image_create_info.tiling = VK_IMAGE_TILING_OPTIMAL;
    image_create_info.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
    ...
    bufferCopyRegion.imageSubresource.mipLevel = 0;
    bufferCopyRegion.imageSubresource.baseArrayLayer = 0;
    bufferCopyRegion.imageSubresource.layerCount = 6;//数组层的数量
    bufferCopyRegion.imageExtent.width = ctdo->width;
    bufferCopyRegion.imageExtent.height = ctdo->height;
    bufferCopyRegion.imageExtent.depth = 1;
    ...
    vk::vkResetCommandBuffer(cmdBuffer, 0);//重置命令缓冲

    result = vk::vkBeginCommandBuffer(cmdBuffer, &cmd_buf_info);//启动命令缓冲

    setImageLayoutCube(cmdBuffer, textureImage, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);//修改图像布局
    ...
    view_info.subresourceRange.levelCount = 1;
    view_info.subresourceRange.baseArrayLayer = 0;
    view_info.subresourceRange.layerCount = 6;//数组层的数量
    view_info.image = textureImageList[texName];
...
void TextureManager::initTextures(VkDevice& device, VkPhysicalDevice& gpu, VkPhysicalDeviceMemoryProperties& memoryroperties, VkCommandBuffer& cmdBuffer, VkQueue& queueGraphics)
{
    ...
    for (int i = 0; i<texNames.size(); i++)//遍历纹理数组
    {
