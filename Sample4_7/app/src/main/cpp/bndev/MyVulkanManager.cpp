...
DrawableObjectCommon*  MyVulkanManager::objectForDraw;
...
void MyVulkanManager::create_vulkan_devices()
{
    ...
    if (pdf.wideLines == VK_TRUE)							//判断是否支持wideLines特性
   	    {LOGE("支持wideLines特性！\n");}						//打印调试信息
    else{LOGE("不支持wideLines特性！\n");}
    VkDeviceCreateInfo deviceInfo = {};  					//构建逻辑设备创建信息结构体实例
    deviceInfo.sType = VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO;//指定结构体类型
    deviceInfo.pNext = NULL;								//自定义数据的指针
    deviceInfo.queueCreateInfoCount = 1;					//指定设备队列创建信息结构体数量
    deviceInfo.pQueueCreateInfos = &queueInfo;				//指定设备队列创建信息结构体列表
    ...
    deviceInfo.enabledLayerCount = 0;								//启动的Layer数量
    deviceInfo.ppEnabledLayerNames = NULL;						//启动的Layer名称列表
...
void MyVulkanManager::createDrawableObject()
{
    ...
    objectForDraw=new DrawableObjectCommon(ObjectData::vdata,ObjectData::dataByteCount,ObjectData::vCount,device,memoryroperties);
}


void MyVulkanManager::destroyDrawableObject()
{
    delete objectForDraw;
}
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        objectForDraw->drawSelf(cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline[topologyWay],&(sqsCL->descSet[0]));
