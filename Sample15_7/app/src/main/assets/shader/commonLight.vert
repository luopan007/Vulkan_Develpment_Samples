...
layout (location = 5) out vec3 vlight;
layout (location = 6) out vec2 outTexCoor;
layout (location = 7) out float outToneMapping;
out gl_PerVertex {
	vec4 gl_Position;
};
...
void main() {
   ...
   if(myConstantVals.isShadow==1){						//标志位为1，则绘制阴影
    ...
    vPosition=pos;
    vNormal=inNormal;
    vlight=myBufferVals.light.xyz;
    outLightPosition=myBufferVals.lightPosition;
    shadow=myConstantVals.isShadow;
    outTexCoor = inTexCoor;
    outToneMapping=myBufferVals.uCamera.w;
}
