 ...
 * @param tx ty tz为树干位置
 * */
TexTrunkDrawableObject::TexTrunkDrawableObject(float* vdataIn,int dataByteCount,int vCountIn,VkDevice& device,
...
TexTrunkDrawableObject::~TexTrunkDrawableObject()
{
    delete vdata;
    vk::vkDestroyBuffer(*devicePointer, vertexDatabuf, NULL);
    vk::vkFreeMemory(*devicePointer, vertexDataMem, NULL);
}
void TexTrunkDrawableObject::drawSelf(VkCommandBuffer& cmd,VkPipelineLayout& pipelineLayout,VkPipeline& pipeline,VkDescriptorSet* desSetPointer)
{
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(centerx,centery,centerz);
