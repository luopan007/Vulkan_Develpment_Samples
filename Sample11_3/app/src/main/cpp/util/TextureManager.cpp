...
#include <assert.h>
std::vector<std::string> TextureManager::texNames={"texture/treetrunk.bntex","texture/leaf.bntex", "texture/water.bntex","texture/sky.bntex","texture/caodi.bntex","texture/sand.bntex"};
std::vector<std::string> TextureManager::orTexNames={"texture/sky.bntex"};
std::vector<std::string> TextureManager::waterNames={"texture/water.bntex"};
std::vector<std::string> TextureManager::doubleNames={"texture/caodi.bntex","texture/sand.bntex"};
...
{
    int result=-1;
    for(int i=0;i<orTexNames.size();i++)
    {
        if(orTexNames[i].compare(texName.c_str())==0)
        {
            result=i;
            break;
        }
...
int TextureManager::getVkDescriptorWaterSetIndex(std::string texName)
{
    int result=-1;
    for(int i=0;i<waterNames.size();i++)
    {
        if(waterNames[i].compare(texName.c_str())==0)//与内容字符串进行比较
        {
            result=i;
            break;
        }
...
int TextureManager::getVkDescriptorDoubleSetIndex(std::string texName)
{
    int result=-1;
    for(int i=0;i<doubleNames.size();i++)
    {
        if(doubleNames[i].compare(texName.c_str())==0)//与内容字符串进行比较
        {
            result=i;
            break;
        }
