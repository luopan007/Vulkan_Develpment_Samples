...
ShaderQueueSuit_CommonTex_Xie* MyVulkanManager::sqsXie;
ShaderQueueSuit_CommonTexDouble* MyVulkanManager::sqsDou;
ShaderQueueSuit_CommonTexTrunk* MyVulkanManager::sqsTrunk;
ShaderQueueSuit_CommonTexBlend* MyVulkanManager::sqsBlend;
TexDrawableObject* MyVulkanManager::landForDraw;
TexDrawableObject* MyVulkanManager::waterForDraw;
TexDrawableObject* MyVulkanManager::skyForDraw;
TreeControl* MyVulkanManager::treeControl;
float uWidthSpan = 12.0f;//宽度步进
float uStartAngle = 0.0f;//水面起始角度
float direction_degree=30;//风的方向(XOZ平面的角度)
float wind_speed_init=200;
float wind_speed=wind_speed_init;//风的大小
float wind_force_init=-2.0f;
float wind_force_init2=-4.0f;
float wind_force=wind_force_init;//风的步进
float bend_R_max=10000;//最大完全半径
float bend_R_min=100;//最大完全半径
float bend_R=bend_R_max;//树的弯曲半径
...
{
    sqsCT=new ShaderQueueSuit_CommonTex(&device,renderPass,memoryroperties);//创建普通纹理管线
    sqsXie=new ShaderQueueSuit_CommonTex_Xie(&device,renderPass,memoryroperties);//创建水面管线
    sqsDou=new ShaderQueueSuit_CommonTexDouble(&device,renderPass,memoryroperties);//双纹理管线
    sqsBlend=new ShaderQueueSuit_CommonTexBlend(&device,renderPass,memoryroperties);//创建混合管线
    sqsTrunk=new ShaderQueueSuit_CommonTexTrunk(&device,renderPass,memoryroperties);//创建树管线
}
...
{
    TreeTrunkData::genVertexData(Constant::bottom_radius,Constant::joint_height,Constant::joint_num,Constant::joint_available_num);//初始化树干数据
    TreeLeaves::genVertexData(Constant::leaves_width,Constant::leaves_height,Constant::TREE_HEIGHT);//初始化树叶数据
    treeControl=new TreeControl();//创建树的绘制者
    float treePostion[4][3]={//记录树位置的数组
            0,0,0,
            -50,0,50,
            -100,0,0,
            -150,0,0
    };
    for(int i=0;i<4;i++){//循环创建树干
        treeControl->trunkVector.push_back(
                new TexTrunkDrawableObject(TreeTrunkData::vdata, TreeTrunkData::dataByteCount, TreeTrunkData::vCount, device, memoryroperties,treePostion[i][0],treePostion[i][1],treePostion[i][2]));//创建树干
    }
    for(int i=0;i<treeControl->trunkVector.size();i++){//循环创建树叶
        for(int j=0;j<6;j++){
            treeControl->leafVector.push_back(
         new TexLeafDrawableObject(TreeLeaves::vdata, TreeLeaves::dataByteCount, TreeLeaves::vCount, device, memoryroperties,treePostion[i][0],treePostion[i][1],treePostion[i][2],j*60,j)
        ); }
    }
    LandData* ld = FileUtil::loadHdtData("texture/land1.bnhdt");//初始化灰度图
    landForDraw=new TexDrawableObject(ld->vData, ld->vCount*5*4, ld->vCount, device, memoryroperties);//山体绘制者
    FlatData::genVertexData();//初始化水面数据
    waterForDraw=new TexDrawableObject(FlatData::vdata, FlatData::dataByteCount, FlatData::vCount, device, memoryroperties);//水面绘制者
    SkyBallData::genVertexData();
    skyForDraw=new TexDrawableObject(SkyBallData::vdata, SkyBallData::dataByteCount, SkyBallData::vCount, device, memoryroperties);//水面绘制者
...
void MyVulkanManager::flushUniformBuffer()
{
    ...
    /////////////////水面相关一致变量数据//////////////////
    float uniformWaterData[2];
    if (uStartAngle < 3) {
        uStartAngle -= 0.1f;
    }if(uStartAngle > 0) {
        uStartAngle += 0.1f;
    }
    uniformWaterData[0] = uWidthSpan;
    uniformWaterData[1] = uStartAngle;
    uint8_t *pData;
    VkResult result = vk::vkMapMemory(device, sqsXie->memUniformBuf, 0, sqsXie->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, uniformWaterData, sqsXie->bufferByteCount);
    vk::vkUnmapMemory(device, sqsXie->memUniformBuf);
    /////////////////树干相关一致变量数据//////////////////
    float uniformTreeData[2];
    //设置椰子树的左右摆动
    ...
    uniformTreeData[0]=bend_R;
    uniformTreeData[1]=direction_degree;
    result = vk::vkMapMemory(device, sqsTrunk->memUniformBuf, 0, sqsXie->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, uniformTreeData, sqsTrunk->bufferByteCount);
    vk::vkUnmapMemory(device, sqsTrunk->memUniformBuf);
}
void MyVulkanManager::flushTexToDesSet()
{
    //加载普通纹理
    for(int i=0;i<TextureManager::orTexNames.size();i++)
    {
        sqsCT->writes[0].dstSet = sqsCT->descSet[i];
        sqsCT->writes[1].dstSet = sqsCT->descSet[i];
        sqsCT->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::orTexNames[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsCT->writes, 0, NULL);
    }
    //加载水面的纹理
    for(int i=0;i<TextureManager::waterNames.size();i++)
    {
        sqsXie->writes[0].dstSet = sqsXie->descSet[i];
        sqsXie->writes[1].dstSet = sqsXie->descSet[i];
        sqsXie->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::waterNames[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsXie->writes, 0, NULL);
    }
    //加载双纹理
    for(int i=0;i<TextureManager::doubleNames.size()/2;i++)
    {
        sqsDou->writes[0].dstSet = sqsDou->descSet[i];
        sqsDou->writes[1].dstSet = sqsDou->descSet[i];
        sqsDou->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::doubleNames[i*2+0]]);//加载第一个纹理
        sqsDou->writes[2].dstSet = sqsDou->descSet[i];
        sqsDou->writes[2].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::doubleNames[i*2+1]]);//加载第二个纹理
        vk::vkUpdateDescriptorSets(device, 3, sqsDou->writes, 0, NULL);
    }
        //送入树叶纹理
        sqsBlend->writes[0].dstSet = sqsBlend->descSet[0];
        sqsBlend->writes[1].dstSet = sqsBlend->descSet[0];
        sqsBlend->writes[1].pImageInfo = &(TextureManager::texImageInfoList["texture/leaf.bntex"]);
        vk::vkUpdateDescriptorSets(device, 2, sqsBlend->writes, 0, NULL);
        //送入树干纹理
        sqsTrunk->writes[0].dstSet = sqsTrunk->descSet[0];
        sqsTrunk->writes[1].dstSet = sqsTrunk->descSet[0];
        sqsTrunk->writes[1].pImageInfo = &(TextureManager::texImageInfoList["texture/treetrunk.bntex"]);
        vk::vkUpdateDescriptorSets(device, 2, sqsTrunk->writes, 0, NULL);
}
...
void MyVulkanManager::doDrawSelf(){
    MatrixState3D::pushMatrix();//绘制草地
    MatrixState3D::translate(0,-100,0);
    landForDraw->drawSelf(cmdBuffer,sqsDou->pipelineLayout, sqsDou->pipeline, &(sqsDou->descSet[0]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();//绘制水面
    MatrixState3D::translate(0,Constant::water_height,0);
    MatrixState3D::rotate(-90,1,0,0);
    waterForDraw->drawSelf(cmdBuffer,sqsXie->pipelineLayout, sqsXie->pipeline, &(sqsXie->descSet[TextureManager::getVkDescriptorWaterSetIndex("texture/water.bntex")]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();//绘制天空穹
    skyForDraw->drawSelf(cmdBuffer,sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/sky.bntex")]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();//绘制树
    treeControl->drawSelf(bend_R,direction_degree);
    MatrixState3D::popMatrix();
}
