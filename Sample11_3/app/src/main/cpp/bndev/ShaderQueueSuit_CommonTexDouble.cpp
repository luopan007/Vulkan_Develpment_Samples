...
//创建Unifirm Buffer
void ShaderQueueSuit_CommonTexDouble::create_uniform_buffer(VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
//销毁Unifirm Buffer相关
void ShaderQueueSuit_CommonTexDouble::destroy_uniform_buffer(VkDevice& device)
{
...
//创建管线layout
void ShaderQueueSuit_CommonTexDouble::create_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonTexDouble::destroy_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonTexDouble::init_descriptor_set(VkDevice& device)
{
    VkDescriptorPoolSize type_count[3];
    type_count[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    type_count[0].descriptorCount = TextureManager::doubleNames.size()/2;
    type_count[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[1].descriptorCount = TextureManager::doubleNames.size()/2;
    type_count[2].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[2].descriptorCount = TextureManager::doubleNames.size()/2;

    VkDescriptorPoolCreateInfo descriptor_pool = {};
    descriptor_pool.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    descriptor_pool.pNext = NULL;
    descriptor_pool.maxSets = TextureManager::doubleNames.size()/2;
    ...
    alloc_info[0].descriptorSetCount = TextureManager::doubleNames.size()/2;
    alloc_info[0].pSetLayouts = layouts.data();

    descSet.resize(TextureManager::doubleNames.size()/2);
...
void ShaderQueueSuit_CommonTexDouble::create_shader(VkDevice& device)
{
...
void ShaderQueueSuit_CommonTexDouble::destroy_shader(VkDevice& device)
{
    vk::vkDestroyShaderModule(device, shaderStages[0].module, NULL);
    vk::vkDestroyShaderModule(device, shaderStages[1].module, NULL);
}

void ShaderQueueSuit_CommonTexDouble::initVertexAttributeInfo()
{
...
void ShaderQueueSuit_CommonTexDouble::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
...
void ShaderQueueSuit_CommonTexDouble::destroy_pipe_line(VkDevice& device)
{
    //销毁管线
...
ShaderQueueSuit_CommonTexDouble::ShaderQueueSuit_CommonTexDouble(VkDevice* deviceIn,VkRenderPass& renderPass,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ShaderQueueSuit_CommonTexDouble::~ShaderQueueSuit_CommonTexDouble()
{
