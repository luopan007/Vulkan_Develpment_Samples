...
void ShaderQueueSuit_CommonTex_Xie::create_uniform_buffer(VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
void ShaderQueueSuit_CommonTex_Xie::destroy_uniform_buffer(VkDevice& device)
{
    vk::vkDestroyBuffer(device, uniformBuf, NULL);
    vk::vkFreeMemory(device, memUniformBuf, NULL);
}
void ShaderQueueSuit_CommonTex_Xie::create_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonTex_Xie::destroy_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonTex_Xie::init_descriptor_set(VkDevice& device)
{
    VkDescriptorPoolSize type_count[2];
    type_count[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    type_count[0].descriptorCount = TextureManager::waterNames.size();
    type_count[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[1].descriptorCount = TextureManager::waterNames.size();

    VkDescriptorPoolCreateInfo descriptor_pool = {};
    descriptor_pool.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    descriptor_pool.pNext = NULL;
    descriptor_pool.maxSets = TextureManager::waterNames.size();
    ...
    alloc_info[0].descriptorSetCount = TextureManager::waterNames.size();
    alloc_info[0].pSetLayouts = layouts.data();
    descSet.resize(TextureManager::waterNames.size());
...
void ShaderQueueSuit_CommonTex_Xie::create_shader(VkDevice& device)
{
    std::string vertStr= FileUtil::loadAssetStr("shader/commonTexXie.vert");
    std::string fragStr= FileUtil::loadAssetStr("shader/commonTexXie.frag");
...
void ShaderQueueSuit_CommonTex_Xie::destroy_shader(VkDevice& device)
{
    vk::vkDestroyShaderModule(device, shaderStages[0].module, NULL);
    vk::vkDestroyShaderModule(device, shaderStages[1].module, NULL);
}
void ShaderQueueSuit_CommonTex_Xie::initVertexAttributeInfo()
{
...
void ShaderQueueSuit_CommonTex_Xie::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
...
void ShaderQueueSuit_CommonTex_Xie::destroy_pipe_line(VkDevice& device)
{
    vk::vkDestroyPipeline(device, pipeline, NULL);
    vk::vkDestroyPipelineCache(device, pipelineCache, NULL);
}
ShaderQueueSuit_CommonTex_Xie::ShaderQueueSuit_CommonTex_Xie(VkDevice* deviceIn,VkRenderPass& renderPass,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ShaderQueueSuit_CommonTex_Xie::~ShaderQueueSuit_CommonTex_Xie()
{
