...
void MyVulkanManager::createDrawableObject()
{
    ...
    cirForDraw=new DrawableObjectCommonLight(CircleData::vdata,CircleData::dataByteCount,CircleData::vCount,CircleData::idata,CircleData::indexByteCount,CircleData::iCount,device,memoryroperties);
}
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        cirForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, //绘制整正十边形
                             sqsCL->pipeline, &(sqsCL->descSet[0]), 0);
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(0, -50, 0); //沿Y 轴负方向平移50
         cirForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, //绘制正十边形的下半部分
                              sqsCL->pipeline,&(sqsCL->descSet[0]), sizeof(VkDrawIndexedIndirectCommand));
         MatrixState3D::popMatrix(); //恢复现场
