...
VkImage MyVulkanManager::depthImageMul;
VkDeviceMemory MyVulkanManager::memDepthMul;
VkImageView MyVulkanManager::depthImageViewMul;
...
VkClearValue MyVulkanManager::clear_values[3];
...
void MyVulkanManager::create_vulkan_devices() {
    ...
    VkPhysicalDeviceProperties pdp; //声明所需的结构体实例
    vk::vkGetPhysicalDeviceProperties (gpus[0],&pdp); //获取各项属性值
    LOGE("framebufferColorSampleCounts%dframebufferDepthSampleCounts%d",//打印最大多重采样数
         pdp.limits.framebufferColorSampleCounts,pdp.limits.framebufferDepthSampleCounts);
    assert(pdp.limits.framebufferColorSampleCounts>= //判断当前设备支持的最大多重采样数是否满足需要
           MULSAMPLE_COUNT&&pdp.limits.framebufferDepthSampleCounts>=MULSAMPLE_COUNT);
}
...
void MyVulkanManager::create_vulkan_DepthBuffer() {
    ...
    image_info.samples = MULSAMPLE_FACTOR; //指定多重采样数
    image_info.usage = VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT //指定图像用途
    ...
    //-------------------------------------------------
    image_info.samples = MULSAMPLE_FACTOR;//<----------------
    image_info.usage = VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
    result = vk::vkCreateImage(device, &image_info, NULL, &depthImageMul);
    assert(result == VK_SUCCESS);
    vk::vkGetImageMemoryRequirements(device, depthImageMul, &mem_reqs);
    mem_alloc.allocationSize = mem_reqs.size;
    requirements_mask = 0;
    flag = memoryTypeFromProperties(memoryroperties, mem_reqs.memoryTypeBits, requirements_mask, &mem_alloc.memoryTypeIndex);
    assert(flag);
    printf("确定内存类型成功 类型索引为%d\n", mem_alloc.memoryTypeIndex);
    result = vk::vkAllocateMemory(device, &mem_alloc, NULL, &memDepthMul);
    assert(result == VK_SUCCESS);
    result = vk::vkBindImageMemory(device, depthImageMul, memDepthMul, 0);
    assert(result == VK_SUCCESS);
    view_info.image = depthImageMul;
    result = vk::vkCreateImageView(device, &view_info, NULL, &depthImageViewMul);
    assert(result == VK_SUCCESS);
}
void MyVulkanManager::destroy_vulkan_DepthBuffer() {
    ...
    vk::vkDestroyImageView(device, depthImageViewMul, NULL);
    vk::vkDestroyImage(device, depthImageMul, NULL);
    vk::vkFreeMemory(device, memDepthMul, NULL);
...
void MyVulkanManager::create_render_pass() {//创建渲染通道的方法
    ...
    attachments[2].format = depthFormat; //设置用于多重采样的深度附件格式
    attachments[2].samples = MULSAMPLE_FACTOR; //设置多重采样数
    attachments[2].finalLayout = //用于多重采样的深度附件的最终布局
    ...
    color_reference.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;//附件布局
    VkAttachmentReference depth_reference = {}; //深度附件引用
    depth_reference.attachment = 2; //对应附件描述信息数组下标
    ...
    subpass.pResolveAttachments = &resolve_Reference; //解析附件列表
    ...
    VkSubpassDependency dependencies[2]; //子通道依赖关系结构体数组
    ...
    dependencies[1].dstSubpass = VK_SUBPASS_EXTERNAL; //依赖子通道的索引(隐含子通道)
    ...
    clear_values[1].color.float32[2] = 0.2f;
    clear_values[1].color.float32[3] = 0.2f;
...
void MyVulkanManager::create_frame_buffer() {
    ...
    attachments[2] = depthImageViewMul;
...
void MyVulkanManager::createDrawableObject() {
    objObject=LoadUtil::loadFromFile("model/SpaceStation.obj",device,memoryroperties);
}
...
void MyVulkanManager::initMatrix() {
    ...
    MatrixState3D::setCamera(0,0,0,0,0,-1,0,1,0);
...
void MyVulkanManager::drawObject() {
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        MatrixState3D::translate(0,-10,-70);
        MatrixState3D::rotate(yAngle,0,1,0);
        MatrixState3D::rotate(xAngle,1,0,0);
        MatrixState3D::scale(2.6f, 2.6f, 2.6f);
        objObject->drawSelf(cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline,
                   &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/kjz.bntex")]));
