...
void ShaderQueueSuit_Common::create_pipeline_layout(VkDevice& device)
{
    ...
    push_constant_ranges[0].stageFlags = VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT;
    push_constant_ranges[0].offset = 0;
    push_constant_ranges[0].size = sizeof(float)*32;
    push_constant_ranges[1].stageFlags = VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT;
    push_constant_ranges[1].offset =sizeof(float)*32;
    push_constant_ranges[1].size = sizeof(float)*4;
...
void ShaderQueueSuit_Common::create_shader(VkDevice& device)//创建着色器的方法
{
    std::string vertStr= FileUtil::loadAssetStr("shader/commonTexLight.vert");
    std::string tcsStr= FileUtil::loadAssetStr("shader/commonTexLight.tcs"); //加载细分控制着色器脚本
    std::string tesStr= FileUtil::loadAssetStr("shader/commonTexLight.tes"); //加载细分求值着色器脚本
    ...
    retVal = GLSLtoSPV( //将细分控制着色器脚本编译为SPV
            VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT,tcsStr.c_str(), tcs_spv);
    assert(retVal); //检查编译是否成功
    LOGE("曲面细分控制着色器脚本编译SPV 成功！");
    moduleCreateInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
    ...
    result = vk::vkCreateShaderModule(device, //创建细分控制着色器模块
    ...
    retVal = GLSLtoSPV( //将细分求值着色器脚本编译为SPV
            VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT, tesStr.c_str(), tes_spv);
    assert(retVal);
    ...
    result = vk::vkCreateShaderModule(device, //创建细分求值着色器模块
                                      &moduleCreateInfo, NULL, &shaderStages[2].module);
    assert(result == VK_SUCCESS); //检查创建是否成功
...
void ShaderQueueSuit_Common::destroy_shader(VkDevice& device)
{
    ...
    vk::vkDestroyShaderModule(device, shaderStages[2].module, NULL);
    vk::vkDestroyShaderModule(device, shaderStages[3].module, NULL);
}
...
void ShaderQueueSuit_Common::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
    ...
    VkPipelineInputAssemblyStateCreateInfo ia;//构建管线图元组装阶段信息结构体
    ia.sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;
    ia.pNext = NULL;
    ia.flags = 0;
    ia.primitiveRestartEnable = VK_FALSE;
    ia.topology = VK_PRIMITIVE_TOPOLOGY_PATCH_LIST; //图元拓扑模式
    ...
    pipelineInfo.pTessellationState = &ts; //管线的细分状态信息
    pipelineInfo.pMultisampleState = &ms;
    pipelineInfo.pDynamicState = &dynamicState;
    pipelineInfo.pViewportState = &vp;
    pipelineInfo.pDepthStencilState = &ds;
    pipelineInfo.pStages = shaderStages;
    pipelineInfo.stageCount = 4; //管线的着色阶段数量
