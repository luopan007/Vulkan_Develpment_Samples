...
ShaderQueueSuit_CommonEdge* MyVulkanManager::sqsCE;
...
void MyVulkanManager::drawObject() {
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(15, 0, -25.0f); //执行平移变换
        chObject->drawSelf(cmdBuffer, sqsCE->pipelineLayout, //绘制茶壶1(描边体)
                           sqsCE->pipeline, &(sqsCE->descSet[0]));
        chObject->drawSelf(cmdBuffer,sqsCL->pipelineLayout, //绘制茶壶1(原物体)
                           sqsCL->pipeline,&(sqsCL->descSet[0]));
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(15, 0, 5.0f); //执行平移变换
        chObject->drawSelf(cmdBuffer, sqsCE->pipelineLayout, sqsCE->pipeline, &(sqsCE->descSet[0]));//绘制茶壶2(描边体)
        chObject->drawSelf(cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline,&(sqsCL->descSet[0])); //绘制茶壶2(原物体)
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(-15, 0, 8.0f);
        qtObject->drawSelf(cmdBuffer, sqsCE->pipelineLayout, sqsCE->pipeline, &(sqsCE->descSet[0]));
        qtObject->drawSelf(cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline,&(sqsCL->descSet[0]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(-15.0f, 3.0f, -2.0f);
        qtObject->drawSelf(cmdBuffer, sqsCE->pipelineLayout, sqsCE->pipeline, &(sqsCE->descSet[0]));
        qtObject->drawSelf(cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline,&(sqsCL->descSet[0]));
        MatrixState3D::popMatrix();
        MatrixState3D::popMatrix(); //恢复现场
...
void MyVulkanManager::initPipeline()
{
    ...
    sqsCE = new ShaderQueueSuit_CommonEdge(&device, renderPass, memoryroperties);
}
