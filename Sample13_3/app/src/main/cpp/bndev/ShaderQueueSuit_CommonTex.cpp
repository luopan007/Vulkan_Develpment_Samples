...
void ShaderQueueSuit_CommonTex::init_descriptor_set(VkDevice& device)
{
    ...
    type_count[0].descriptorCount = TextureManager::texNamesForSkyBox.size();
    type_count[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[1].descriptorCount = TextureManager::texNamesForSkyBox.size();


    VkDescriptorPoolCreateInfo descriptor_pool = {};
    descriptor_pool.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    descriptor_pool.pNext = NULL;
    descriptor_pool.maxSets = TextureManager::texNamesForSkyBox.size();
    ...
    alloc_info[0].descriptorSetCount = TextureManager::texNamesForSkyBox.size();
    alloc_info[0].pSetLayouts = layouts.data();


    descSet.resize(TextureManager::texNamesForSkyBox.size());
