#include "ShaderQueueSuit_CommonEdge.h"
#include <assert.h>
...
void ShaderQueueSuit_CommonEdge::create_uniform_buffer(VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
    bufferByteCount=sizeof(float)*1;
...
void ShaderQueueSuit_CommonEdge::destroy_uniform_buffer(VkDevice& device)
{
    vk::vkDestroyBuffer(device, uniformBuf, NULL);
    vk::vkFreeMemory(device, memUniformBuf, NULL);
}
void ShaderQueueSuit_CommonEdge::create_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonEdge::destroy_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonEdge::init_descriptor_set(VkDevice& device)
{
...
void ShaderQueueSuit_CommonEdge::create_shader(VkDevice& device)
{
    std::string vertStr= FileUtil::loadAssetStr("shader/commonEdge.vert");
    std::string fragStr= FileUtil::loadAssetStr("shader/commonEdge.frag");
...
void ShaderQueueSuit_CommonEdge::destroy_shader(VkDevice& device)
{
    vk::vkDestroyShaderModule(device, shaderStages[0].module, NULL);
    vk::vkDestroyShaderModule(device, shaderStages[1].module, NULL);
}
void ShaderQueueSuit_CommonEdge::initVertexAttributeInfo()
{
...
void ShaderQueueSuit_CommonEdge::create_pipe_line(VkDevice& device,VkRenderPass& renderPass) {
    ...
    rs.sType = VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_STATE_CREATE_INFO;//指定结构体类型
    rs.pNext = NULL; //自定义数据指针
    rs.flags = 0; //供将来使用的标志
    rs.polygonMode = VK_POLYGON_MODE_FILL; //绘制方式为填充
    rs.cullMode = VK_CULL_MODE_BACK_BIT; //剪裁方式为丢弃背面三角形方式
    rs.frontFace = VK_FRONT_FACE_CLOCKWISE; //卷绕方向为顺时针
    ...
    ds.pNext = NULL; //自定义数据指针
    ds.flags = 0; //供将来使用的标志
    ds.depthTestEnable = VK_TRUE; //打开深度检测
    ds.depthWriteEnable = VK_TRUE; //打开深度值写入
...
void ShaderQueueSuit_CommonEdge::destroy_pipe_line(VkDevice& device)
{
    vk::vkDestroyPipeline(device, pipeline, NULL);
    vk::vkDestroyPipelineCache(device, pipelineCache, NULL);
}
ShaderQueueSuit_CommonEdge::ShaderQueueSuit_CommonEdge(VkDevice* deviceIn,VkRenderPass& renderPass,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ShaderQueueSuit_CommonEdge::~ShaderQueueSuit_CommonEdge(){
