...
ObjObject*  MyVulkanManager::houseObjForDraw;
ObjObject*  MyVulkanManager::qiaoObjForDraw;
ObjObject*  MyVulkanManager::tongObjForDraw;
ObjObject*  MyVulkanManager::treeObjForDraw;
ObjObject*  MyVulkanManager::skyboxObjForDraw;
ObjObject*  MyVulkanManager::tableObjForDraw;
ObjObject*  MyVulkanManager::woodObjForDraw;
ObjObject*  MyVulkanManager::mushRoomObjForDraw;
ObjObject*  MyVulkanManager::fbObjForDraw;
ObjObject*  MyVulkanManager::flowerObjForDraw;
TextureRect* MyVulkanManager::trWater;
float* MyVulkanManager::mirrorMatrix;
mutex MyVulkanManager::myLock;
...
{
    houseObjForDraw = LoadUtil::loadFromFile("model/house.obj", device, memoryroperties);
    qiaoObjForDraw = LoadUtil::loadFromFile("model/qiao.obj", device, memoryroperties);
    tongObjForDraw = LoadUtil::loadFromFile("model/tong.obj", device, memoryroperties);
    treeObjForDraw = LoadUtil::loadFromFile("model/tree.obj", device, memoryroperties);
    skyboxObjForDraw = LoadUtil::loadFromFile("model/skybox.obj", device, memoryroperties);
    tableObjForDraw = LoadUtil::loadFromFile("model/table.obj", device, memoryroperties);
    woodObjForDraw = LoadUtil::loadFromFile("model/woodpile.obj", device, memoryroperties);
    mushRoomObjForDraw = LoadUtil::loadFromFile("model/mushroom.obj", device, memoryroperties);
    fbObjForDraw = LoadUtil::loadFromFile("model/fb.obj", device, memoryroperties);
    flowerObjForDraw = LoadUtil::loadFromFile("model/flower.obj", device, memoryroperties);
    VertData::genVertData();
    trWater = new TextureRect(VertData::vdata, VertData::dataByteCount, VertData::vCount, VertData::indexData, VertData::indexDataByteCount, VertData::indexCount, device, memoryroperties);
}
void MyVulkanManager::destroyDrawableObject()
{
    delete houseObjForDraw;
    delete qiaoObjForDraw;
    delete tongObjForDraw;
    delete treeObjForDraw;
    delete skyboxObjForDraw;
    delete tableObjForDraw;
    delete woodObjForDraw;
    delete mushRoomObjForDraw;
    delete flowerObjForDraw;
    delete fbObjForDraw;
    delete trWater;
}
...
void MyVulkanManager::initMatrix()
{
    ...
    MatrixState3D::setProjectFrustum(-ratio*0.5f, ratio*0.5f, -1*0.5f, 1*0.5f, 1.0f, 500);
    LightManager::setLightPosition(10, 80, 50);
...
{
    for (int i = 0; i<TextureManager::texNamesForObj.size(); i++)
    {
        sqsCL->writes[0].dstSet = sqsCL->descSet[i];
        sqsCL->writes[1].dstSet = sqsCL->descSet[i];
        sqsCL->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForObj[i]]);
...
void MyVulkanManager::flushUniformBufferForToScreen()
{
    ...
    memcpy(pData, vertexUniformData, sqsCL->bufferByteCount);
    vk::vkUnmapMemory(device, sqsCL->memUniformBuf);
    result = vk::vkMapMemory(device, sqsM->memUniformBuf, 0, sqsM->bufferByteCount, 0, (void **)&pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsM->bufferByteCount);
    vk::vkUnmapMemory(device, sqsM->memUniformBuf);
    float fragUniformData[16] =
            {
                    mirrorMatrix[0],  mirrorMatrix[1],  mirrorMatrix[2],  mirrorMatrix[3],                             
                    mirrorMatrix[4],  mirrorMatrix[5],  mirrorMatrix[6],  mirrorMatrix[7],
                    mirrorMatrix[8],  mirrorMatrix[9],  mirrorMatrix[10], mirrorMatrix[11],
                    mirrorMatrix[12], mirrorMatrix[13], mirrorMatrix[14], mirrorMatrix[15]
            };
    result = vk::vkMapMemory(device, sqsM->memUniformBufFrag, 0, sqsM->bufferByteCountFrag, 0, (void **)&pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, fragUniformData, sqsM->bufferByteCountFrag);
    vk::vkUnmapMemory(device, sqsM->memUniformBufFrag);
}
void MyVulkanManager::flushTexToDesSetForToScreen()
{
    for (int i = 0; i < TextureManager::texNamesForWater.size() / 2; i++)
    {
        sqsM->writes[0].dstSet = sqsM->descSet[i];
        sqsM->writes[1].dstSet = sqsM->descSet[i];
        sqsM->writes[2].dstSet = sqsM->descSet[i];
        sqsM->writes[2].pImageInfo = &colorImageInfo;
        sqsM->writes[3].dstSet = sqsM->descSet[i];
        sqsM->writes[3].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForWater[i * 2 + 0]]);
        sqsM->writes[4].dstSet = sqsM->descSet[i];
        sqsM->writes[4].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForWater[i * 2 + 1]]);
        vk::vkUpdateDescriptorSets(device, 5, sqsM->writes, 0, NULL);
    }
}
void MyVulkanManager::drawSceneToTex()
{
    CameraUtil::calMirrorCamera();
    CameraUtil::flushCameraToMatrix();
    mirrorMatrix = MatrixState3D::getViewProjMatrix();
...
void MyVulkanManager::drawSceneToScreen()
{
    ...
    MatrixState3D::translate(0, 0, 22);
    trWater->drawSelf(cmdBuffer, sqsM->pipelineLayout, sqsM->pipeline,&(sqsM->descSet[0]));
...
void MyVulkanManager::updateWater()
{
    while(loopDrawFlag)
    {
        trWater->calVerticesNormalAndTangent();
        trWater->mytime+=0.02f;
        myLock.lock();
        trWater->copyData();
        myLock.unlock();
    }
...
{
    FPSUtil::init();
    thread t1(&MyVulkanManager::updateWater);
    t1.detach();
...
void MyVulkanManager::init_texture()
{
    ...
    colorImageInfo.sampler = TextureManager::samplerList[1];
}
...
void MyVulkanManager::drawThings()
{
    MatrixState3D::pushMatrix();
    skyboxObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                               &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/sky.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(0, 0, -66.5f);
    houseObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                              &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/house.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-25, 5, -15);
    qiaoObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                             &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/qiao.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(15, -0.5f, 16);
    tongObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                             &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/stuff02.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(40, 0, -15);
    treeObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                             &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/tree.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-25, -0.5f, 5);
    tableObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                              &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/tree.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(23, -0.5f, 10);
    woodObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                             &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/stuff01.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(46, 0, -43);
    mushRoomObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                                 &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/vegetation01.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-50, 0, -50);
    fbObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                           &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/stuff01.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(30, 0, -43);
    flowerObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                               &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/vegetation01.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(35, 0, -43);
    flowerObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                               &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/vegetation01.bntex")]));
    MatrixState3D::popMatrix();
}