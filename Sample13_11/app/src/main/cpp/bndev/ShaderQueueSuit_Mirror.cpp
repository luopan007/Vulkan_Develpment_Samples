...
void ShaderQueueSuit_Mirror::create_uniform_buffer_vert(VkDevice& device, VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
void ShaderQueueSuit_Mirror::destroy_uniform_buffer_vert(VkDevice& device)
{
    vk::vkDestroyBuffer(device, uniformBuf, NULL);
    vk::vkFreeMemory(device, memUniformBuf, NULL);
}
/*创建片元着色器一致变量数据缓冲*/
void ShaderQueueSuit_Mirror::create_uniform_buffer_frag(VkDevice& device, VkPhysicalDeviceMemoryProperties& memoryroperties)
{
    bufferByteCountFrag = sizeof(float) * 16;
    VkBufferCreateInfo buf_info = {};
    buf_info.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
    buf_info.pNext = NULL;
    buf_info.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
    buf_info.size = bufferByteCountFrag;
    buf_info.queueFamilyIndexCount = 0;
    buf_info.pQueueFamilyIndices = NULL;
    buf_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    buf_info.flags = 0;
    VkResult result = vk::vkCreateBuffer(device, &buf_info, NULL, &uniformBufFrag);
    assert(result == VK_SUCCESS);
    VkMemoryRequirements mem_reqs;
    vk::vkGetBufferMemoryRequirements(device, uniformBufFrag, &mem_reqs);
    VkMemoryAllocateInfo alloc_info = {};
    alloc_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    alloc_info.pNext = NULL;
    alloc_info.memoryTypeIndex = 0;
    ...
    result = vk::vkAllocateMemory(device, &alloc_info, NULL, &memUniformBufFrag);
    assert(result == VK_SUCCESS);
    result = vk::vkBindBufferMemory(device, uniformBufFrag, memUniformBufFrag, 0);
    assert(result == VK_SUCCESS);
    uniformBufferInfoFrag.buffer = uniformBufFrag;
    uniformBufferInfoFrag.offset = 0;
    uniformBufferInfoFrag.range = bufferByteCountFrag;
}
void ShaderQueueSuit_Mirror::destroy_uniform_buffer_frag(VkDevice& device)
{
    vk::vkDestroyBuffer(device, uniformBufFrag, NULL);
    vk::vkFreeMemory(device, memUniformBufFrag, NULL);
}
void ShaderQueueSuit_Mirror::create_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_Mirror::destroy_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_Mirror::init_descriptor_set(VkDevice& device)
{
    VkDescriptorPoolSize type_count[5];
    type_count[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    type_count[0].descriptorCount = TextureManager::texNamesForWater.size() / 2;
    type_count[1].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    type_count[1].descriptorCount = TextureManager::texNamesForWater.size() / 2;
    type_count[2].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[2].descriptorCount = TextureManager::texNamesForWater.size() / 2;
    type_count[3].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[3].descriptorCount = TextureManager::texNamesForWater.size() / 2;
    type_count[4].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[4].descriptorCount = TextureManager::texNamesForWater.size() / 2;
    VkDescriptorPoolCreateInfo descriptor_pool = {};
    descriptor_pool.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    descriptor_pool.pNext = NULL;
    descriptor_pool.maxSets = TextureManager::texNamesForWater.size() / 2;
    ...
    alloc_info[0].descriptorSetCount = TextureManager::texNamesForWater.size() / 2;
    ...
    writes[1].pBufferInfo = &uniformBufferInfoFrag;
...
void ShaderQueueSuit_Mirror::create_shader(VkDevice& device)
{
    std::string vertStr= FileUtil::loadAssetStr("shader/mirrorTex.vert");
    std::string fragStr= FileUtil::loadAssetStr("shader/mirrorTex.frag");
...
void ShaderQueueSuit_Mirror::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
...
void ShaderQueueSuit_Mirror::destroy_pipe_line(VkDevice& device)
{
    vk::vkDestroyPipeline(device, pipeline, NULL);
    vk::vkDestroyPipelineCache(device, pipelineCache, NULL);
}
ShaderQueueSuit_Mirror::ShaderQueueSuit_Mirror(VkDevice* deviceIn,VkRenderPass& renderPass,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
    this->devicePointer=deviceIn;
    create_uniform_buffer_vert(*devicePointer, memoryroperties);
    create_uniform_buffer_frag(*devicePointer, memoryroperties);
...
ShaderQueueSuit_Mirror::~ShaderQueueSuit_Mirror()
{
    destroy_pipe_line(*devicePointer);
    destroy_shader(*devicePointer);
    destroy_pipeline_layout(*devicePointer);
    destroy_uniform_buffer_vert(*devicePointer);
    destroy_uniform_buffer_frag(*devicePointer);
}