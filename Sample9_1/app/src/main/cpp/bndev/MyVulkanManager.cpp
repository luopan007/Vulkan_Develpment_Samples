...
TexDrawableObject* MyVulkanManager::trees[9];
float MyVulkanManager::cx=0;
float MyVulkanManager::cz=50;
float MyVulkanManager::direction=0;
float MyVulkanManager::Offset=50;
float MyVulkanManager::DEGREE_SPAN=(float)(3.0/180.0f*3.14);
...
void MyVulkanManager::createDrawableObject() { //创建绘制用物体对象的方法
    RectData::getVertexData(); //生成纹理矩形的顶点数据(底部沙漠)
    texRect = new TexDrawableObject(RectData::vData, RectData::dataByteCount,
                                    RectData::vCount, device, memoryroperties, 0, 0); //创建绘制底部沙漠用的纹理矩形
    float position[9][3] = { //储存植物组中各植物位置坐标的数组
            0, 0, 0, 22, 0, 0, 16, 0, 16, //前3 棵植物的位置坐标
            0, 0, -22, -16, 0, 16, -22, 0, 0, //中间3 棵植物的位置坐标
            -16,0,-16, 0, 0, 22, 16, 0, -16}; //后3 棵植物的位置坐标
    TreeData::getVertexData(); //生成纹理矩形顶点数据(树)
    for(int i = 0; i < 9; i ++) { //遍历植物组
        trees[i]=new TexDrawableObject(TreeData::vData,TreeData::dataByteCount,TreeData::vCount,
                                       device,memoryroperties,position[i][0],position[i][2]);} //创建不同位置坐标的植物
    TreeControl::sortForTree(); //对植物组中各个植物进行排序
}
...
{
    MatrixState3D::setCamera(cx,10,cz,0,0,0,0,1,0);
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag) {
        ...
        texRect->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                         &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(
                                 "texture/desert.bntex")]));
        MatrixState3D::popMatrix();
        for (int i = 0; i < 9; i++) {
            MatrixState3D::pushMatrix();
            trees[i]->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline,
                                  &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/tree.bntex")]));
            MatrixState3D::popMatrix(); }
