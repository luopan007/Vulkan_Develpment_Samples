...
#include <math.h>
TexDrawableObject::TexDrawableObject(float* vdataIn,int dataByteCount,int vCountIn,VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties,float xIn,float zIn)
{
...
void TexDrawableObject::calculateBillboardDirection(){ //根据摄像机的位置计算纹理矩形的朝向
    float xspan=x-MyVulkanManager::cx; //计算从植物位置到摄像机位置向量的x 分量
    float zspan=z-MyVulkanManager::cz; //计算从植物位置到摄像机位置向量的z 分量
