...
{
    LYobjDraw1=LoadUtil::loadFromFile("model/head.obj",device, memoryroperties);
}
...
{
    uFatFactor=uFatFactor+operatorT*span;
    if(uFatFactor>0.04f)
    {
        uFatFactor=0.04f;
        operatorT=-operatorT;
    } else if(uFatFactor<0)
    {
        uFatFactor=0;
        operatorT=-operatorT;
    }
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        MatrixState3D::scale(80,80,80);
        LYobjDraw1->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/head.bntex")]));
