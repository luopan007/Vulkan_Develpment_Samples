...
VkQueryPool MyVulkanManager::queryPool;//查询池对象
int* MyVulkanManager::queryResultData;//存放查询结果数据的数组
int MyVulkanManager::queryCount;//查询数量
...
ShaderQueueSuit_ForQuery* MyVulkanManager::sqsFQ;
ObjObject*  MyVulkanManager::objObjectZDT;
ObjObject*  MyVulkanManager::objObjectQT;
int CR =5;//方阵阶数
float span = 3.0f;//每格子的宽度
float xStart = -(CR - 1)*span / 2;
float yStart = -(CR - 1)*span / 2;
float zStart = -(CR - 1)*span / 2;
...
void MyVulkanManager::create_vulkan_devices()
{
    ...
    if (pdf.occlusionQueryPrecise == VK_TRUE) //判断是否支持occlusionQueryPrecise 特性
    {
        LOGE("支持精确遮挡查询occlusionQueryPrecise特性！\n");
    } //打印调试信息
    else { LOGE("不支持精确遮挡查询occlusionQueryPrecise特性！\n"); }
...
void MyVulkanManager::creat_QueryPool(){ //创建查询池的方法
    queryCount = CR*CR*CR; //初始化查询总数（本案例中为球的总数）
    queryResultData = new int[queryCount](); //创建查询结果数据数组
    VkQueryPoolCreateInfo qpci; //构建查询池创建信息结构体实例
    qpci.sType = VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO; //结构体类型
    qpci.pNext = NULL; //自定义数据的指针
    qpci.flags = 0; //标志位
    qpci.queryType = VK_QUERY_TYPE_OCCLUSION; //指定查询池的查询方式
    qpci.queryCount = queryCount; //给定查询总数
    qpci.pipelineStatistics = NULL; //控制统计如何执行的附加标志(这里为空)
    VkResult result = vk::vkCreateQueryPool(device, &qpci, NULL, &queryPool);//创建查询池
    assert(result == VK_SUCCESS); //检查查询池创建是否成功
    LOGE("查询池创建成功"); //打印查询池创建成功信息
}
void MyVulkanManager::destroy_QueryPool(){ //销毁查询池
    delete[] queryResultData; //释放查询结果数据数组
    vk::vkDestroyQueryPool(device, queryPool, NULL); //销毁对应的查询池
}
...
{
    objObjectZDT = LoadUtil::loadFromFile("model/zdt.obj", device, memoryroperties);
    objObjectQT = LoadUtil::loadFromFile("model/qt100.obj", device, memoryroperties);
}
...
void MyVulkanManager::initMatrix()
{
    ...
    MatrixState3D::setProjectFrustum(-ratio,ratio,-1,1,2.0f,100);
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        vk::vkCmdResetQueryPool(cmdBuffer, queryPool, 0, queryCount);//重置查询池  查询池中的每个查询都被标记为可用或不可用。 最初，池中的所有查询处于未定义状态，并且在可以使用任何查询之前，您需要重置池
        vk::vkCmdBeginRenderPass(cmdBuffer, &rp_begin, VK_SUBPASS_CONTENTS_INLINE);
        MatrixState3D::pushMatrix();
        drawOcclusionObj(); //绘制遮挡体(木板)
        drawForQuery(); //启用遮挡查询并绘制所有被遮挡体(球)
        getQueryResult(); //获取遮挡查询的结果
        drawForPresent(); //实际绘制未被完全遮挡的球
...
void MyVulkanManager::drawOcclusionObj()
{
    //平板默认在xOz平面  每个方向绘制CR-1个
    for (int i = 0; i < CR - 1; i++)
    {
        MatrixState3D::pushMatrix();
        MatrixState3D::scale(CR, 1.0f, CR);
        MatrixState3D::translate(0, yStart + span / 2 + span*i, 0);
        objObjectZDT->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                               &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/muwen.bntex")]));
        MatrixState3D::popMatrix();
    }
    //将平板旋转到yOz平面  每个方向绘制CR-1个
    for (int i = 0; i < CR - 1; i++)
    {
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(xStart + span / 2 + span*i, 0, 0);
    ...
    //将平板旋转到xOy平面  每个方向绘制CR-1个
    for (int i = 0; i < CR - 1; i++)
    {
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(0, 0, zStart + span / 2 + span*i);
...
void MyVulkanManager::drawForQuery()
{
    int indexForQuery = 0;
    for (int x = 0; x < CR; x++){ //X 方向遍历
        for (int y = 0; y < CR; y++){ //Y 方向遍历
            for (int z = 0; z < CR; z++){ //Z 方向遍历
                vk::vkCmdBeginQuery(cmdBuffer, queryPool, indexForQuery, VK_QUERY_CONTROL_PRECISE_BIT);
                MatrixState3D::pushMatrix(); //保护现场
                MatrixState3D::translate(x*span + xStart, y*span + yStart, z*span + zStart); //平移球
                objObjectQT->drawSelf(cmdBuffer, sqsFQ->pipelineLayout, sqsFQ->pipeline, &(sqsFQ->descSet[0]));
                MatrixState3D::popMatrix(); //恢复现场
                vk::vkCmdEndQuery(cmdBuffer, queryPool, indexForQuery); //关闭遮挡查询
                indexForQuery++; //查询索引自增
            }
        }
    }
}
void MyVulkanManager::drawForPresent()
{
    int indexForQuery = 0; //索引临时变量
    for (int x = 0; x < CR; x++){ //X 方向遍历
        for (int y = 0; y < CR; y++){ //Y 方向遍历
            for (int z = 0; z < CR; z++){ //Z 方向遍历
                if (queryResultData[indexForQuery] > 0){ //如果遮挡查询结果大于0 则进行实际绘制
                    MatrixState3D::pushMatrix(); //保护现场
                    MatrixState3D::translate(x*span + xStart, y*span + yStart, z*span + zStart); //平移变换
                    objObjectQT->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, //绘制球体
                                          &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/qt.bntex")]));
                    MatrixState3D::popMatrix(); //恢复现场
                }
                indexForQuery++; //绘制物体索引自增
            }
        }
    }
}
void MyVulkanManager::getQueryResult()
{
    //将查询结果写入数组版本
    VkResult result = vk::vkGetQueryPoolResults(
            ...
            0, //查询起始位置
            queryCount, //查询数据条数
            sizeof(int) * queryCount, //查询数据所占总字节数
            queryResultData, //存放遮挡查询结果数据的数组首地址指针
            sizeof(int), //各条遮挡查询结果之间的字节跨度
            VK_QUERY_RESULT_PARTIAL_BIT); //查询操作特定工作标志位
    assert(result == VK_SUCCESS|VK_NOT_READY); //检查遮挡查询是否成功
...
void MyVulkanManager::initPipeline()
{
    ...
    sqsFQ = new ShaderQueueSuit_ForQuery(&device, renderPass, memoryroperties);
}
