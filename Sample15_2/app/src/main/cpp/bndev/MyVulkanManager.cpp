...
VkClearValue MyVulkanManager::clear_values_self[3];
...
VkImage MyVulkanManager::colorImage[2];
VkDeviceMemory MyVulkanManager::memColor[2];
VkImageView MyVulkanManager::colorImageView[2];
VkDescriptorImageInfo MyVulkanManager::colorImageInfo[2];
...
Dashboard2DObject* MyVulkanManager::sky;
Dashboard2DObject* MyVulkanManager::landForDraw;
ObjObject*  MyVulkanManager::tree1;
ObjObject*  MyVulkanManager::tree2;
Dashboard2DObject* MyVulkanManager::d2dA;
float MyVulkanManager::SPAN=1.0f;
float MyVulkanManager::cz=100;
float MyVulkanManager::targetz=0;
float MyVulkanManager::preCZ=cz;
float MyVulkanManager::preTargetZ=targetz;
float* MyVulkanManager::mViewProjectionInverseMatrix=new float[16];
float* MyVulkanManager::mPreviousProjectionMatrix;
...
void MyVulkanManager::create_render_pass_self()
{
    ...
    VkFormat colorFormat[2] = { VK_FORMAT_R8G8B8A8_UNORM,VK_FORMAT_R16_SFLOAT};
    ...
    for(int i=0;i<OUT_TEX_COUNT;i++)
    {
        clear_values_self[i].color.float32[0] = 0.2f;
        clear_values_self[i].color.float32[1] = 0.2f;
        clear_values_self[i].color.float32[2] = 0.2f;
        clear_values_self[i].color.float32[3] = 0.2f;
    }
    clear_values_self[2].depthStencil.depth = 1.0f;
    clear_values_self[2].depthStencil.stencil = 0;
    ...
    rp_begin_self.clearValueCount = 3;
...
{
    tree1=LoadUtil::loadFromFile("model/tree.obj",device,memoryroperties);
    tree2=LoadUtil::loadFromFile("model/tree01.obj",device,memoryroperties);
    float ratio = float(screenWidth*1.0 / screenHeight);
    float* vdataIn = new float[30]
            {
                    -ratio,1,0,0,0, ratio,-1,0,1,1, ratio,1,0,1,0,
                    -ratio,1,0,0,0, -ratio,-1,0,0,1, ratio,-1,0,1,1
            };
    d2dA = new Dashboard2DObject(vdataIn, 30 * 4, 6, device, memoryroperties);
    SkyCloudData::initData();
    sky=new Dashboard2DObject(SkyCloudData::vdata,SkyCloudData::dataByteCount,SkyCloudData::vCount,device, memoryroperties);
    LandData* ld = FileUtil::loadHdtData("texture/hd1.bnhdt");
    landForDraw=new Dashboard2DObject(ld->vData, ld->vCount*5*4, ld->vCount, device, memoryroperties);
}
...
void MyVulkanManager::initMatrixAndLight()
{
    ...
    MatrixState3D::setProjectFrustum(-ratio, ratio, -1, 1, 1.0f, 300);
    MatrixState3D::setCamera(0, 0, 100, 0, 0, 0, 0, 1, 0);
    LightManager::setLightPosition(0, 0, 50);
    LightManager::setlightAmbient(0.75f,0.75f,0.75f,1.0f);
...
void MyVulkanManager::flushUniformBufferForToTex()
{
    ...
    result = vk::vkMapMemory(device, sqsCT->memUniformBuf, 0, sqsCT->bufferByteCount, 0, (void **)&pData);
    assert(result == VK_SUCCESS);
...
void MyVulkanManager::flushTexToDesSetForToTex()
{
    ...
    for (int i = 0; i<TextureManager::texNamesForSky.size(); i++)
    {
        ...
        sqsCT->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForSky[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsCT->writes, 0, NULL);
    }
}
void MyVulkanManager::drawSceneToTex()//将场景绘制到2 幅纹理的方法
{
    rp_begin_self.framebuffer = selfTexFramebuffer;//指定当前帧缓冲
    vk::vkResetCommandBuffer(cmdBuffer, 0);//恢复命令缓冲到初始状态
    VkResult result = vk::vkBeginCommandBuffer(cmdBuffer, &cmd_buf_info);//启动命令缓冲
    float ratio = (float)screenWidth / (float)screenHeight;//计算视口宽高比
    MatrixState3D::setProjectFrustum(-ratio, ratio, -1, 1, 1.0f, 300);//设置透视投影
    MatrixState3D::setCamera(0, 0, preCZ, 0, 0, preTargetZ, 0, 1, 0);//设置摄像机前一帧的参数
    mPreviousProjectionMatrix=MatrixState3D::getViewProjMatrixPrevious();//前一帧的投影与观察矩阵
    MatrixState3D::setCamera(0, 0, cz, 0, 0, targetz, 0, 1, 0);//设置摄像机当前帧的参数
    float* mViewProjectionMatrix=MatrixState3D::getViewProjMatrixCurrent();//当前帧投影与观察组合矩阵
    Matrix::invertM(mViewProjectionInverseMatrix, 0, mViewProjectionMatrix, 0);//求当前投影与观察组合矩阵的逆矩阵
    MyVulkanManager::flushUniformBufferForToTex();//将一致变量数据送入缓冲
    ...
    MatrixState3D::translate(0,0,45);
    MatrixState3D::scale(0.1,-0.1,0.1);
    landForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                          &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForSky("texture/grass.bntex")]));
    MatrixState3D::popMatrix(); 
    MatrixState3D::pushMatrix(); 
    MatrixState3D::translate(0,-10.0f,0);
    MatrixState3D::rotate(90, 0, 1, 0);
    sky->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                         &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForSky("texture/sky_cloud.bntex")]));
    MatrixState3D::popMatrix(); 
    drawTrees(1,30,-13.0f,40);
    drawTrees(1,29,-14.0f,0);
    drawTrees(1,30,-14.0f,-10);
    drawTrees(1,28,-20.0f,-40);
    drawTrees(1,30,-19.0f,-45);
    drawTrees(1,26,-15.0f,-55);
    drawTrees(1,2,-8.0f,-55);
    drawTrees(1,-26,-12.0f,52);
    drawTrees(1,-26,-12.0f,40);
    drawTrees(1,-30,-13.0f,20);
    drawTrees(1,-28,-13.5f,5);
    drawTrees(1,-10,-13.0f,0);
    drawTrees(1,-28,-13.0f,-30);
    drawTrees(1,-30,-15.0f,-45);
    drawTrees(2,28,-20.0f,60);
    drawTrees(2,30,-20.0f,55);
    drawTrees(2,2,-25.0f,35);
    drawTrees(2,26,-12.0f,15);
    drawTrees(2,28,-11.0f,10);
    drawTrees(2,26,-20.0f,-50);
    drawTrees(2,30,-17.0f,-55);
    drawTrees(2,-30,-10.0f,60);
    drawTrees(2,-28,-8.0f,45);
    drawTrees(2,-26,-10.0f,30);
    drawTrees(2,-30,-12.0f,-20);
    drawTrees(2,-26,-11.0f,-40);
    drawTrees(2,-26,-10.0f,-60);
...
void MyVulkanManager::drawSceneToScreen()//通过2 幅纹理生成屏幕最终画面的的方法
{
    ...
    float ratio = (float)screenWidth / (float)screenHeight;
    MatrixState3D::setProjectOrtho(-ratio, ratio, -1, 1, 2, 100);
    MatrixState3D::setCamera(0,0,3,0,0,0,0,1.0f,0.0f);
    MyVulkanManager::flushUniformBufferForToScreen();//将一致变量数据送入缓冲
    MyVulkanManager::flushTexToDesSetForToScreen();//更新绘制用描述集
...
void MyVulkanManager::updatePosition() {
    while(true){
        preCZ=cz;
        preTargetZ=targetz;
        cz-=SPAN;
...
{
    thread thread1(&MyVulkanManager::updatePosition);
    thread1.detach();
    FPSUtil::init();
...
void MyVulkanManager::drawTrees(int treeIndex,float transX,float transY,float transZ)
{
    if(treeIndex==1)
    {
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(transX, transY, transZ);
        tree1->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, 
    ...
    }else
    {
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(transX, transY, transZ);
        tree2->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, 
                        &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndex("texture/tree1.bntex")]));
        MatrixState3D::popMatrix();
    }
}
...
void MyVulkanManager::initPipeline()
{
    ...
    sqsCT = new ShaderQueueSuit_CommonTex(&device, renderPassSelf, memoryroperties);
...
void MyVulkanManager::create_vulkan_SelfColorBuffer() {
    VkFormat colorFormat[2] = { VK_FORMAT_R8G8B8A8_UNORM,VK_FORMAT_R16_SFLOAT};//格式数组
    for(int i=0;i<2;i++)//遍历所有颜色附件
    {
        create_vulkan_SelfColorBufferSpec(colorFormat[i], i);//创建颜色附件对应图像
    }
}
...
{
    float fragmentUniformData[33] = {
                                      mViewProjectionInverseMatrix[0],mViewProjectionInverseMatrix[1],mViewProjectionInverseMatrix[2],mViewProjectionInverseMatrix[3],
                                      mViewProjectionInverseMatrix[4],mViewProjectionInverseMatrix[5],mViewProjectionInverseMatrix[6],mViewProjectionInverseMatrix[7],
                                      mViewProjectionInverseMatrix[8],mViewProjectionInverseMatrix[9],mViewProjectionInverseMatrix[10],mViewProjectionInverseMatrix[11],
                                      mViewProjectionInverseMatrix[12],mViewProjectionInverseMatrix[13],mViewProjectionInverseMatrix[14],mViewProjectionInverseMatrix[15],
                                      mPreviousProjectionMatrix[0],mPreviousProjectionMatrix[1],mPreviousProjectionMatrix[2],mPreviousProjectionMatrix[3],
                                      mPreviousProjectionMatrix[4],mPreviousProjectionMatrix[5],mPreviousProjectionMatrix[6],mPreviousProjectionMatrix[7],
                                      mPreviousProjectionMatrix[8],mPreviousProjectionMatrix[9],mPreviousProjectionMatrix[10],mPreviousProjectionMatrix[11],
                                      mPreviousProjectionMatrix[12],mPreviousProjectionMatrix[13],mPreviousProjectionMatrix[14],mPreviousProjectionMatrix[15],
                                      2.0f
    };
    uint8_t *pData;
...
void MyVulkanManager::flushTexToDesSetForToScreen()
{
        ...
        sqsD2D->writes[1].pImageInfo =&(colorImageInfo[0]);
        sqsD2D->writes[2].dstSet = sqsD2D->descSet[0];
        sqsD2D->writes[2].pImageInfo =&(colorImageInfo[1]);
        vk::vkUpdateDescriptorSets(device, 3, sqsD2D->writes, 0, NULL);
}