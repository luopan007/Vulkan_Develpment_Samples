...
std::vector<std::string> TextureManager::texNames={"texture/grass.bntex","texture/sky_cloud.bntex","texture/tree.bntex","texture/tree1.bntex"};
std::vector<std::string> TextureManager::texNamesForSky={"texture/sky_cloud.bntex","texture/grass.bntex"};
...
int TextureManager::getVkDescriptorSetIndexForSky(std::string texName)
{
    int result=-1;
    for(int i=0;i<texNamesForSky.size();i++)
    {
        if(texNamesForSky[i].compare(texName.c_str())==0)
        {
            result=i;
            break;
        }
