...
ShaderQueueSuit_Shadow* MyVulkanManager::sqsC;
ShaderQueueSuit_CommonTex* MyVulkanManager::sqsCT;
float* MyVulkanManager::MProjCameraMatrix;
ObjObject*  MyVulkanManager::objObjectCH1;
ObjObject*  MyVulkanManager::objObjectCH2;
ObjObject*  MyVulkanManager::objObjectPTCH;
...
void MyVulkanManager::createDrawableObject(){					
    objObjectCH1	 = LoadUtil::loadFromFileNoNormal("model/ch1.obj", device, memoryroperties);
    objObjectCH2 = LoadUtil::loadFromFileNoNormal("model/ch2.obj", device, memoryroperties);
    objObjectPTCH	 = LoadUtil::loadFromFile("model/ptch.obj", device, memoryroperties);
    objObjectPM	 = LoadUtil::loadFromFileNoNormal("model/pm.obj", device, memoryroperties);
}
void MyVulkanManager::destroyDrawableObject()
{
    delete objObjectCH1;
    delete objObjectCH2;
    delete objObjectPTCH;
    delete objObjectPM;
}
...
void MyVulkanManager::initMatrixAndLight()
{
    ...
    MatrixState3D::setProjectFrustum(-ratio,ratio,-1,1,2,1000);
    CameraUtil::calCamera(30,0);
    LightManager::setLightPosition(186,111,104);
...
{
    float vertexUniformDataShadow[32]=
            {
    ...
    uint8_t *pData;
    VkResult result = vk::vkMapMemory(device, sqsC->memUniformBuf, 0, sqsC->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, vertexUniformDataShadow, sqsC->bufferByteCount);
    vk::vkUnmapMemory(device,sqsC->memUniformBuf);
...
{
    sqsC->writes[0].dstSet = sqsC->descSet[0];
    sqsC->writes[1].dstSet = sqsC->descSet[0];
    sqsC->writes[1].pImageInfo =&(TextureManager::texImageInfoList[TextureManager::texNamesForCommonTexLight[0]]);
    vk::vkUpdateDescriptorSets(device, 2, sqsC->writes, 0, NULL);
    for(int i=0;i<TextureManager::texNamesForCommonTexLight.size();i++)
    {
        ...
        sqsCT->writes[1].pImageInfo =&(TextureManager::texImageInfoList[TextureManager::texNamesForCommonTexLight[i]]);
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        objObjectPM->drawSelfNormal(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                              &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/pm.bntex")]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        objObjectCH1->drawSelfNormal(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                                    &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/c1.bntex")]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        objObjectCH2->drawSelfNormal(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                                    &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/c2.bntex")]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::scale(1.2f, 1.2f, 1.2f);
        MatrixState3D::translate(18, 0, 20);
        objObjectPTCH->drawSelf(cmdBuffer, sqsC->pipelineLayout, sqsC->pipeline,
                               &(sqsC->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/ghxp.bntex")]),1);
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::scale(1.2f, 1.2f, 1.2f);
        MatrixState3D::translate(18, 0, 20);
        objObjectPTCH->drawSelf(cmdBuffer, sqsC->pipelineLayout, sqsC->pipeline,
                                &(sqsC->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/ghxp.bntex")]),0);
        MatrixState3D::popMatrix();
...
{
    sqsC=new ShaderQueueSuit_Shadow(&device,renderPass,memoryroperties);
