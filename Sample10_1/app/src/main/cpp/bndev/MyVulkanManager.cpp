...
ObjObject*  MyVulkanManager::lftForDraw;
ObjObject*  MyVulkanManager::pmForDraw;
int MyVulkanManager::ScissorCenterX;
int MyVulkanManager::ScissorCenterY;
...
void MyVulkanManager::createDrawableObject()
{
    ...
    lftForDraw=LoadUtil::loadFromFile("model/cft.obj",device,memoryroperties);
    pmForDraw=LoadUtil::loadFromFile("model/plane.obj",device,memoryroperties);
}
...
void MyVulkanManager::initMatrix()
{
    ...
    LightManager::setLightPosition(100,100,100);
...
{
    ScissorCenterX=screenWidth/2;
    ScissorCenterY=screenHeight/2;
    FPSUtil::init();
    while(MyVulkanManager::loopDrawFlag){
        ...
        VkRect2D scissor ; //剪裁窗口的信息（2D 矩形）
        scissor.extent.width = screenWidth / 2; //剪裁窗口的宽度
        scissor.extent.height= screenHeight / 2; //剪裁窗口的高度
        scissor.offset.x = ScissorCenterX - screenWidth / 4; //剪裁窗口的X 坐标
        scissor.offset.y = ScissorCenterY - screenHeight / 4; //剪裁窗口的Y 坐标
        vk::vkCmdSetScissor( //设置剪裁窗口
                cmdBuffer, //使用的命令缓冲
                0, //第一个剪裁窗口的索引
                1, //剪裁窗口的数量
                &scissor //剪裁窗口信息列表首地址
        ...
        pmForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                             &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/floor.bntex")]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(-10, 0, 0);
        lftForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                            &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/box.bntex")]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(10, 0, 0);
        qtForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                            &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/ball.bntex")]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(0, 0, -8);
        yhForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                            &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/yh.bntex")]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(0, 0, 8);
        chForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                            &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/ghxp.bntex")]));
        MatrixState3D::popMatrix();
...
void MyVulkanManager::destroy_Texture()
{
