...
void ShaderQueueSuit_NoDepth::create_uniform_buffer(VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
void ShaderQueueSuit_NoDepth::destroy_uniform_buffer(VkDevice& device)
{
    vk::vkDestroyBuffer(device, uniformBuf, NULL);
    vk::vkFreeMemory(device, memUniformBuf, NULL);
}
void ShaderQueueSuit_NoDepth::create_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_NoDepth::destroy_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_NoDepth::init_descriptor_set(VkDevice& device)
{
...
void ShaderQueueSuit_NoDepth::create_shader(VkDevice& device)
{
...
void ShaderQueueSuit_NoDepth::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
    ...
    ds.flags = 0; //供将来使用的标志
    ds.depthTestEnable = VK_FALSE; //关闭深度检测
    ...
    ds.stencilTestEnable = VK_TRUE; //开启模板测试
    ds.front.failOp=VK_STENCIL_OP_KEEP; //正面片元未通过模板测试时的操作
    ds.front.depthFailOp=VK_STENCIL_OP_KEEP; //正面片元未通过深度测试时的操作
    ds.front.passOp=VK_STENCIL_OP_KEEP; //正面片元模板测试、深度测试都通过时的操作
    ds.front.compareOp=VK_COMPARE_OP_EQUAL; //正面片元模板测试的比较操作
...
void ShaderQueueSuit_NoDepth::destroy_pipe_line(VkDevice& device)
{
    vk::vkDestroyPipeline(device, pipeline, NULL);
    vk::vkDestroyPipelineCache(device, pipelineCache, NULL);
}
ShaderQueueSuit_NoDepth::ShaderQueueSuit_NoDepth(VkDevice* deviceIn,VkRenderPass& renderPass,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ShaderQueueSuit_NoDepth::~ShaderQueueSuit_NoDepth()
{
