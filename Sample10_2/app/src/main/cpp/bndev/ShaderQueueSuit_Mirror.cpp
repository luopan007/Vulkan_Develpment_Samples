...
void ShaderQueueSuit_Mirror::create_uniform_buffer(VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties) {
...
void ShaderQueueSuit_Mirror::destroy_uniform_buffer(VkDevice& device) {
...
void ShaderQueueSuit_Mirror::create_pipe_line(VkDevice& device,VkRenderPass& renderPass) {
    ...
    ds.flags = 0; //供将来使用的标志
    ds.depthTestEnable = VK_FALSE; //开启深度检测
    ds.depthWriteEnable = VK_FALSE; //开启深度值写入
    ...
    ds.stencilTestEnable = VK_FALSE; //开启模板测试
    ds.front.failOp=VK_STENCIL_OP_KEEP; //正面片元未通过模板测试时的操作
    ds.front.depthFailOp=VK_STENCIL_OP_KEEP; //正面片元未通过深度测试时的操作
    ds.front.passOp=VK_STENCIL_OP_REPLACE; //正面片元模板测试、深度测试都通过时的操作
    ds.front.compareOp=VK_COMPARE_OP_ALWAYS; //正面片元模板测试的比较操作
    ds.front.reference=1; //正面片元模板测试的参考值
    ds.front.writeMask=1; //正面片元模板测试的写入掩码
    ds.front.compareMask=1; //正面片元模板测试的比较掩码
    ds.back.failOp = VK_STENCIL_OP_KEEP; //反面片元未通过模板测试时的操作
    ds.back.depthFailOp = VK_STENCIL_OP_KEEP; //反面片元未通过深度测试时的操作
    ds.back.passOp = VK_STENCIL_OP_KEEP; //反面片元模板测试、深度测试都通过时的操作
    ds.back.compareOp = VK_COMPARE_OP_NEVER; //反面片元模板测试的比较操作
    ds.back.reference = 0; //反面片元模板测试的参考值
    ds.back.writeMask = 0; //反面片元模板测试的写入掩码
    ds.back.compareMask = 0; //反面片元模板测试的比较掩码
