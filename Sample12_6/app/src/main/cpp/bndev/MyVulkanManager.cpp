...
ShaderQueueSuit_CommonTexDouble* MyVulkanManager::sqsCT;
...
void MyVulkanManager::flushTexToDesSet()
{
    for(int i=0;i<TextureManager::texNames.size()/2;i++)
    {
        ...
        sqsCT->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i*2+0]]);//加载第一个纹理
        sqsCT->writes[2].dstSet = sqsCT->descSet[i];
        sqsCT->writes[2].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i*2+1]]);//加载第二个纹理
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        texTri->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, &(sqsCT->descSet[0]));
...
{
    sqsCT=new ShaderQueueSuit_CommonTexDouble(&device,renderPass,memoryroperties);
}
