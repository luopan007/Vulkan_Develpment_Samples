...
VkImage MyVulkanManager::colorImage;
VkDeviceMemory MyVulkanManager::memColor;
VkImageView MyVulkanManager::colorImageView;
VkDescriptorImageInfo MyVulkanManager::colorImageInfo;
...
ShaderQueueSuit_CommonTexLightBlend* MyVulkanManager::sqsCTLB;
ShaderQueueSuit_TexBlur* MyVulkanManager::sqsD2D;
ShaderQueueSuit_Sky* MyVulkanManager::sqsSky;
ObjObject*  MyVulkanManager:: floor;
ObjObject*  MyVulkanManager::  roof;
ObjObject*  MyVulkanManager::  wall;
ObjObject*  MyVulkanManager::  window;
ObjObject*  MyVulkanManager::  door;
ObjObject*  MyVulkanManager::  doorframe;
ObjObject*  MyVulkanManager::  pillarY;
ObjObject*  MyVulkanManager::  pillarW;
ObjObject*  MyVulkanManager::  platform;
ObjObject*  MyVulkanManager::  balckboard;
ObjObject*  MyVulkanManager::  flag;
ObjObject*  MyVulkanManager::  chair;
ObjObject*  MyVulkanManager::  whiteboard;
ObjObject*  MyVulkanManager::  desk;
ObjObject*  MyVulkanManager::  glass;
ObjObject*  MyVulkanManager::  galsshandle;
ObjObject*  MyVulkanManager::  glasscover;
float MyVulkanManager::objScale=0.0241f;
float MyVulkanManager::objBeiZi=0.02f;
float MyVulkanManager::tx=0;
float MyVulkanManager::ty=22;
float MyVulkanManager::tz=40;
float MyVulkanManager::blurWidth=60.0f;
float MyVulkanManager::blurPosition=30.0f;
DrawableObject* MyVulkanManager::sky;
...
{
    floor=LoadUtil::loadFromFile("model/floor.obj", device, memoryroperties);
    roof=LoadUtil::loadFromFile("model/roof.obj", device, memoryroperties);
    wall=LoadUtil::loadFromFile("model/wall.obj", device, memoryroperties);
    window=LoadUtil::loadFromFile("model/window.obj", device, memoryroperties);
    door=LoadUtil::loadFromFile("model/door.obj", device, memoryroperties);
    doorframe=LoadUtil::loadFromFile("model/doorframe.obj", device, memoryroperties);
    pillarY=LoadUtil::loadFromFile("model/pillarY.obj", device, memoryroperties);
    pillarW=LoadUtil::loadFromFile("model/pillarW.obj", device, memoryroperties);
    platform=LoadUtil::loadFromFile("model/platform.obj", device, memoryroperties);
    balckboard=LoadUtil::loadFromFile("model/balckboard.obj", device, memoryroperties);
    flag=LoadUtil::loadFromFile("model/flag.obj", device, memoryroperties);
    chair=LoadUtil::loadFromFile("model/chair.obj", device, memoryroperties);
    whiteboard=LoadUtil::loadFromFile("model/whiteboard.obj", device, memoryroperties);
    desk=LoadUtil::loadFromFile("model/desk.obj", device, memoryroperties);
    glass=LoadUtil::loadFromFile("model/glass.obj", device, memoryroperties);
    galsshandle=LoadUtil::loadFromFile("model/galsshandle.obj", device, memoryroperties);
    glasscover=LoadUtil::loadFromFile("model/glasscover.obj", device, memoryroperties);
    SkyCloudData::initData();
    sky=new DrawableObject(SkyCloudData::vdata,SkyCloudData::dataByteCount,SkyCloudData::vCount,device, memoryroperties);
}
...
{
    MatrixState3D::setCamera(0,22,-90,tx,ty,tz,0,1,0);
    MatrixState3D::setInitStack();
    float ratio=(float)screenWidth/(float)screenHeight;
    MatrixState3D::setProjectFrustum(-ratio*5, ratio*5, -5, 5, 11.0f, 3000);
    LightManager::setLightPosition(0,100,-70);
    LightManager::setlightAmbient(0.5f,0.5f,0.5f,0.5f);
...
void MyVulkanManager::flushUniformBufferForToTex()
{
    ...
    result = vk::vkMapMemory(device, sqsCTLB->memUniformBuf, 0, sqsCTLB->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCTLB->bufferByteCount);
    vk::vkUnmapMemory(device,sqsCTLB->memUniformBuf);
    float fragmentUniformData[1]=
            {
                    0.9
            };
    result = vk::vkMapMemory(device, sqsSky->memUniformBuf, 0, sqsSky->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, fragmentUniformData, sqsSky->bufferByteCount);
    vk::vkUnmapMemory(device,sqsSky->memUniformBuf);
}
void MyVulkanManager::flushTexToDesSetForToTex()
{
    ...
    for(int i=0;i<TextureManager::texNamesForCommonTexLight.size();i++)
    {
        sqsCTLB->writes[0].dstSet = sqsCTLB->descSet[i];
        sqsCTLB->writes[1].dstSet = sqsCTLB->descSet[i];
        sqsCTLB->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForCommonTexLight[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsCTLB->writes, 0, NULL);
    }
    for(int i=0;i<TextureManager::texNamesForSky.size();i++)
    {
        sqsSky->writes[0].dstSet = sqsSky->descSet[i];
        sqsSky->writes[1].dstSet = sqsSky->descSet[i];
        sqsSky->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForSky[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsSky->writes, 0, NULL);
    }
}
void MyVulkanManager::drawSceneToTex()
{
    rp_begin.framebuffer = selfTexFramebuffer;
    ...
    MatrixState3D::translate(-4.0f,15,-66);
    MatrixState3D::scale(objBeiZi,objBeiZi,objBeiZi);
    glass->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                        &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/glass.bntex")]));
    galsshandle->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                      &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/roof.bntex")]));
    glasscover->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                       &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/roof.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(5.0f,15,-66);
    MatrixState3D::scale(objBeiZi,objBeiZi,objBeiZi);
    glass->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                    &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/glass.bntex")]));
    galsshandle->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
    ...
    glasscover->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                       &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/roof.bntex")]));
    MatrixState3D::popMatrix();
    sky->drawSelf(cmdBuffer,sqsSky->pipelineLayout,sqsSky->pipeline,
                  &(sqsSky->descSet[0]));
    MatrixState3D::pushMatrix();
    MatrixState3D::scale(objScale,objScale,objScale);
    floor->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                    &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/floor.bntex")]));
    roof->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                       &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/roof.bntex")]));
    window->drawSelf(cmdBuffer,sqsCTLB->pipelineLayout,sqsCTLB->pipeline,
                       &(sqsCTLB->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/window.bntex")]));
    wall->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                     &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/floor.bntex")]));
    door->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                  &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/whiteboard.bntex")]));
    doorframe->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                       &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/whiteboard.bntex")]));
    pillarY->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                       &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/floor.bntex")]));
    pillarW->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                     &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/roof.bntex")]));
    platform->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                       &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/floor.bntex")]));
    flag->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                     &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/flag.bntex")]));
    chair->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                        &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/chair.bntex")]));
    whiteboard->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                     &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/blackboard.bntex")]));
    balckboard->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                     &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/blackboard.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::scale(objScale,objScale,objScale);
        desk->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                           &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/chair.bntex")]));
    MatrixState3D::translate(2000,0,0);
        desk->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                           &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/chair.bntex")]));
    MatrixState3D::translate(2000,0,0);
        desk->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                           &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/chair.bntex")]));
    MatrixState3D::translate(0,0,-1200);
        desk->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                           &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/chair.bntex")]));
    MatrixState3D::translate(-2000,0,0);
        desk->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
...
void MyVulkanManager::drawSceneToScreen()//完成第二轮绘制的方法
{
    ...
    MatrixState3D::translate(-4.0f,15,-66);
    MatrixState3D::scale(objBeiZi,objBeiZi,objBeiZi);
    glass->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    galsshandle->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    glasscover->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(5.0f,15,-66);
    MatrixState3D::scale(objBeiZi,objBeiZi,objBeiZi);
    ...
    MatrixState3D::scale(objScale,objScale,objScale);//进行缩放变换
    floor->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));//绘制地板
    roof->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));//绘制屋顶
    window->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));//绘制窗户
    wall->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    door->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    doorframe->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    pillarY->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    pillarW->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    platform->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    flag->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    chair->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    whiteboard->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    balckboard->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    MatrixState3D::popMatrix();//恢复现场
    MatrixState3D::pushMatrix();
    MatrixState3D::scale(objScale,objScale,objScale);
    desk->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    MatrixState3D::translate(2000,0,0);
    desk->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
    MatrixState3D::translate(2000,0,0);
    desk->drawSelf(cmdBuffer,sqsD2D->pipelineLayout,sqsD2D->pipeline,&(sqsD2D->descSet[0]));
...
void MyVulkanManager::init_texture()
{
    ...
    colorImageInfo.sampler=TextureManager::samplerList[0];
}
...
void MyVulkanManager::initPipeline()
{
    ...
    sqsCTLB=new ShaderQueueSuit_CommonTexLightBlend(&device,renderPass,memoryroperties);
    sqsD2D=new ShaderQueueSuit_TexBlur(&device,renderPass,memoryroperties);
    sqsSky=new ShaderQueueSuit_Sky(&device,renderPass,memoryroperties);
}
...
void MyVulkanManager::create_vulkan_SelfColorBuffer()
{
    ...
    image_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    image_info.flags = 0;
    VkResult result = vk::vkCreateImage(device, &image_info, NULL, &colorImage);
    assert(result == VK_SUCCESS);
    VkMemoryAllocateInfo mem_alloc = {};
    mem_alloc.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    mem_alloc.pNext = NULL;
    ...
    mem_alloc.memoryTypeIndex = 0;
    VkMemoryRequirements mem_reqs;
    vk::vkGetImageMemoryRequirements(device, colorImage, &mem_reqs);
    mem_alloc.allocationSize = mem_reqs.size;
    VkFlags requirements_mask=0;
    bool flag=memoryTypeFromProperties(memoryroperties, mem_reqs.memoryTypeBits,requirements_mask,&mem_alloc.memoryTypeIndex);
    assert(flag);
    LOGE("确定内存类型成功 类型索引为%d",mem_alloc.memoryTypeIndex);
    result = vk::vkAllocateMemory(device, &mem_alloc, NULL, &memColor);
    assert(result == VK_SUCCESS);
    result = vk::vkBindImageMemory(device, colorImage, memColor, 0);
    assert(result == VK_SUCCESS);
    VkImageViewCreateInfo view_info = {};
    view_info.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
    view_info.pNext = NULL;
    ...
    result = vk::vkCreateImageView(device, &view_info, NULL, &colorImageView);
    assert(result == VK_SUCCESS);
}
void MyVulkanManager::destroy_vulkan_SelfColorBuffer()
{
    ...
    vk::vkFreeMemory(device, memColor, NULL);
}
void MyVulkanManager::flushUniformBufferForToScreen()
{
    float fragmentUniformData[8]=
            {
                    CameraUtil::camera9Para[0],CameraUtil::camera9Para[1],CameraUtil::camera9Para[2],1.0,                                   
                    blurWidth,blurPosition,(float)screenWidth,(float)screenHeight
            };
    uint8_t *pData;
...
void MyVulkanManager::flushTexToDesSetForToScreen()
{
    ...
    sqsD2D->writes[1].pImageInfo =&colorImageInfo;
