...
ObjObject*  MyVulkanManager::mountain;
...
int MyVulkanManager::pipeLineIndex=0;
...
void MyVulkanManager::createDrawableObject(){					
    mountain=LoadUtil::loadFromFile("model/mountainPart.obj",device, memoryroperties);
    SkyBallData::genVertexData();
    skyForDraw=new TexDrawableObject(SkyBallData::vdata, SkyBallData::dataByteCount, SkyBallData::vCount, device, memoryroperties);
}
...
{
    MatrixState3D::setCamera(0,-10,-85,tx,ty,tz,0,1,0);
    ...
    LightManager::setlightAmbient(0.7f,0.7f,0.7f,0.7f);
...
void MyVulkanManager::flushTexToDesSet()
{
    ...
    for (int i = 0; i<TextureManager::texNamesForCommonTexLight.size(); i++){
        ...
        sqsDTL->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForCommonTexLight[i]]); 	
        sqsDTL->writes[2].dstSet = sqsDTL->descSet[i]; 	
        sqsDTL->writes[2].pImageInfo = &(TextureManager:: texImageInfoList[TextureManager::texNamesForSnow[0]]);	
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        FPSUtil::calFPS();
        FPSUtil::before();
        MatrixState3D::setCamera(0,-10,-80,tx,ty,tz,0,1,0);
        ...
        if(pipeLineIndex==0){
            ...
            MatrixState3D::translate(2.5,0.2,0);
            mountain->drawSelf(cmdBuffer,sqsDTL->pipelineLayout,sqsDTL->pipeline,
                               &(sqsDTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/mountain.bntex")]));
            ...
            MatrixState3D::scale(10,10,10);
            MatrixState3D::translate(2.5,0.2,0);
            mountain->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                               &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/mountain.bntex")]));
            MatrixState3D::popMatrix();
        }
        MatrixState3D::pushMatrix();
        skyForDraw->drawSelf(cmdBuffer,sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForCOmmonTex("texture/sky.bntex")]));
        MatrixState3D::popMatrix();
