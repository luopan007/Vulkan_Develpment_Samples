...
void MyVulkanManager::create_vulkan_DepthStencilBuffer() { 
    ...
    view_info.format = depthStencilFormat; 
...
void MyVulkanManager::drawObject() {
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        MatrixState3D::scale(0.7,1.0,0.5); //执行缩放
        planeForDraw->drawSelf(cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline,//绘制反射面(不透明)
        &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/mdb.bntex")]),false);
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::scale(1,-1,1); //执行缩放
        MatrixState3D::translate(0,bfc->currentY+0.4,0); //执行平移
        ballForDraw->drawSelf(cmdBuffer,sqsND->pipelineLayout,sqsND->pipeline,//绘制镜像体
        &(sqsND->descSet[TextureManager::getVkDescriptorSetIndex("texture/ball.bntex")]),true);
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::scale(0.7,1.0,0.5); //执行缩放
        planeForDraw->drawSelf(cmdBuffer,sqsND->pipelineLayout,sqsND->pipeline,//绘制反射面(透明)
        &(sqsND->descSet[TextureManager::getVkDescriptorSetIndex("texture/mdbtm.bntex")]),false);
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(0,bfc->currentY+0.4,0); //执行平移
        ballForDraw->drawSelf(cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline,//绘制物体本身
