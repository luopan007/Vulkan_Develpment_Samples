...
void MyVulkanManager::flushTexToDesSet()
{
    ...
    for (int i = 0; i<TextureManager::texNamesPair.size() / 2; i++){//遍历所有地球纹理组
        ...
        sqsDTL->writes[1].pImageInfo = &(TextureManager:: 	//写入属性1对应的纹理图像信息（白天）
        texImageInfoList[TextureManager::texNamesPair[i * 2 + 0]]);
        sqsDTL->writes[2].dstSet = sqsDTL->descSet[i]; 	//更新描述集对应的写入属性2(纹理)
        sqsDTL->writes[2].pImageInfo = &(TextureManager:: 	//写入属性2对应的纹理图像信息（黑夜）
        texImageInfoList[TextureManager::texNamesPair[i * 2 + 1]]);
        vk::vkUpdateDescriptorSets(device, 3, sqsDTL->writes, 0, NULL);//更新描述集
    }
}
void MyVulkanManager::drawObject()
{
    float eAngle = 0;//执行绘制的方法
    float mAngle = 0;//地球自转角
    ...
    while(MyVulkanManager::loopDrawFlag)//每循环一次绘制一帧画面
    {
        ...
        FPSUtil::calFPS();
        FPSUtil::before();
        CameraUtil::flushCameraToMatrix();//根据摄像机参数更新摄像机矩阵
        ...
        MatrixState3D::rotate(eAngle, 0, 1, 0); //绕 y 轴旋转（地球自传）
        MatrixState3D::pushMatrix(); //保护现场
        ...
        MatrixState3D::translate(180, 0, 0); //沿 x 轴平移（地月距离）
        MatrixState3D::rotate(mAngle, 0, 1, 0); //绕 y 轴旋转（月球自转）
        ballForDraw->drawSelf(cmdBuffer, sqsSTL->pipelineLayout, sqsSTL->pipeline,//绘制月球
        &(sqsSTL->descSet[TextureManager::
        getVkDescriptorSetIndexForCommonTexLight("texture/moon.bntex" )]));
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix(); //保护现场
