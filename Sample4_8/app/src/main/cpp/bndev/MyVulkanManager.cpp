...
DrawableObjectCommonLight*  MyVulkanManager::cirForDraw;
...
void MyVulkanManager::createDrawableObject()
{
    ...
    triForDraw=new DrawableObjectCommonLight(BeltData::vdata,BeltData::dataByteCount,BeltData::vCount,device,memoryroperties);
    CircleData::genVertexData();
    cirForDraw=new DrawableObjectCommonLight(CircleData::vdata,CircleData::dataByteCount,CircleData::vCount,device,memoryroperties);
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        MatrixState3D::pushMatrix();									//保护现场
     	MatrixState3D::translate(90,0,0);								//沿X轴正方向平移90
     	triForDraw->drawSelf(cmdBuffer,sqsCL->pipelineLayout,				//绘制三角形条带
    		sqsCL->pipeline[0],&(sqsCL->descSet[0]));
     	MatrixState3D::popMatrix();									//恢复现场

        MatrixState3D::pushMatrix();									//保护现场
     	MatrixState3D::translate(-90,0,0);								//沿X轴负方向平移90
     	cirForDraw->drawSelf(cmdBuffer,sqsCL->pipelineLayout,				//绘制扇形
    		sqsCL->pipeline[1],&(sqsCL->descSet[0]));
     	MatrixState3D::popMatrix();									//恢复现场
