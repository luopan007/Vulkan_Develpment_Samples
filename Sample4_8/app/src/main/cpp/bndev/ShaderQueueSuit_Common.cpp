...
void ShaderQueueSuit_Common::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
    ...
    VkPipelineInputAssemblyStateCreateInfo ia[2];	//管线图元组装状态创建信息数组
    for (int i = 0; i < 2; ++i) {						//遍历数组中每个元素进行初始化
        ia[i].sType = 										//指定结构体类型
        ...
        switch(i){								//数组中的每个元素设置不同的图元组装方式
            case 0:
                ia[i].topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP;//三点成面，点共用
                break;
            case 1:
                ia[i].topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_FAN;//三点成面，点共用
                break;
        }}
    ...
    assert(result == VK_SUCCESS);
    for(int i=0;i<2;i++){
        pipelineInfo.pInputAssemblyState = &ia[i];
...
void ShaderQueueSuit_Common::destroy_pipe_line(VkDevice& device)
{
    for (int i = 0; i < 2; ++i)
    {
        vk::vkDestroyPipeline(device, pipeline[i], NULL);
    }
