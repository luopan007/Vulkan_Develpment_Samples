...
void SixPointedStar::genStarData(float R, float r, float z) {
    ...
    for (float angle = 0; angle < 360; angle += tempAngle) {                    //六角星的每个角计算一次
        ...
        alVertix.push_back((float) (r * UNIT_SIZE *
                                    cos(toRadians(angle + tempAngle / 2))));            //第三个点的x坐标
        alVertix.push_back(
                (float) (r * UNIT_SIZE * sin(toRadians(angle + tempAngle / 2))));//第三个点的y坐标
        alVertix.push_back(z);                                        //第三个点的z坐标
        alVertix.push_back(0);                                        //第四个点的x坐标
        alVertix.push_back(0);                                    //第四个点的y坐标
        alVertix.push_back(z);                                    //第四个点的z坐标
        alVertix.push_back((float) (r * UNIT_SIZE *
                                    cos(toRadians(angle + tempAngle / 2))));            //第五个点的x坐标
        alVertix.push_back(
                (float) (r * UNIT_SIZE * sin(toRadians(angle + tempAngle / 2))));        //第五个点的y坐标
        alVertix.push_back(z);                                        //第五个点的z坐标
        alVertix.push_back(
                (float) (R * UNIT_SIZE * cos(toRadians(angle + tempAngle))));        //第六个点的x坐标

        alVertix.push_back(
                (float) (R * UNIT_SIZE * sin(toRadians(angle + tempAngle))));    //第六个点的y坐标
    ...
    for (int i = 0; i < vCount; i++) {                                    //向顶点数据数组中填充数据
        vdata[index++] = alVertix[i * 3 + 0];                            //存入当前顶点x坐标
