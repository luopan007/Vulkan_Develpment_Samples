...
void MyVulkanManager::flushUniformBuffer() { //将当前帧的一致变量数据送入一致变量缓冲的方法
    ...
    VkResult result = vk::vkMapMemory(device, sqsDT->memUniformBuf, 0, sqsDT->bufferByteCount,
                                      0, (void **)&pData); //将指定设备内存映射为CPU 可访问
    assert(result==VK_SUCCESS); //检查映射是否成功
    memcpy(pData, fragmentUniformData, sqsDT->bufferByteCount); //将数据拷贝进设备内存
    vk::vkUnmapMemory(device,sqsDT->memUniformBuf); } //解除内存映射
void MyVulkanManager::flushTexToDesSet() { //将纹理等数据与描述集关联的方法
    for (int i = 0; i<TextureManager::texNames.size() / 2; i++) {//遍历所有纹理
        ...
        vk::vkUpdateDescriptorSets(device, 3, sqsDT->writes, 0, NULL);//更新描述集
    }}
