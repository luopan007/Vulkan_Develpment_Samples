...
void ShaderQueueSuit_CommonTex::create_pipeline_layout(VkDevice& device)
{
    ...
    assert(result == VK_SUCCESS);
    const unsigned push_constant_range_count = 2; //推送常量范围实例数量
    ...
    push_constant_ranges[1].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;//着色器阶段
    push_constant_ranges[1].offset = sizeof(float)*16; //起始偏移量
    push_constant_ranges[1].size = sizeof(float)*1; //数据字节数
