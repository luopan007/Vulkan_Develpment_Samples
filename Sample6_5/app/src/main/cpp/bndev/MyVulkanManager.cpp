...
{
    MatrixState3D::setCamera(0,0,22,0,0,0,0,1,0);
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        for(int i=0;i<3;i++){ //循环列
            for(int j=0;j<3;j++){ //循环行
                ...
                MatrixState3D::translate(startX+SPAN*j,startY-i*SPAN,0); //沿 x 轴、y 轴平移
                ...
                texRect->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, //绘制物体（正方形）
                                  &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/mipmap.bntex")]),
