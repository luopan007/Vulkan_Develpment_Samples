...
ShaderQueueSuit_Shadow* MyVulkanManager::sqsCTL;
ShaderQueueSuit_CommonTex* MyVulkanManager::sqsD2D;
...
float MyVulkanManager::lAngle=0;
...
void MyVulkanManager::drawSceneToTex()
{
    ...
    float ratio=(float)screenWidth/(float)screenHeight;
    MatrixState3D::setProjectFrustum(-ratio,ratio,-1,1,1.5f,1000);
    objObjectPM->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                          &(sqsCTL->descSet[0]));
...
void MyVulkanManager::drawSceneToScreen()
{
    ...
    vk::vkCmdBeginRenderPass(cmdBuffer, &rp_begin, VK_SUBPASS_CONTENTS_INLINE);
    float ratio=(float)screenWidth/(float)screenHeight;
    MatrixState3D::setProjectOrtho(-ratio,ratio,-1,1,1.5f,10);
    MatrixState3D::setCamera(0,0,1.5f,0,0,-1,0,1.0f,0.0f);
    MatrixState3D::pushMatrix();
...
{
    sqsCTL=new ShaderQueueSuit_Shadow(&device,renderPass,memoryroperties);
    sqsD2D=new ShaderQueueSuit_CommonTex(&device,renderPass,memoryroperties);
}
