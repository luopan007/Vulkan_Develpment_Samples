...
ObjObject* MyVulkanManager::LYobjDraw1;
int operatorT=1;
float span=0.05f;
float bfbCurr=0.0f;
...
{
    std::vector<float> tVectorA= LoadUtil::loadFromFileVertexAndTex("model/laoying01.obj"); //加载关键帧1 模型的顶点坐标和纹理坐标
    std::vector<float> tVectorB= LoadUtil::loadFromFileVertex("model/laoying02.obj");//加载关键帧2 模型的顶点坐标
    std::vector<float> tVectorC= LoadUtil::loadFromFileVertex("model/laoying03.obj"); //加载关键帧3 模型的顶点坐标
    int vCount=tVectorA.size()/5; //计算顶点数量
    int dataByteCount=vCount*11*sizeof(float); //计算顶点数据总字节数
    float* vdataIn=new float[vCount*11]; //创建存放顶点数据的数组
    int indexTemp=0; //辅助索引
    for (int i=0;i<vCount;i++){ //遍历每个顶点
        vdataIn[indexTemp++]=tVectorA[i*5+0]; //保存关键帧1 的顶点X 坐标
        vdataIn[indexTemp++]=tVectorA[i*5+1]; //保存关键帧1 的顶点Y 坐标
        vdataIn[indexTemp++]=tVectorA[i*5+2]; //保存关键帧1 的顶点Z 坐标
        vdataIn[indexTemp++]=tVectorB[i*3+0]; //保存关键帧2 的顶点X 坐标
        vdataIn[indexTemp++]=tVectorB[i*3+1]; //保存关键帧2 的顶点Y 坐标
        vdataIn[indexTemp++]=tVectorB[i*3+2]; //保存关键帧2 的顶点Z 坐标
        vdataIn[indexTemp++]=tVectorC[i*3+0]; //保存关键帧3 的顶点X 坐标
        vdataIn[indexTemp++]=tVectorC[i*3+1]; //保存关键帧3 的顶点Y 坐标
        vdataIn[indexTemp++]=tVectorC[i*3+2]; //保存关键帧3 的顶点Z 坐标
        vdataIn[indexTemp++]=tVectorA[i*5+3]; //保存关键帧1 的顶点纹理坐标S 分量
        vdataIn[indexTemp++]=tVectorA[i*5+4]; //保存关键帧1 的顶点纹理坐标T 分量
    }
    LYobjDraw1=new ObjObject(vdataIn,dataByteCount,vCount, device, memoryroperties); //创建绘制用物体
...
{
    bfbCurr=bfbCurr+operatorT*span;
    if(bfbCurr>2.0f)
    {
        bfbCurr=2.0f;
        operatorT=-operatorT;
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        MatrixState3D::scale(0.8f,0.8f,0.8f);
        LYobjDraw1->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/laoying.bntex")]));
