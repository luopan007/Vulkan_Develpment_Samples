...
std::vector<float> LoadUtil::loadFromFileVertexAndTex(const string& vname)
{
    vector<float> alv;
    vector<float> alvResult;
    vector<float> alt;
    ...
    int vCount=(int)alvResult.size()/3;
    std::vector<float> lastVector;
    for(int i=0;i<vCount;i++)
    {
        lastVector.push_back(alvResult[i*3+0]);
        lastVector.push_back(alvResult[i*3+1]);
        lastVector.push_back(alvResult[i*3+2]);
        lastVector.push_back(altResult[i*2+0]);
        lastVector.push_back(altResult[i*2+1]);
        if(i==4511){
        }
    }
    return lastVector;
}
std::vector<float> LoadUtil::loadFromFileVertex(const string& vname)
{
    vector<float> alv;
    vector<float> alvResult;
    std::string resultStr=FileUtil::loadAssetStr(vname);
