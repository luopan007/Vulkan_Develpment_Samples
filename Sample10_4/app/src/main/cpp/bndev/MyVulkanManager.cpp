...
{
    objObject=LoadUtil::loadFromFile("model/ch.obj",device,memoryroperties);
}
...
void MyVulkanManager::initMatrix()
{
    ...
    LightManager::setLightPosition(50,100,50);
...
void MyVulkanManager::flushUniformBuffer(){ //将当前帧的一致数据送入一致变量缓冲的方法
    if (countE >= 2) { //判断参考值是否大于2
        spanE = -0.01f; //将步进设置为-0.01
    } else if (countE <= 0) { //判断参考值是否小于0
        spanE = 0.01f; //将步进设置为0.01
    }
    countE = countE + spanE; //计算剪裁平面方程中的参数
    float vertexUniformData[24]={ //一致变量缓冲数据数组
                    ...
                    1, countE - 1, -countE + 1, 0//剪裁平面解析方程中的4 个参数
            };
    uint8_t *pData;
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        objObject->drawSelf(cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline,&(sqsCL->descSet[0]));
