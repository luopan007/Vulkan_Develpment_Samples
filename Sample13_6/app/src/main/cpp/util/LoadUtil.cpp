...
ObjObjectUt* LoadUtil::loadFromFileUt(const string& vname,VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties){
    ...
    int triCount = alvResult.size() / (3 * 3);
    vector<float> altnResult;
    for (int i = 0; i<triCount; i++)
    {
        double p0x = alvResult[i * 9 + 0];
        double p0y = alvResult[i * 9 + 1];
        double p0z = alvResult[i * 9 + 2];
        double p1x = alvResult[i * 9 + 3];
        double p1y = alvResult[i * 9 + 4];
        double p1z = alvResult[i * 9 + 5];
        double p2x = alvResult[i * 9 + 6];
        double p2y = alvResult[i * 9 + 7];
        double p2z = alvResult[i * 9 + 8];
        double p0s = altResult[i * 6 + 0];
        double p0t = altResult[i * 6 + 1];
        double p1s = altResult[i * 6 + 2];
        double p1t = altResult[i * 6 + 3];
        double p2s = altResult[i * 6 + 4];
        double p2t = altResult[i * 6 + 5];
        vector<double> tangent = QKJUtil::calQKJ
                (
                        p0x, p0y, p0z,
                        p1x, p1y, p1z,
                        p2x, p2y, p2z,
    ...
    for (int i = 0; i<vCount; i++) {
        ...
        vdataIn[indexTemp++] = altnResult[i * 3 + 0];
        vdataIn[indexTemp++] = altnResult[i * 3 + 1];
        vdataIn[indexTemp++] = altnResult[i * 3 + 2];
    }
