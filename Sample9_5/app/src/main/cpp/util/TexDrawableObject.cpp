...
void TexDrawableObject::drawSelf(VkCommandBuffer& cmd,VkPipelineLayout& pipelineLayout,VkPipeline& pipeline,VkDescriptorSet* desSetPointer)
{
    ...
    float* landData = new float[2]{2.0f, 300.0f};
    memcpy(pushConstantData+16, landData, sizeof(float)*2);
    vk::vkCmdPushConstants(cmd, pipelineLayout,VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(float)*18,pushConstantData);
