...
void MyVulkanManager::createDrawableObject() {
    CommonTexDataObject* landObject=FileUtil::loadCommonTexData("texture/land.bntex");
    LandData* land=new LandData(landObject->width,landObject->height);
    landForDraw=new TexDrawableObject(land->vData,land->vCount*7*4,land->vCount,device,memoryroperties);
}
...
void MyVulkanManager::flushTexToDesSet() { //将纹理等数据与描述集关联的方法
    for (int i = 0; i<1; i++) { //遍历所有纹理组（本案例中仅有一组）
        sqsDT->writes[0].dstSet = sqsDT->descSet[i];
        sqsDT->writes[1].dstSet = sqsDT->descSet[i];
        sqsDT->writes[1].pImageInfo = &(TextureManager::texImageInfoList[
                TextureManager::texNames[i * 3+ 0]]);
        sqsDT->writes[2].dstSet = sqsDT->descSet[i];
        sqsDT->writes[2].pImageInfo = &(TextureManager::texImageInfoList[
                TextureManager::texNames[i * 3 + 1]]);
        sqsDT->writes[3].dstSet = sqsDT->descSet[i]; //更新描述集对应的写入属性(灰度图纹理)
        sqsDT->writes[3].pImageInfo = &(TextureManager::texImageInfoList[
                TextureManager::texNames[i * 3 + 2]]); //写入属性对应的纹理信息(灰度图纹理)
        vk::vkUpdateDescriptorSets(device, 4, sqsDT->writes, 0, NULL); //更新描述集
    }
}
