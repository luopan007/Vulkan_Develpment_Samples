...
BNModel* MyVulkanManager::bnModelA;
BNModel* MyVulkanManager::bnModelB;
...
void MyVulkanManager::init_vulkan_instance()
{
    ...
    MyInputStream::setAAssetManager(aam);
    if (!vk::loadVulkan()){
...
{
    bnModelA = new BNModel("bnggdh/a.bnggdh","texture/a.bntex", 0.0014f, device, memoryroperties);
    bnModelB = new BNModel("bnggdh/b.bnggdh","texture/b.bntex", 0.002f, device, memoryroperties);
}
...
void MyVulkanManager::flushUniformBuffer()
{
    ...
    assert(result==VK_SUCCESS);
    memcpy(pData, fragUniformData, sqsCL->bufferByteCount);
...
void MyVulkanManager::updateBNGGDH(){ //用于更新骨骼动画的方法
    while (MyVulkanManager::loopDrawFlag){ //若循环绘制标志为true
        bnModelA->update(); //更新第一个模型的骨骼动画数据
        bnModelB->update(); //更新第二个模型的骨骼动画数据
        myLock.lock(); //获取资源锁
        bnModelA->copy(); //将第一个模型更新后的顶点数据拷贝到中间传输区
        bnModelB->copy(); //将第二个模型更新后的顶点数据拷贝到中间传输区
        myLock.unlock(); //释放资源锁
    }}
void MyVulkanManager::drawObject()
{
    thread t1(&MyVulkanManager::updateBNGGDH); //开辟一个线程，负责不断更新骨骼动画
    t1.detach(); //将线程独立出来
    FPSUtil::init();
    while (MyVulkanManager::loopDrawFlag)
    {
        ...
        FPSUtil::before();
        myLock.lock(); //获取资源锁
        bnModelA->arrange(); //将第一个模型的顶点数据从中间传输区拷贝进绘制用内存
        bnModelB->arrange(); //将第二个模型的顶点数据从中间传输区拷贝进绘制用内存
        myLock.unlock(); //释放资源锁
        ...
        MatrixState3D::translate(-60, -50, 0); //执行平移变换
        MatrixState3D::rotate(150,0,1,0); //执行旋转变换
        bnModelA->draw(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline); //绘制第一个动画模型
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(60, -50, 0);
        MatrixState3D::rotate(90, 0, 1, 0);
        bnModelB->draw(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline);
