...
VkCommandBufferBeginInfo MyVulkanManager::secondary_begin;
VkCommandBufferInheritanceInfo MyVulkanManager::cmd_buf_inheritance_info;
ObjObject*  MyVulkanManager::objObject;
ObjObject* MyVulkanManager::objObject1;
 ObjObject* MyVulkanManager::objObject2;
ObjObject* MyVulkanManager::objObject3;
Semaphore* MyVulkanManager::hSemaphore[4];

PlanePosition* MyVulkanManager::planePosition0;
PlanePosition* MyVulkanManager::planePosition1;
PlanePosition* MyVulkanManager::planePosition2;
PlanePosition* MyVulkanManager::planePosition3;
...
void MyVulkanManager::create_render_pass()
{
    ...
    cmd_buf_inheritance_info.pNext = NULL; //自定义数据指针
    ...
    secondary_begin.pNext = NULL; //自定义数据指针
    secondary_begin.flags =  VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT//描述使用标志
                             |VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT;
    secondary_begin.pInheritanceInfo = &cmd_buf_inheritance_info; //命令缓冲继承信息
...
{
    objObject=LoadUtil::loadFromFile("model/plane01.obj",device,memoryroperties);
    planePosition0=new PlanePosition();
    objObject1=LoadUtil::loadFromFile("model/plane02.obj",device,memoryroperties);
    planePosition1=new PlanePosition();
    objObject2=LoadUtil::loadFromFile("model/plane03.obj",device,memoryroperties);
    planePosition2=new PlanePosition();
    objObject3=LoadUtil::loadFromFile("model/plane04.obj",device,memoryroperties);
    planePosition3=new PlanePosition();
...
void MyVulkanManager::destroyDrawableObject()
{
    ...
    delete objObject2;
    delete objObject3;
    delete planePosition0;
    delete planePosition1;
    delete planePosition2;
    delete planePosition3;
    for(int i=0;i<4;i++){
        delete  hSemaphore[i];
    }
}

void MyVulkanManager::drawTask0(SCBArangeThreadTask* stta){
    while (MyVulkanManager::loopDrawFlag)
    {
        hSemaphore[0]->WaitForSingleObject(); //申请信号量
        stta->initBeforeDraw(); //绘制任务前准备二级命令缓冲
        planePosition0->update(); //更新第0 组飞船的位置
        for(int i=0;i<planePosition0->count;i++){ //循环绘制一组飞船
            drawSpec3D(stta, planePosition0->curPosition[i*4+0],
                       planePosition0->curPosition[i*4+1], planePosition0->curPosition[i*4+2],
                       0.2, 0.2, 0.2, objObject, "texture/plane01.bntex",stta->currMatrix, stta->MVP);
        }
        stta->closeAfterDraw(); //结束二级命令缓冲
        stta->doTaskFlag = true; //修改当前线程任务完成标志
    }
};
void MyVulkanManager::drawTask1(SCBArangeThreadTask* stta){
    while (MyVulkanManager::loopDrawFlag)
    {
        hSemaphore[1]->WaitForSingleObject();
        stta->initBeforeDraw();
        planePosition1->update();
        for(int i=0;i<planePosition1->count;i++){
            drawSpec3D(stta, planePosition1->curPosition[i*4+0], planePosition1->curPosition[i*4+1], planePosition1->curPosition[i*4+2], 0.2, 0.2, 0.2, objObject1, "texture/plane02.bntex",stta->currMatrix, stta->MVP);
        }
        stta->closeAfterDraw();
        stta->doTaskFlag = true;
    }
};
void MyVulkanManager::drawTask2(SCBArangeThreadTask* stta){
    while (MyVulkanManager::loopDrawFlag)
    {
        hSemaphore[2]->WaitForSingleObject();
        stta->initBeforeDraw();
        planePosition2->update();
        for(int i=0;i<planePosition2->count;i++){
            drawSpec3D(stta, planePosition2->curPosition[i*4+0], planePosition2->curPosition[i*4+1], planePosition2->curPosition[i*4+2], 0.2, 0.2, 0.2, objObject2, "texture/plane03.bntex",stta->currMatrix, stta->MVP);
        }
        stta->closeAfterDraw();
        stta->doTaskFlag = true;
    }
};
void MyVulkanManager::drawTask3(SCBArangeThreadTask* stta){
    while (MyVulkanManager::loopDrawFlag)
    {
        hSemaphore[3]->WaitForSingleObject();
        stta->initBeforeDraw();
        planePosition3->update();
        for(int i=0;i<planePosition3->count;i++){
            drawSpec3D(stta, planePosition3->curPosition[i*4+0], planePosition3->curPosition[i*4+1], planePosition3->curPosition[i*4+2], 0.2, 0.2, 0.2, objObject3, "texture/plane04.bntex",stta->currMatrix, stta->MVP);
        }
        stta->closeAfterDraw();
        stta->doTaskFlag = true;
    }
};
void MyVulkanManager::drawSpec3D(SCBArangeThreadTask* stta, float pyx, float pyy, float pyz,
                                        float scx, float scy, float scz, ObjObject* oo, std::string texName,float* currMatrix, float* MVP){
    MatrixState3D::setInitStack(currMatrix); //初始化基本变换矩阵
    MatrixState3D::translate(currMatrix, pyx, pyy, pyz); //平移变换
    MatrixState3D::scale(currMatrix, scx, scy, scz); //缩放变换
    MatrixState3D::getFinalMatrix(MVP, currMatrix); //得到最终变换矩阵
    oo->drawSelf(stta->secondary_cmds[0], MyVulkanManager::sqsCL->pipelineLayout,//绘制物体(飞船)
                 MyVulkanManager::sqsCL->pipeline,&(MyVulkanManager::sqsCL->descSet[TextureManager::getVkDescriptorSetIndex(texName)]), MVP, currMatrix);
...
void MyVulkanManager::initMatrix()
{
    ...
    MatrixState3D::setProjectFrustum(-ratio,ratio,-1,1,2.0f,1000);
    MatrixState3D::setCamera(0, 50, -300, 0, 0, 0, 0, 1, 0);
...
void MyVulkanManager::drawObject()
{
    ...
    const int THREAD_COUNT = 4; //并行执行绘制任务的线程数量
    for(int i=0;i<THREAD_COUNT;i++){ //创建与独立绘制线程数量相同的信号量
        hSemaphore[i]=new Semaphore(1); //创建信号量
        hSemaphore[i]->WaitForSingleObject(); //申请信号量
    }
    ...
    thread tempThread0(&MyVulkanManager::drawTask0, stta[0]);//创建独立的绘制线程0
    tempThread0.detach(); //将线程0 独立出主线程
    stta[1] = new SCBArangeThreadTask(queueGraphicsFamilyIndex,&device,&(secondary_begin));
    thread tempThread1(&MyVulkanManager::drawTask1, stta[1]);//创建独立绘制线程1
    tempThread1.detach(); //将线程1 独立出主线程

    stta[2] = new SCBArangeThreadTask(queueGraphicsFamilyIndex, &device, &(secondary_begin));
    thread tempThread2(&MyVulkanManager::drawTask2, stta[2]);
    tempThread2.detach();

    stta[3] = new SCBArangeThreadTask(queueGraphicsFamilyIndex, &device, &(secondary_begin));
    thread tempThread3(&MyVulkanManager::drawTask3, stta[3]);
    tempThread3.detach();
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        cmd_buf_inheritance_info.framebuffer =framebuffers[currentBuffer];//绑定当前的帧缓冲
        ...
        //绘制前各绘制子线程准备
        for (int i = 0; i<THREAD_COUNT; i++)//遍历四个独立绘制线程
        {
            stta[i]->doTaskFlag = false;//修改独立绘制线程任务完成标志为false
            hSemaphore[i]->ReleaseSemaphore();//释放独立绘制线程完成一帧任务时所需的信号量
        }

        //循环检测对应数量的子任务是否都已经完成
       while (true)//循环检测对应数量的独立绘制线程是否都已经完成
		{
			bool tempFlag = true;//辅助标志
			for (int i = 0; i<THREAD_COUNT; i++)//遍历各独立绘制线程
			{
				if (stta[i]->doTaskFlag == false)//判断当前考察的独立绘制线程是否已经完成任务
				{
					tempFlag = false;//若不是则更新辅助标志值
				}
			}
			if (tempFlag)//若独立绘制线程都已经完成任务则跳出检查循环
			{
				break;
			}
		}
        vk::vkCmdBeginRenderPass(cmdBuffer, &rp_begin, VK_SUBPASS_CONTENTS_INLINE);
        for (int i = 0; i<THREAD_COUNT; i++)//遍历各独立绘制线程并执行对应二级命令缓冲中的命令
        {
            vk::vkCmdExecuteCommands(cmdBuffer, 1, stta[i]->secondary_cmds);
        }
