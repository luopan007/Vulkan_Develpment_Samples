...
ShaderQueueSuit_Cubemap* MyVulkanManager::sqsCube;
ShaderQueueSuit_CommonTex* MyVulkanManager::sqsTexOnly;
ObjObject* MyVulkanManager::texForDraw;
TexDrawableObject* MyVulkanManager::guangForDraw;
Flare* MyVulkanManager::flare;
...
void MyVulkanManager::createDrawableObject()
{
    ...
    texForDraw = new ObjObject(RectData::vData, RectData::dataByteCount, RectData::vCount, device, memoryroperties);
    FlatData::genVertexData();
    guangForDraw = new TexDrawableObject(FlatData::vdata, FlatData::dataByteCount, FlatData::vCount, device, memoryroperties);
    flare = new Flare();
}
...
void MyVulkanManager::initMatrixAndLight()
{
    ...
    Constant::DIS_MAX = (int)sqrt(ratio*ratio + 1);
}
...
void MyVulkanManager::flushUniformBuffer()
{
    ...
    uint8_t *pData;
    VkResult result = vk::vkMapMemory(device, sqsCube->memUniformBuf, 0, sqsCube->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCube->bufferByteCount);
    vk::vkUnmapMemory(device,sqsCube->memUniformBuf);

    float fragmentUniformData[1]=
    {
            0.9
    };
    result = vk::vkMapMemory(device, sqsTexOnly->memUniformBuf, 0, sqsTexOnly->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, fragmentUniformData, sqsTexOnly->bufferByteCount);
    vk::vkUnmapMemory(device,sqsTexOnly->memUniformBuf);
...
void MyVulkanManager::flushTexToDesSet()
{
    ...
    {
        sqsTexOnly->writes[0].dstSet = sqsTexOnly->descSet[i];
        sqsTexOnly->writes[1].dstSet = sqsTexOnly->descSet[i];
        sqsTexOnly->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForSkyBox[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsTexOnly->writes, 0, NULL);
    }


    for(int i=0;i<TextureManager::texNamesForGuang.size();i++)
    {
        sqsCube->writes[0].dstSet = sqsCube->descSet[i];
        sqsCube->writes[1].dstSet = sqsCube->descSet[i];
        sqsCube->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForGuang[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsCube->writes, 0, NULL);
    }
}

void MyVulkanManager::drawSkyBox()
{
    const float tzz = 0.4f;
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(0,0,-Constant::UNIT_SIZE + tzz);
    MatrixState3D::rotate(180,0,1,0);
    texForDraw->drawSelf(cmdBuffer,sqsTexOnly->pipelineLayout,sqsTexOnly->pipeline,
                   &(sqsTexOnly->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox(
                           "texture/skycubemap_back.bntex")]));
    ...
    texForDraw->drawSelf(cmdBuffer,sqsTexOnly->pipelineLayout,sqsTexOnly->pipeline,
                   &(sqsTexOnly->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox(
                           "texture/skycubemap_front.bntex")]));

    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-Constant::UNIT_SIZE + tzz,0,0);
    MatrixState3D::rotate(90,0,1,0);
    texForDraw->drawSelf(cmdBuffer,sqsTexOnly->pipelineLayout,sqsTexOnly->pipeline,
                   &(sqsTexOnly->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox(
                           "texture/skycubemap_right.bntex")]));
    MatrixState3D::popMatrix();
    ...
    texForDraw->drawSelf(cmdBuffer,sqsTexOnly->pipelineLayout,sqsTexOnly->pipeline,
                   &(sqsTexOnly->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox(
                           "texture/skycubemap_left.bntex")]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix();
    MatrixState3D::translate(0,Constant::UNIT_SIZE - tzz,0);
    MatrixState3D::rotate(90,1,0,0);
    MatrixState3D::rotate(180,0,0,1);
    texForDraw->drawSelf(cmdBuffer,sqsTexOnly->pipelineLayout,sqsTexOnly->pipeline,
                   &(sqsTexOnly->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox(
                           "texture/skycubemap_up.bntex")]));
    ...
    texForDraw->drawSelf(cmdBuffer,sqsTexOnly->pipelineLayout,sqsTexOnly->pipeline,
                   &(sqsTexOnly->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox(
                           "texture/skycubemap_down.bntex")]));
    MatrixState3D::popMatrix();

}
string guangStr[] = { "texture/flare1.bntex","texture/flare2.bntex","texture/flare3.bntex" };
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        MatrixState3D::setProjectFrustum(-Constant::RATIO, Constant::RATIO, -1, 1, 2.0f, 1000);
        CameraUtil::flush3DCamera();
        MyVulkanManager::drawSkyBox();


        vector<float> ls = CameraUtil::calLightScreen(Constant::RATIO);//计算光源屏幕投影位置
        lpx = ls[0];lpy = ls[1];//将结果存储到临时变量中
        if(lpx<Constant::RATIO&&lpy<1){//若光源投影位置在屏幕可视范围内
            flare->update(lpx, lpy);//更新光晕绘制位置
            MatrixState3D::setProjectOrtho(-Constant::RATIO, Constant::RATIO, -1, 1, 2.0f, 1000);
            MatrixState3D::setCamera(0, 0, 0, 0, 0, -1, 0, 1, 0);//设置摄像机位置
            for (auto ss : flare->sFl)//遍历光晕元素列表
            {
                MatrixState3D::pushMatrix();
                MatrixState3D::translate(ss->px, ss->py, -100 + ss->distance);//平移到指定位置
                MatrixState3D::scale(ss->bSize, ss->bSize, ss->bSize);//缩放为指定尺寸
                guangForDraw->drawSelf(cmdBuffer, sqsCube->pipelineLayout, sqsCube->pipeline,//绘制光晕元素
                                       &(sqsCube->descSet[TextureManager::getVkDescriptorSetIndexForGuang(guangStr[ss->idIn])]),ss->color);
...
{
    sqsCube=new ShaderQueueSuit_Cubemap(&device,renderPass,memoryroperties);
    sqsTexOnly=new ShaderQueueSuit_CommonTex(&device,renderPass,memoryroperties);
}
