...
MS3DModel * MyVulkanManager::ms3d;
float mTime = 0;	
...
{
    unsigned char* binaryData = FileUtil::loadBinary("ms3d/ninja.ms3d");
    ms3d = new MS3DModel(device, memoryroperties, binaryData);
}
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        for(int i=-k;i<=k;i++){ //对列循环绘制忍者模型
            for(int j=-k;j<=k;j++){ //对行循环绘制忍者模型
                MatrixState3D::pushMatrix(); //保护现场
                MatrixState3D::translate(i*span, 0, j*span); //执行平移变换
                ms3d->animate(mTime, //调用执行动画的方法，以完成绘制
                              cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline);
                MatrixState3D::popMatrix(); //恢复现场
            }}
        mTime += 0.015f; //更新模型动画时间
        if(mTime > ms3d->totalTime){ //若当前播放时间大于总的动画时间
            mTime = mTime - ms3d->totalTime; //实际播放时间等于当前播放时间减去总的动画时间
        }
        MatrixState3D::popMatrix(); //恢复现场
