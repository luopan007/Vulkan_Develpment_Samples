...
float MyVulkanManager::rotation=0;//旋转角度
float ratioT=0;//旋转系数
float span=0.003;//旋转步进
float operationT=1;//辅助增加位
float max_ration=0.2;//最大范围
...
void MyVulkanManager::createDrawableObject()
{
    ...
    texTri=new TexDrawableObject(TriangleData::vdata,TriangleData::dataByteCount,TriangleData::vCount,device, memoryroperties);
}
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        //将三角形移动到屏幕中心
        float upOffset= (float) (24/2/cos(3.1415926/6));
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(0, upOffset, 0);
        MatrixState3D::rotate(rotation, 0, 1, 0);
