...
{
    pushConstantData=new float[33];
    pushConstantData_Land=new float[32];
...
void ObjObject::drawSelf(VkCommandBuffer& cmd,VkPipelineLayout& pipelineLayout,VkPipeline& pipeline,VkDescriptorSet* desSetPointer,float isShadow)
{
    ...
    memcpy(pushConstantData+32, &isShadow, sizeof(float)*1);
    vk::vkCmdPushConstants(cmd, pipelineLayout,VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(float)*33,pushConstantData);
    vk::vkCmdDraw(cmd, vCount, 1, 0, 0);
}
void ObjObject::drawLand(VkCommandBuffer& cmd,VkPipelineLayout& pipelineLayout,VkPipeline& pipeline,VkDescriptorSet* desSetPointer)
{
    vk::vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,pipeline);
    vk::vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1,desSetPointer, 0, NULL);
    const VkDeviceSize offsetsVertex[1] = {0};
    ...
    float* mvp=MatrixState3D::getFinalMatrix();
    float* mm=MatrixState3D::getMMatrix();
    memcpy(pushConstantData_Land, mvp, sizeof(float)*16);
    memcpy(pushConstantData_Land+16, mm, sizeof(float)*16);
    vk::vkCmdPushConstants(cmd, pipelineLayout,VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(float)*32,pushConstantData_Land);
    vk::vkCmdDraw(cmd, vCount, 1, 0, 0);
}