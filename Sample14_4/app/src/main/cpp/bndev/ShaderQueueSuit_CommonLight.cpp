#include "ShaderQueueSuit_CommonLight.h"
#include <assert.h>
...
void ShaderQueueSuit_CommonLight::create_uniform_buffer_vert(VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
    bufferByteCount=sizeof(float)*36;
...
void ShaderQueueSuit_CommonLight::destroy_uniform_buffer_vert(VkDevice& device)
{
    vk::vkDestroyBuffer(device, uniformBuf, NULL);
    vk::vkFreeMemory(device, memUniformBuf, NULL);
}
/*创建片元着色器一致变量数据缓冲*/
void ShaderQueueSuit_CommonLight::create_uniform_buffer_frag(VkDevice& device, VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
void ShaderQueueSuit_CommonLight::destroy_uniform_buffer_frag(VkDevice& device)
{
    vk::vkDestroyBuffer(device, uniformBufFrag, NULL);
    vk::vkFreeMemory(device, memUniformBufFrag, NULL);
}
void ShaderQueueSuit_CommonLight::create_pipeline_layout(VkDevice& device)
{
    ...
    push_constant_ranges[0].size = sizeof(float)*33;
...
void ShaderQueueSuit_CommonLight::destroy_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonLight::init_descriptor_set(VkDevice& device)
{
    ...
    writes[2].pBufferInfo = &uniformBufferInfoFrag;
    writes[2].dstArrayElement = 0;
    writes[2].dstBinding = 2;
}
void ShaderQueueSuit_CommonLight::create_shader(VkDevice& device)
{
    std::string vertStr= FileUtil::loadAssetStr("shader/commonLight.vert");
    std::string fragStr= FileUtil::loadAssetStr("shader/commonLight.frag");
...
void ShaderQueueSuit_CommonLight::destroy_shader(VkDevice& device)
{
    vk::vkDestroyShaderModule(device, shaderStages[0].module, NULL);
    vk::vkDestroyShaderModule(device, shaderStages[1].module, NULL);
}
void ShaderQueueSuit_CommonLight::initVertexAttributeInfo()
{
...
void ShaderQueueSuit_CommonLight::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
...
void ShaderQueueSuit_CommonLight::destroy_pipe_line(VkDevice& device)
{
    vk::vkDestroyPipeline(device, pipeline, NULL);
    vk::vkDestroyPipelineCache(device, pipelineCache, NULL);
}
ShaderQueueSuit_CommonLight::ShaderQueueSuit_CommonLight(VkDevice* deviceIn,VkRenderPass& renderPass,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ShaderQueueSuit_CommonLight::~ShaderQueueSuit_CommonLight()
{
