...
ShaderQueueSuit_Common* MyVulkanManager::sqsCT;
ShaderQueueSuit_CommonLight* MyVulkanManager::sqsCL;
float* MyVulkanManager::MProjCameraMatrix;
ObjObject*  MyVulkanManager::objObjectCH;
ObjObject*  MyVulkanManager::objObjectPM;
float MyVulkanManager::tx=0;
float MyVulkanManager::ty=0;
float MyVulkanManager::tz=0;
float MyVulkanManager::lightElevation=40;
int MyVulkanManager::lightAzimuth=180;
...
void MyVulkanManager::createDrawableObject(){					
    objObjectCH	 = LoadUtil::loadFromFile("model/ver.obj", device, memoryroperties);
    objObjectPM	 = LoadUtil::loadFromFile("model/pm.obj", device, memoryroperties);
}
...
void MyVulkanManager::initMatrixAndLight()
{
    ...
    MatrixState3D::setCamera(0,30,60,0,0,0,0,1,0);
    float ratio=(float)screenWidth/(float)screenHeight;
    MatrixState3D::setProjectFrustum(-ratio, ratio, -1.0f, 1.0f, 2, 1000);
    LightManager::setLightPosition(40,60,0);
...
void MyVulkanManager::flushUniformBuffer()
{
    float VertUniformData[36]=
            {
                    ...
                    MProjCameraMatrix[0],MProjCameraMatrix[1],MProjCameraMatrix[2],MProjCameraMatrix[3],
                    MProjCameraMatrix[4],MProjCameraMatrix[5],MProjCameraMatrix[6],MProjCameraMatrix[7],
                    MProjCameraMatrix[8],MProjCameraMatrix[9],MProjCameraMatrix[10],MProjCameraMatrix[11],
                    MProjCameraMatrix[12],MProjCameraMatrix[13],MProjCameraMatrix[14],MProjCameraMatrix[15],
            };
    uint8_t *pData;
    VkResult result = vk::vkMapMemory(device, sqsCT->memUniformBuf, 0, sqsCT->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, VertUniformData, sqsCT->bufferByteCount);
    vk::vkUnmapMemory(device,sqsCT->memUniformBuf);
    result = vk::vkMapMemory(device, sqsCL->memUniformBuf, 0, sqsCL->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, VertUniformData, sqsCL->bufferByteCount);
    ...
    result = vk::vkMapMemory(device, sqsCT->memUniformBufFrag, 0, sqsCT->bufferByteCountFrag, 0, (void **)&pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, fragUniformData, sqsCT->bufferByteCountFrag);
    vk::vkUnmapMemory(device, sqsCT->memUniformBufFrag);
    result = vk::vkMapMemory(device, sqsCL->memUniformBufFrag, 0, sqsCL->bufferByteCountFrag, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, fragUniformData, sqsCL->bufferByteCountFrag);
    vk::vkUnmapMemory(device,sqsCL->memUniformBufFrag);
}
void MyVulkanManager::flushTexToDesSet()
{
    sqsCT->writes[0].dstSet = sqsCT->descSet[0];
    sqsCT->writes[1].dstSet = sqsCT->descSet[0];
    sqsCT->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForCommonTexLight[0]]);
    sqsCT->writes[2].dstSet = sqsCT->descSet[0];
    vk::vkUpdateDescriptorSets(device, 3, sqsCT->writes, 0, NULL);
    sqsCL->writes[0].dstSet = sqsCL->descSet[0];
    sqsCL->writes[1].dstSet = sqsCL->descSet[0];
    sqsCL->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForCommonTexLight[0]]);
    sqsCL->writes[2].dstSet = sqsCL->descSet[0];
    vk::vkUpdateDescriptorSets(device, 3, sqsCL->writes, 0, NULL);
}
void MyVulkanManager::drawThings(float isShadow)
{
        MatrixState3D::pushMatrix();
        MatrixState3D::scale(1.5f, 1.5f, 1.5f);
        MatrixState3D::translate(0, 0, 10);
        objObjectCH->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                                &(sqsCL->descSet[0]),isShadow);
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        MatrixState3D::setProjectFrustum(-ratio, ratio, -1.0f, 1.0f, 2, 1000);
        LightManager::setLightPosition(40,60,0);
        MProjCameraMatrix = MatrixState3D::getViewProjMatrix();
        ...
        objObjectPM->drawLand(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                              &(sqsCT->descSet[0]));
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(tx, 0,tz);
...
{
    sqsCT=new ShaderQueueSuit_Common(&device,renderPass,memoryroperties);
    sqsCL=new ShaderQueueSuit_CommonLight(&device,renderPass,memoryroperties);
}
