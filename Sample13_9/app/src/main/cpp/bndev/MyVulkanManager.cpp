...
void MyVulkanManager::create_vulkan_SelfColorBuffer(){ //创建颜色缓冲的方法
    colorFormat = VK_FORMAT_R8G8B8A8_UNORM; //指定图像的格式
    VkImageCreateInfo image_info = {}; //构建图像创建信息结构体实例
    image_info.format = colorFormat; //设置图像格式
    image_info.usage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT |
                       VK_IMAGE_USAGE_SAMPLED_BIT; //设置图像用途
    ...
    view_info.format = colorFormat; //图像视图的格式
...
void MyVulkanManager::destroy_vulkan_SelfColorBuffer(){ //销毁颜色缓冲相关
    vk::vkDestroyImageView(device, colorImageView, NULL); //销毁图像视图对象
    vk::vkDestroyImage(device, colorImage, NULL); //销毁图像对象
    vk::vkFreeMemory(device, memColor, NULL); //释放图像设备内存
}
...
void MyVulkanManager::create_frame_buffer() {//创建绘制到纹理专用帧缓冲的方法
    ...
    attachments[0] = colorImageView; //指定颜色附件对应图像视图
    VkResult result = vk::vkCreateFramebuffer(device, //创建服务于绘制到纹理的帧缓冲
                                              &fb_info, NULL, &selfTexFramebuffer);
    assert(result == VK_SUCCESS); //检查帧缓冲是否创建成功
}
...
{
    objObject=LoadUtil::loadFromFile("model/ch_t.obj",device,memoryroperties);
...
void MyVulkanManager::flushTexToDesSetForToScreen(){//绘制到屏幕时将纹理等数据与描述集关联
    sqsD2D->writes[0].dstSet = sqsD2D->descSet[0]; //描述集对应的写入属性0（一致变量）
    sqsD2D->writes[1].dstSet = sqsD2D->descSet[0]; //描述集对应的写入属性1（纹理）
    sqsD2D->writes[1].pImageInfo = &colorImageInfo; //写入属性1 对应的纹理图像信息
...
void MyVulkanManager::drawSceneToTex() { //将场景绘制到纹理的方法
    ...
    objObject->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, //绘制茶壶物体
    ...
    submit_info[0].pWaitSemaphores = NULL; //等待的信号量列表
...
void MyVulkanManager::drawSceneToScreen()
{
    ...
    MatrixState2D::scale(0.7f, 0.7f, 0.7f);
...
void MyVulkanManager::init_texture(){ //初始化纹理的方法
    TextureManager::initTextures //调用方法初始化纹理
            (device,gpus[0],memoryroperties,cmdBuffer,queueGraphics);
    colorImageInfo.imageView = colorImageView; //采用的图像视图
    colorImageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL; //图像布局
    colorImageInfo.sampler = TextureManager::samplerList[0]; //采用的采样器
}
