...
void MyVulkanManager::createDrawableObject(){//创建绘制用物体的方法
    ...
    float* vdataIn=new float[vcount*3]{//顶点数据数组
            0,20,0, -10,10,0, 10,10,0, 0,0,0, -10,-10,0, 10,-10,0, 0,-20,0	//顶点的数据
    };
    texTri=new TexDrawableObject(vdataIn,vcount*3*4,vcount,device, memoryroperties); //创建绘制物体
}
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        MatrixState3D::translate(-10,0,0); //沿 X 轴平移
        MatrixState3D::rotate(yAngle,0,1,0); //绕 y 轴旋转
        MatrixState3D::rotate(zAngle,0,0,1); //绕 z 轴旋转
        texTri->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline,//绘制左侧点精灵集
                         &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/fp.bntex")]));
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(10,0,0); //沿 X 轴平移
        MatrixState3D::rotate(yAngle,0,1,0); //绕 y 轴旋转
        MatrixState3D::rotate(zAngle,0,0,1); //绕 z 轴旋转
        texTri->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, //绘制右侧点精灵集
                         &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/fp.bntex")]));
        MatrixState3D::popMatrix(); //恢复现场
