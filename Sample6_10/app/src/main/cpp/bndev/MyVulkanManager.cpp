...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        MatrixState3D::translate(-10,0,0); //沿 x 轴移动
        MatrixState3D::rotate(yAngle,0,1,0); //绕 y 轴旋转
        MatrixState3D::rotate(zAngle,0,0,1); //绕 z 轴旋转
        texTri->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, //绘制左侧纹理三角形
                         &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/vulkan.bntexa")]),0);
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(10,0,0); //沿 x 轴移动
        MatrixState3D::rotate(yAngle,0,1,0); //绕 y 轴旋转
        MatrixState3D::rotate(zAngle,0,0,1); //绕 z 轴旋转
        texTri->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, //绘制右侧纹理三角形
                         &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/vulkan.bntexa")]),1);
        MatrixState3D::popMatrix(); //恢复现场
