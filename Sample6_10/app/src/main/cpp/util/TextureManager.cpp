...
std::vector<std::string> TextureManager::texNames={"texture/vulkan.bntexa"};
...
void TextureManager::initTextures(VkDevice& device, VkPhysicalDevice& gpu, VkPhysicalDeviceMemoryProperties& memoryroperties, VkCommandBuffer& cmdBuffer, VkQueue& queueGraphics)
{
    ...
    for (int i = 0; i<texNames.size(); i++)
    {
        ...
        init_SPEC_2DArray_Textures(texNames[i],device,gpu,memoryroperties,cmdBuffer,queueGraphics, VK_FORMAT_R8G8B8A8_UNORM, ctdo);
    }
}
void TextureManager::init_SPEC_2DArray_Textures(std::string texName, VkDevice& device, VkPhysicalDevice& gpu, VkPhysicalDeviceMemoryProperties& memoryroperties, VkCommandBuffer& cmdBuffer, VkQueue& queueGraphics, VkFormat format, TexArrayDataObject* ctdo)
{
    ...
    mem_alloc.allocationSize = mem_reqs.size; LOGE("mem_reqs.size=%d\n", (int)(mem_reqs.size));
