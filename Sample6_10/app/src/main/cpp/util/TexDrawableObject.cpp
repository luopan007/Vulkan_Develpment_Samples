...
void TexDrawableObject::drawSelf(VkCommandBuffer& cmd,VkPipelineLayout& pipelineLayout,VkPipeline& pipeline,VkDescriptorSet* desSetPointer,int texArrayIndex)
{
    ...
    pushConstantData[16]=texArrayIndex; //将纹理数组索引数据送入推送常量数据
    vk::vkCmdPushConstants(cmd, pipelineLayout, //将推送常量数据送入推送常量
                           VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(float)*17,pushConstantData);
