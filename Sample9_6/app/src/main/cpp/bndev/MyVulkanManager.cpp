...
{
	LandData* ld = FileUtil::loadHdtData("hdt/default.bnhdt");
	landForDraw = new TexDrawableObject(ld->vData, ld->vCount * 8 * 4, ld->vCount, device, memoryroperties);
}
...
void MyVulkanManager::initMatrix()
{
	...
	LightManager::setLightPosition(200, 200, 0);
...
void MyVulkanManager::flushTexToDesSet()
{
	for (int i = 0; i<TextureManager::texNames.size()/6; i++)
	{
        ...
        vk::vkUpdateDescriptorSets(device, 7, sqsCT->writes, 0, NULL);
	}
}
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag){
		...
		landForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
			&(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/default_c.bntex")]));
