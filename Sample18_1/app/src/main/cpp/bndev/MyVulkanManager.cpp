...
TextureRect* MyVulkanManager::zhuomian;
TextureRect* MyVulkanManager::dangban;
ObjObject* MyVulkanManager::zhuomianObject;
ObjObject* MyVulkanManager::dangbanObject;
ObjObject* MyVulkanManager::zqObject;
vector<ObjObject*> MyVulkanManager::objList;
vector<BallForControl*>MyVulkanManager::allBall;
...
{
    zhuomian = new TextureRect(TABLE_SIZE, TABLE_SIZE);
    dangban = new TextureRect(TABLE_SIZE, BANISTERS_HEIGHT);
    zhuomianObject = new ObjObject(zhuomian->vdata, zhuomian->dataByteCount, zhuomian->vCount, device, memoryroperties);
    dangbanObject = new ObjObject(dangban->vdata, dangban->dataByteCount, dangban->vCount, device, memoryroperties);
    zqObject = LoadUtil::loadFromFile("model/qt.obj", device, memoryroperties);
    allBall.push_back(new BallForControl(zqObject, -1.0f, 5.0f, "texture/snooker0.bntex"));
    allBall.push_back(new BallForControl(zqObject, 0.0f, 1.0f, "texture/snooker1.bntex"));
    allBall.push_back(new BallForControl(zqObject, -BALL_R - 0.1f, 0.0f, "texture/snooker2.bntex"));
    allBall.push_back(new BallForControl(zqObject, BALL_R + 0.1f, 0.0f, "texture/snooker3.bntex"));
    allBall.push_back(new BallForControl(zqObject, -2.0f * BALL_R - 0.1f, -1.0f, "texture/snooker4.bntex"));
    allBall.push_back(new BallForControl(zqObject, 0.0f, -1.0f, "texture/snooker5.bntex"));
    allBall.push_back(new BallForControl(zqObject, 2.0f * BALL_R + 0.1f, -1.0f, "texture/snooker6.bntex"));
}
void MyVulkanManager::destroyDrawableObject()
{
    for (BallForControl* bfc : allBall)
    {
        delete bfc;
    }
    delete zhuomian;
    delete dangban;
    delete zhuomianObject;
    delete dangbanObject;
    delete zqObject;
}
...
void MyVulkanManager::initMatrix()
{
    ...
    MatrixState3D::setProjectFrustum(-ratio,ratio,-1,1,3.5f,100);
    CameraUtil::calCamera(0,0);
    LightManager::setLightPosition(50,50,50);
    LightManager::setlightAmbient(0.2f,0.2f,0.2f,1.0f);
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        zhuomianObject->drawSelf(cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline, //绘制桌面
                                 &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/green_table.bntex")]));
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(0, 0, -TABLE_SIZE); //执行平移变换
        MatrixState3D::rotate(90, 1, 0, 0); //执行旋转变换
        dangbanObject->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,//绘制桌面前挡板
                                &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/wall.bntex")]));
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(0, 0, TABLE_SIZE);
        MatrixState3D::rotate(90, 1, 0, 0);
        dangbanObject->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                                &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/wall.bntex")]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(-TABLE_SIZE, 0, 0);
        ...
        dangbanObject->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                                &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/wall.bntex")]));
        MatrixState3D::popMatrix();
        for (BallForControl* bfc : allBall){ //遍历所有桌球控制对象
            bfc->go(); //调用go 方法进行滚动模拟计算
        }
        for (BallForControl* bfc : allBall){ //遍历所有桌球控制对象
            bfc->drawSelf(cmdBuffer,sqsCL); //调用drawSelf 方法绘制桌球
        }
        MatrixState3D::popMatrix(); //恢复现场
