...
std::vector<std::string> TextureManager::texNames={ "texture/mipmapIsotropy.bntex","texture/mipmapAnisotropy.bntex"};
...
void TextureManager::initSampler(VkDevice& device, VkPhysicalDevice& gpu)
{
    ...
    samplerCreateInfo.maxAnisotropy = 8;//各向异性过滤最大采样数
    samplerCreateInfo.compareOp = VK_COMPARE_OP_NEVER;
    samplerCreateInfo.minLod = 0.0;
    samplerCreateInfo.maxLod = 9.0;
    samplerCreateInfo.compareEnable = VK_FALSE;
    samplerCreateInfo.borderColor = VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE;
    for (int i = 0; i<SAMPLER_COUNT; i++)//循环创建2个不同的采样器
    {
        if (i != 0)//若不是第0个采样器
        {
            samplerCreateInfo.anisotropyEnable = VK_FALSE;//禁用各向异性过滤
            samplerCreateInfo.maxAnisotropy = 1;//各向异性过滤最大采样数
        }
...
void TextureManager::init_SPEC_Textures_ForMipMap(std::string texName, VkDevice& device, VkPhysicalDevice& gpu, VkPhysicalDeviceMemoryProperties& memoryroperties, VkCommandBuffer& cmdBuffer, VkQueue& queueGraphics, VkFormat format, TexDataObject* ctdo, int levels,int samplerIndex)
{
    ...
    texImageInfo.sampler = samplerList[samplerIndex];
...
void TextureManager::initTextures(VkDevice& device, VkPhysicalDevice& gpu, VkPhysicalDeviceMemoryProperties& memoryroperties,VkCommandBuffer& cmdBuffer, VkQueue& queueGraphics){//加载所有纹理的方法
    ...
    for (int i = 0; i<texNames.size(); i++){ //遍历纹理文件名称列表
        ...
        init_SPEC_Textures_ForMipMap(texNames[i], device, gpu, memoryroperties, //加载 2D 纹理
                                     cmdBuffer, queueGraphics, VK_FORMAT_R8G8B8A8_UNORM, ctdo, levels,i);
    }}
