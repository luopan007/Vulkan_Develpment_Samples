...
void MyVulkanManager::create_vulkan_devices()
{
    ...
    assert(pdf.samplerAnisotropy == VK_TRUE);				//检查是否支持各向异性过滤
...
void MyVulkanManager::createDrawableObject()
{
    ...
    texRect=new TexDrawableObject(vdataIn,30*4,6,device, memoryroperties);
}
...
{
    MatrixState3D::setCamera(0,0,11,0,0,0,0,1,0);
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        float startY=SPAN; //y 坐标初始值
                ...
                currLodLevel++; //纹理采样细节级别加 1
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(startX + SPAN*2.4, startY - 1 * SPAN, 0); //沿 x 轴、y 轴平移
        texRect->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, //绘制物体（正方形）
                          &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/mipmapIsotropy.bntex")]), currLodLevel);//传入纹理采样细节级别
        MatrixState3D::popMatrix(); //恢复现场

        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(startX + SPAN*1.35, startY - 1*SPAN, 0); //沿 x 轴、y 轴平移
        MatrixState3D::rotate(90, 0, 1, 0); //绕 y 轴旋转
        texRect->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, //绘制物体（正方形）
                          &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/mipmapIsotropy.bntex")]), currLodLevel);//传入纹理采样细节级别
        MatrixState3D::popMatrix(); //恢复现场

        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(startX + SPAN * 0.65, startY - 1 * SPAN, 0); //沿 x 轴、y 轴平移
        MatrixState3D::rotate(90, 0, 1, 0); //绕 y 轴旋转
        texRect->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, //绘制物体（正方形）
                          &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/mipmapAnisotropy.bntex")]), currLodLevel);//传入纹理采样细节级别
        MatrixState3D::popMatrix(); //恢复现场

        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(startX + SPAN * -0.4, startY - 1 * SPAN, 0); //沿 x 轴、y 轴平移
        texRect->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, //绘制物体（正方形）
                          &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/mipmapAnisotropy.bntex")]), currLodLevel);//传入纹理采样细节级别
        MatrixState3D::popMatrix(); //恢复现场
