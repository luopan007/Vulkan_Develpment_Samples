		...
		if(AInputEvent_getSource(event)==AINPUT_SOURCE_TOUCHSCREEN)
		{
			...
			switch(id){
				...
				case AMOTION_EVENT_ACTION_MOVE: //触控点移动
					xDis=x-xPre; //触控点x 方向位移
					yDis=y-yPre; //触控点y 方向位移
					...
					if(!isClick) { //判断是否为滑动操作
						...
						MyVulkanManager::zAngle+=yDis/10; //计算绕z 轴转角
						xPre=x; //更新触控点x 坐标
						yPre=y; //更新触控点y 坐标
					}
					break;
				case AMOTION_EVENT_ACTION_UP: //触控点抬起
					if(isClick) { //判断是否为点击操作
						MyVulkanManager:: ProjectPara =
								++MyVulkanManager:: ProjectPara %2; //更新ProjectPara 的值
