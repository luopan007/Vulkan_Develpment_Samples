...
TexDrawableObject* MyVulkanManager::texBall;
...
void MyVulkanManager::createDrawableObject()
{
    ...
    BallData::genBallData(10);
    texBall=new TexDrawableObject(BallData::vdata,BallData::dataByteCount,BallData::vCount,device, memoryroperties);
}
...
{
    MatrixState3D::setCamera(0,0,70,0,0,0,0,1,0);
    MatrixState3D::setInitStack();
    float ratio=(float)screenWidth/(float)screenHeight;
    MatrixState3D::setProjectFrustum(-ratio,ratio,-1,1,4.0f,1000);
}
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        texBall->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline,
                &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/boardRed.bn3dtex")]));
        ...
        MatrixState3D::rotate(zAngle,0,0,1);
        texBall->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline,
                          &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/boardGreen.bn3dtex")]));
        MatrixState3D::popMatrix();
