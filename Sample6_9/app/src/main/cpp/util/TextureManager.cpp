...
std::vector<std::string> TextureManager::texNames={"texture/boardRed.bn3dtex","texture/boardGreen.bn3dtex"};
...
void TextureManager::initTextures(VkDevice& device, VkPhysicalDevice& gpu, VkPhysicalDeviceMemoryProperties& memoryroperties, VkCommandBuffer& cmdBuffer, VkQueue& queueGraphics)
{
    ...
    for (int i = 0; i<texNames.size(); i++)
    {
        ...
        printf("%s w %d h %d d %d \n", texNames[i].c_str(), ctdo->width, ctdo->height, ctdo->depth);
        init_SPEC_3D_Textures(texNames[i], device, gpu, memoryroperties, cmdBuffer, queueGraphics, VK_FORMAT_R8G8B8A8_UNORM, ctdo);
    }
}
