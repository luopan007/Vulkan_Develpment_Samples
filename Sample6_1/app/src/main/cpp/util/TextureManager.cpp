...
#include <assert.h>
void setImageLayout(VkCommandBuffer cmd, VkImage image,VkImageAspectFlags aspectMask,
    ...
    image_memory_barrier.pNext = NULL; //自定义数据的指针
    ...
    image_memory_barrier.subresourceRange.baseMipLevel = 0; //基础 mipmap 级别
    image_memory_barrier.subresourceRange.levelCount = 1; //mipmap 级别的数量
    ...
    if (old_image_layout == VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL) {//判断旧布局
        image_memory_barrier.srcAccessMask = //设置源访问掩码
    ...
    if (new_image_layout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL) {//判断新布局
        image_memory_barrier.srcAccessMask = //源访问掩码
    ...
    if (new_image_layout == VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL) {//判断新布局
        image_memory_barrier.dstAccessMask = //目标访问掩码
                VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
    }
    ...
    vk::vkCmdPipelineBarrier(cmd, src_stages, dest_stages, 0, 0, //放置屏障
...
void TextureManager::initSampler(VkDevice& device, VkPhysicalDevice& gpu){
    ...
    for (int i = 0; i<SAMPLER_COUNT; i++){ //循环创建指定数量的采样器
        ...
        VkResult result = vk::vkCreateSampler(device,
...
void TextureManager::init_SPEC_2D_Textures(std::string texName, VkDevice& device, VkPhysicalDevice& gpu, VkPhysicalDeviceMemoryProperties& memoryroperties, VkCommandBuffer& cmdBuffer, VkQueue& queueGraphics,VkFormat format, TexDataObject* ctdo)
{
    ...
    if (needStaging)
    {
        ...
        buf_info.flags = 0; //标志
        VkResult result = vk::vkCreateBuffer(device, &buf_info, NULL, &tempBuf); //创建缓冲
        ...
        result = vk::vkAllocateMemory(device, &alloc_info, NULL, &memTemp); //分配设备内存
        assert(result == VK_SUCCESS); //检查内存分配是否成功
        uint8_t *pData; //CPU 访问时的辅助指针
        result = vk::vkMapMemory(device, memTemp,
        ...
        result = vk::vkCreateImage(device, &image_create_info, NULL, &textureImage);//创建图像
        ...
        result = vk::vkAllocateMemory(device, &mem_alloc, NULL, &(textureMemory)); //分配设备内存
        ...
        vk::vkCreateFence(device, &fenceInfo, NULL, &copyFence); //创建栅栏
        ...
        vk::vkCmdCopyBufferToImage(cmdBuffer, tempBuf, //将缓冲中的数据拷贝到纹理图像中
        ...
        vk::vkDestroyBuffer(device, tempBuf, NULL); //销毁中转缓冲
        vk::vkFreeMemory(device, memTemp, NULL); //释放中转缓冲的设备内存
        vk::vkDestroyFence(device,copyFence,NULL); //销毁拷贝任务用栅栏
    }
    else
    {
        VkImageCreateInfo image_create_info = {}; //构建图像创建信息结构体实例
        ...
        uint8_t *pData; // CPU 访问时的辅助指针
        vk::vkMapMemory(device, textureMemory, 0,
...
void TextureManager::initTextures(VkDevice& device, //加载所有纹理的方法
    ...
    for (int i = 0; i<texNames.size(); i++){ //遍历纹理文件名称列表
        ...
        init_SPEC_2D_Textures(texNames[i], device, gpu, memoryroperties, //加载 2D 纹理
        cmdBuffer, queueGraphics, VK_FORMAT_R8G8B8A8_UNORM,ctdo);
    }}
void TextureManager::deatroyTextures(VkDevice& device){ //销毁纹理的方法
    for(int i=0;i<SAMPLER_COUNT;i++){ //遍历所有采样器
        vk::vkDestroySampler(device, samplerList[i], NULL); //销毁采样器
    }
    for(int i=0;i<texNames.size();i++){ //遍历所有纹理
        vk::vkDestroyImageView(device, viewTextureList[texNames[i]], NULL); //销毁图像视图
        vk::vkDestroyImage(device, textureImageList[texNames[i]], NULL); //销毁图像
        vk::vkFreeMemory(device, textureMemoryList[texNames[i]], NULL); //释放设备内存
    }}
