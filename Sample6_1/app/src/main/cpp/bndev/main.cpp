			...
			if(AInputEvent_getSource(event)==AINPUT_SOURCE_TOUCHSCREEN){//如果是触屏
				...
				switch(id){
					...
					case AMOTION_EVENT_ACTION_MOVE:	//触控点移动
						...
						MyVulkanManager::yAngle=MyVulkanManager::yAngle+xDis*180.0/600;//y轴旋转角
						MyVulkanManager::zAngle=MyVulkanManager::zAngle+yDis*180.0/600;//z轴旋转角
						xPre=x;	//记录触控点x坐标
						yPre=y; //记录触控点y坐标
						break;
					case AMOTION_EVENT_ACTION_UP:break;//触控点抬起
				}
			}return true;
		}return false;
	}
