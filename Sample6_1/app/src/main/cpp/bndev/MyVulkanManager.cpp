...
void MyVulkanManager::flushUniformBuffer(){	//将当前帧的一致变量数据送入一致变量缓冲的方法
    float fragmentUniformData[1]={0.9};	//亮度调节系数，与片元着色器中brightFactor对应
    uint8_t *pData;	//CPU 访问设备内存时的辅助指针
    VkResult result = vk::vkMapMemory(device, sqsCT->memUniformBuf,
                                      0, sqsCT->bufferByteCount, 0, (void **)&pData);//将指定设备内存映射为CPU可访问
    assert(result==VK_SUCCESS);   //检查映射是否成功
    memcpy(pData, fragmentUniformData, sqsCT->bufferByteCount);	//将数据拷贝进设备内存
    vk::vkUnmapMemory(device,sqsCT->memUniformBuf);	//解除内存映射
}
void MyVulkanManager::flushTexToDesSet(){	//将纹理等数据与描述集关联的方法
    for(int i=0;i<TextureManager::texNames.size();i++){	//遍历所有纹理
        sqsCT->writes[0].dstSet = sqsCT->descSet[i];//更新描述集对应的写入属性0(一致变量)
        sqsCT->writes[1].dstSet = sqsCT->descSet[i];//更新描述集对应的写入属性1(纹理)
        sqsCT->writes[1].pImageInfo = //写入属性1对应的纹理图像信息
                &(TextureManager::texImageInfoList[TextureManager::texNames[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsCT->writes, 0, NULL);//更新描述集
    }}
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        texTri->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, //绘制纹理三角形
                         &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/wall.bntex")]));
