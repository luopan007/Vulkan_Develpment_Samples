...
float* MatrixState3D::getFinalMatrixExternal(float* vpM){ //获取光源处总变换矩阵的方法
	Matrix::multiplyMM(mMVPMatrix, 0, vpM, 0, currMatrix, 0); //投影与观察组合矩阵与当前变换矩阵相乘
	Matrix::multiplyMM(mMVPMatrix, 0, //进一步乘以 Vulkan 标准设备空间调整矩阵
					   vulkanClipMatrix, 0, mMVPMatrix, 0);
	return mMVPMatrix; //返回指向总变换矩阵数组首地址的指针
}
