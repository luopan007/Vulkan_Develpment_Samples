...
ShaderQueueSuit_CommonDepth* MyVulkanManager::sqsCD;
...
ObjObject*  MyVulkanManager::pmForDraw;
...
void MyVulkanManager::createDrawableObject()
{
    ...
    pmForDraw=LoadUtil::loadFromFile("model/pm.obj",device, memoryroperties);
}
...
void MyVulkanManager::initMatrixAndLight()
{
    ...
    LightManager::setLightPosition(0,30);
...
{
    float vertexUniformData[6]=
    {
            LightManager::lx, LightManager::ly, LightManager::lz, 1.0, 
            LIGNT_CAMERA_NEAR,LIGNT_CAMERA_FAR
    };
    uint8_t *pData;
    VkResult result = vk::vkMapMemory(device, sqsCD->memUniformBuf, 0, sqsCD->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCD->bufferByteCount);
    vk::vkUnmapMemory(device,sqsCD->memUniformBuf);
}
void MyVulkanManager::flushTexToDesSetForToTex()
{
    sqsCD->writes[0].dstSet = sqsCD->descSet[0];
    vk::vkUpdateDescriptorSets(device, 1, sqsCD->writes, 0, NULL);
}
void MyVulkanManager::drawSceneToTex(){ //将场景绘制到纹理的方法
    rp_begin.framebuffer = selfTexFramebuffer; //将自定义帧缓冲设置为渲染通道的当前帧缓冲
    vk::vkResetCommandBuffer(cmdBuffer, 0); //恢复命令缓冲到初始状态
    VkResult result = vk::vkBeginCommandBuffer(cmdBuffer, &cmd_buf_info); //启动命令缓冲
    LightManager::setLightPosition(1,30); //设置光源位置
    LightManager::getTYJZ(screenWidth*1.0/screenHeight); //获取投影与观察组合矩阵的方法
    MyVulkanManager::flushUniformBufferForToTex(); //将一致变量缓冲送入渲染管线
    ...
    MatrixState3D::translate(0,0,-100); //进行平移变换
    MatrixState3D::scale(3,3,3); //进行缩放变换
    CHForDraw->drawSelf(cmdBuffer,sqsCD->pipelineLayout,sqsCD->pipeline,//绘制茶壶
                        &(sqsCD->descSet[0]),true);
    MatrixState3D::popMatrix(); //恢复现场
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-87,0,50);
    treeForDraw->drawSelf(cmdBuffer,sqsCD->pipelineLayout,sqsCD->pipeline,
                          &(sqsCD->descSet[0]),true);
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(87,0,50);
    carForDraw->drawSelf(cmdBuffer,sqsCD->pipelineLayout,sqsCD->pipeline,
                         &(sqsCD->descSet[0]),true);
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    pmForDraw->drawSelf(cmdBuffer,sqsCD->pipelineLayout,sqsCD->pipeline,
                         &(sqsCD->descSet[0]),true);
    MatrixState3D::popMatrix();
...
void MyVulkanManager::drawSceneToScreen()
{
    ...
    CHForDraw->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline, //绘制茶壶
                        &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight
                                ("texture/ghxp.bntex")]),false);
    MatrixState3D::popMatrix(); //恢复现场
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-87,0,50);
    treeForDraw->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                          &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/tree.bntex")]),false);
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(87,0,50);
    carForDraw->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                         &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/car.bntex")]),false);
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    pmForDraw->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                        &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndexForCommonTexLight("texture/floor.bntex")]),false);
    MatrixState3D::popMatrix();
    vk::vkCmdEndRenderPass(cmdBuffer); //结束渲染通道
    result = vk::vkEndCommandBuffer(cmdBuffer);
...
{
    sqsCD=new ShaderQueueSuit_CommonDepth(&device,renderPass,memoryroperties);
...
void MyVulkanManager::flushUniformBufferForToScreen()
{
    ...
    uint8_t *pData;
    VkResult result = vk::vkMapMemory(device, sqsCTL->memUniformBufVert, 0, sqsCTL->bufferByteCountVert, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCTL->bufferByteCountVert);
    vk::vkUnmapMemory(device,sqsCTL->memUniformBufVert);
    float vertexUniformDataFrag[18]=
    {
            LightManager::lcpm[0], LightManager::lcpm[1], LightManager::lcpm[2], LightManager::lcpm[3],                             
            LightManager::lcpm[4], LightManager::lcpm[5], LightManager::lcpm[6], LightManager::lcpm[7],
            LightManager::lcpm[8], LightManager::lcpm[9], LightManager::lcpm[10], LightManager::lcpm[11],
            LightManager::lcpm[12], LightManager::lcpm[13], LightManager::lcpm[14], LightManager::lcpm[15],
            LIGNT_CAMERA_NEAR,LIGNT_CAMERA_FAR
    };
    result = vk::vkMapMemory(device, sqsCTL->memUniformBufFrag, 0, sqsCTL->bufferByteCountFrag, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, vertexUniformDataFrag, sqsCTL->bufferByteCountFrag);
    vk::vkUnmapMemory(device,sqsCTL->memUniformBufFrag);
}
void MyVulkanManager::flushTexToDesSetForToScreen()
{
    for(int i=0;i<TextureManager::texNamesForCommonTexLight.size();i++)
    {
        ...
        sqsCTL->writes[2].dstSet = sqsCTL->descSet[i];
        sqsCTL->writes[1].dstSet = sqsCTL->descSet[i];
        sqsCTL->writes[1].pImageInfo = &colorImageInfo;
        sqsCTL->writes[3].dstSet = sqsCTL->descSet[i];
        sqsCTL->writes[3].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForCommonTexLight[i]]);
        vk::vkUpdateDescriptorSets(device, 4, sqsCTL->writes, 0, NULL);
    }
}