...
std::vector<std::string> TextureManager::texNames={"texture/moon.bntex","texture/earth.bntex" ,"texture/earthn.bntex", "texture/cloud.bntex" };
...
std::vector<std::string> TextureManager::texNameCloud = { "texture/cloud.bntex" };
...
{
    int result = -1;
    for (int i = 0; i<texNamesSingle.size(); i++)
    {
        if (texNamesSingle[i].compare(texName.c_str()) == 0)
        {
            result = i;
            break;
        }
