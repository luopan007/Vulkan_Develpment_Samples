...
ShaderQueueSuit_DoubleTexLight* MyVulkanManager::sqsDTL;
ShaderQueueSuit_SingleTexLight* MyVulkanManager::sqsSTL;
ShaderQueueSuit_Color* MyVulkanManager::sqsC;
ShaderQueueSuit_Cloud* MyVulkanManager::sqsCloud;
TexLightObject*  MyVulkanManager::ballForDraw;
TexLightObject* MyVulkanManager::cloudBallForDraw;
...
{
    BallData::genBallData(3, 30);
    ballForDraw = new TexLightObject(BallData::vdata, BallData::dataByteCount, BallData::vCount, device, memoryroperties);
    BallData::genBallData(3, 30.2);
    cloudBallForDraw = new TexLightObject(BallData::vdata, BallData::dataByteCount, BallData::vCount, device, memoryroperties);
    SkyData::genSkyData(400);
    skyForDrawBig = new ColorObject(SkyData::vdata, SkyData::dataByteCount, SkyData::vCount, device, memoryroperties, 4.0f);
    SkyData::genSkyData(1600);
    skyForDrawSmall = new ColorObject(SkyData::vdata, SkyData::dataByteCount, SkyData::vCount, device, memoryroperties, 2.0f);
}
void MyVulkanManager::destroyDrawableObject()
{
    delete ballForDraw;
    delete cloudBallForDraw;
...
void MyVulkanManager::flushUniformBuffer()
{
    ...
    result = vk::vkMapMemory(device, sqsCloud->memUniformBuf, 0, sqsCloud->bufferByteCount, 0, (void **)&pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCloud->bufferByteCount);
    vk::vkUnmapMemory(device, sqsCloud->memUniformBuf);
}
void MyVulkanManager::flushTexToDesSet()
{
    ...
    {
        sqsDTL->writes[0].dstSet = sqsDTL->descSet[i];
        sqsDTL->writes[1].dstSet = sqsDTL->descSet[i];
        sqsDTL->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesPair[i * 2 + 0]]);
        sqsDTL->writes[2].dstSet = sqsDTL->descSet[i];
        sqsDTL->writes[2].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesPair[i * 2 + 1]]);
        vk::vkUpdateDescriptorSets(device, 3, sqsDTL->writes, 0, NULL);
    }
    for(int i = 0; i < TextureManager::texNameCloud.size(); i ++)
    {
        sqsCloud->writes[0].dstSet = sqsCloud->descSet[i];
        sqsCloud->writes[1].dstSet = sqsCloud->descSet[i];
        sqsCloud->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNameCloud[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsCloud->writes, 0, NULL);
    }
}
...
{   //执行绘制的方法
    ...
    FPSUtil::init(); //初始化 FPS 计算
    while(MyVulkanManager::loopDrawFlag)
    {
        //每循环一次绘制一帧画面
        eAngle = float(eAngle + 0.4);if (eAngle >= 360)eAngle = 0; //更新地球自转角
        mAngle = float(mAngle + 0.4);if (mAngle >= 360)mAngle = 0;//更新月球自转角
        sAngle = float(sAngle + 0.02);if (sAngle >= 360)sAngle = 0; //更新星空自转角
        FPSUtil::calFPS();
        FPSUtil::before();
        CameraUtil::flushCameraToMatrix(); //设置摄像机位置
        ...
        MatrixState3D::rotate(eAngle, 0, 1, 0); //绕 y 轴旋转（地球自转）
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::scale(3, 3, 3); //进行缩放（地球比月球大）
        ballForDraw->drawSelf(cmdBuffer, sqsDTL->pipelineLayout, //绘制地球
                sqsDTL->pipeline,&(sqsDTL->descSet[0]));
        cloudBallForDraw->drawSelf(cmdBuffer, sqsCloud->pipelineLayout, sqsCloud->pipeline,
                &(sqsCloud->descSet[0])); //绘制地球云层
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::translate(180, 0, 0); //沿 x 轴移动（地月距离）
        MatrixState3D::rotate(mAngle, 0, 1, 0); //绕 y 轴旋转（月球自转）
        ballForDraw->drawSelf(cmdBuffer, sqsSTL->pipelineLayout, sqsSTL->pipeline,//绘制月球
                &(sqsSTL->descSet[TextureManager:: getVkDescriptorSetIndexForCommonTexLight
                        ("texture/moon.bntex" )]));
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::rotate(sAngle, 0, 1, 0); //绕 y 轴旋转（星空缓慢自传）
        skyForDrawBig->drawSelf(cmdBuffer, sqsC->pipelineLayout, sqsC->pipeline); //绘制星空(大)
        skyForDrawSmall->drawSelf(cmdBuffer, sqsC->pipelineLayout, sqsC->pipeline);//绘制星空(小)
        MatrixState3D::popMatrix(); //恢复现场
...
{
    sqsC = new ShaderQueueSuit_Color(&device, renderPass, memoryroperties);
    sqsDTL = new ShaderQueueSuit_DoubleTexLight(&device, renderPass, memoryroperties);
    sqsSTL = new ShaderQueueSuit_SingleTexLight(&device, renderPass, memoryroperties);
    sqsCloud = new ShaderQueueSuit_Cloud(&device, renderPass, memoryroperties);
    LOGE("管线初始化成功 sqsC sqsDTL sqsSTL sqsCloud");
}
