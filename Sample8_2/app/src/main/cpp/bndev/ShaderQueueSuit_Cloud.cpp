#include "ShaderQueueSuit_Cloud.h"
#include <assert.h>
...
void ShaderQueueSuit_Cloud::create_uniform_buffer(VkDevice& device, VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
void ShaderQueueSuit_Cloud::destroy_uniform_buffer(VkDevice& device)
{
    vk::vkDestroyBuffer(device, uniformBuf, NULL);
    vk::vkFreeMemory(device, memUniformBuf, NULL);
}
void ShaderQueueSuit_Cloud::create_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_Cloud::destroy_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_Cloud::init_descriptor_set(VkDevice& device)
{
    VkDescriptorPoolSize type_count[2];
    type_count[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    type_count[0].descriptorCount = TextureManager::texNameCloud.size();
    type_count[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[1].descriptorCount = TextureManager::texNameCloud.size();
    VkDescriptorPoolCreateInfo descriptor_pool = {};
    descriptor_pool.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    descriptor_pool.pNext = NULL;
    descriptor_pool.maxSets = TextureManager::texNameCloud.size();
    ...
    alloc_info[0].descriptorSetCount = TextureManager::texNameCloud.size();
    alloc_info[0].pSetLayouts = layouts.data();
    descSet.resize(TextureManager::texNameCloud.size());
...
void ShaderQueueSuit_Cloud::create_shader(VkDevice& device)
{
    std::string vertStr = FileUtil::loadAssetStr("shader/CloudTexLight.vert");
    std::string fragStr = FileUtil::loadAssetStr("shader/CloudTexLight.frag");
...
void ShaderQueueSuit_Cloud::create_pipe_line(VkDevice& device, VkRenderPass& renderPass)
{
...
void ShaderQueueSuit_Cloud::destroy_pipe_line(VkDevice& device)
{
    vk::vkDestroyPipeline(device, pipeline, NULL);
    vk::vkDestroyPipelineCache(device, pipelineCache, NULL);
}
ShaderQueueSuit_Cloud::ShaderQueueSuit_Cloud(VkDevice* deviceIn, VkRenderPass& renderPass, VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ShaderQueueSuit_Cloud::~ShaderQueueSuit_Cloud()
{
