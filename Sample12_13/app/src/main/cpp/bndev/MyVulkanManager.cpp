...
void MyVulkanManager::createDrawableObject() {
    LandData* ld = FileUtil::loadHdtData("texture/land.bnhdt");//初始化灰度图
...
void MyVulkanManager::flushUniformBuffer() {
    ...
    float uniformData[5]=
            {
                    CameraUtil::cx,CameraUtil::cy,CameraUtil::cz,0,uFactor
            };
    uint8_t *pData;
    VkResult result = vk::vkMapMemory(device, sqsDou->memUniformBuf, 0, sqsDou->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, uniformData, sqsDou->bufferByteCount);
    vk::vkUnmapMemory(device,sqsDou->memUniformBuf);
}
