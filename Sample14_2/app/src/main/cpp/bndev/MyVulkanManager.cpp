...
ObjObject*  MyVulkanManager::objObjectCFT;
ObjObject*  MyVulkanManager::objObjectQT;
ObjObject*  MyVulkanManager::objObjectYH;
...
void MyVulkanManager::createDrawableObject(){					
    objObjectCH	 = LoadUtil::loadFromFile("model/ch.obj", device, memoryroperties);
    objObjectCFT = LoadUtil::loadFromFile("model/cft.obj", device, memoryroperties);
    objObjectQT	 = LoadUtil::loadFromFile("model/qt.obj", device, memoryroperties);
    objObjectYH	 = LoadUtil::loadFromFile("model/yh.obj", device, memoryroperties);
...
void MyVulkanManager::initMatrixAndLight()
{
    ...
    CameraUtil::calCamera(45,0);
    LightManager::setLightPosition();
    LightManager::setlightAmbient(0.4f,0.4f,0.4f,0.4f);
    LightManager::setlightDiffuse(0.7f,0.7f,0.7f,0.7f);
    LightManager::setlightSpecular(0.3f,0.3f,0.3f,0.3f);
}
void MyVulkanManager::flushUniformBuffer()
{
    float vertexUniformDataShadow[36]=
            {
    ...
    assert(result==VK_SUCCESS);
    memcpy(pData, vertexUniformDataShadow, sqsCT->bufferByteCount);
    vk::vkUnmapMemory(device,sqsCT->memUniformBuf);
}
void MyVulkanManager::flushTexToDesSet()
{
    sqsCT->writes[0].dstSet = sqsCT->descSet[0];
    vk::vkUpdateDescriptorSets(device, 1, sqsCT->writes, 0, NULL);
}
void MyVulkanManager::drawThings(float isShadow) //绘制平面上各个物体的方法
{
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::scale(1.5f, 1.5f, 1.5f); //进行缩放变换
        MatrixState3D::translate(0, 0, 10); //进行平移变换
        objObjectCH->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                          &(sqsCT->descSet[0]),isShadow); //绘制茶壶
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix();
        MatrixState3D::scale(1.5f, 1.5f, 1.5f);
        MatrixState3D::translate(10, 0, 0);
        objObjectQT->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                              &(sqsCT->descSet[0]),isShadow);
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::scale(1.5f, 1.5f, 1.5f);
        MatrixState3D::translate(0, 0, -10);
        objObjectYH->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                              &(sqsCT->descSet[0]),isShadow);
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::scale(1.5f, 1.5f, 1.5f);
        MatrixState3D::translate(-10, 0, 0);
        objObjectCFT->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                               &(sqsCT->descSet[0]),isShadow);
        MatrixState3D::popMatrix();
}
void MyVulkanManager::drawObject(){ //执行绘制的方法
    ...
    while(MyVulkanManager::loopDrawFlag){ //每循环一次绘制一帧画面
        ...
        MProjCameraMatrix = MatrixState3D::getViewProjMatrix();//获取投影与观察组合矩阵
        ...
        objObjectPM->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
