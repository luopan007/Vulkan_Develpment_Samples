...
ObjObject* MyVulkanManager::LYobjPeople;
ObjObject* MyVulkanManager::LYobjDiban;
bool MyVulkanManager::isSZTJC;//是否进行视锥体剪裁的标志
int rectNum = 45;
float pSpan = 6;
float startPostion= -(rectNum -1)*pSpan / 2.0f;
int centerNum = rectNum / 2;
...
void MyVulkanManager::initMatrixAndLight()
{
    ...
    MatrixState3D::setProjectFrustum(-ratio,ratio,-1,1,3,1000);
}
void MyVulkanManager::createDrawableObject()
{
    LYobjPeople= LoadUtil::loadFromFile("model/people.obj", device, memoryroperties);
    LYobjDiban = LoadUtil::loadFromFile("model/pm.obj", device, memoryroperties);
}
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        int currDrawCount = 0;
        /*
          @param rectNum 行数、列数、高度数
          @param centerNum 中心位置
          @param startPostion 起始坐标
         * */
        for (int jrow = 0; jrow < rectNum; jrow++) {
            for (int kcol = 0; kcol < rectNum; kcol++) {
                if ( jrow!=centerNum || kcol!=centerNum) {
                    MatrixState3D::pushMatrix();
                    MatrixState3D::translate(startPostion + kcol*pSpan, 0, startPostion + jrow*pSpan);
                    if (isSZTJC)//开启视锥体剪裁
                    {
                        if (SZTJCUtil::isAABBInSZT(MatrixState3D::getGLFinalMatrix(), LYobjPeople->aabb))
                        {
                            LYobjPeople->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/people.bntex")]));
                            currDrawCount++;
                        }
                    }
                    else
        ...
        if (isSZTJC)//开启视锥体剪裁
        {
            if (SZTJCUtil::isAABBInSZT(MatrixState3D::getGLFinalMatrix(), LYobjDiban->aabb))
            {
                LYobjDiban->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(("texture/diban.bntex"))]));
                currDrawCount++;
            }
        }
        else
        ...
        FPSUtil::drawCount.push_back(currDrawCount);
        FPSUtil::after(60);
    }
}
