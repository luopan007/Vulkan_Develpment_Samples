...
void ShaderQueueSuit_CommonTexLight::create_uniform_buffer_vert(VkDevice &device,
                                                                VkPhysicalDeviceMemoryProperties &memoryroperties)
{
    bufferByteCountVert=sizeof(float)*(20);
    VkBufferCreateInfo buf_info = {};
    buf_info.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
    buf_info.pNext = NULL;
    buf_info.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
    buf_info.size = bufferByteCountVert;
    buf_info.queueFamilyIndexCount = 0;
    buf_info.pQueueFamilyIndices = NULL;
    buf_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    buf_info.flags = 0;
    VkResult result = vk::vkCreateBuffer(device, &buf_info, NULL, &uniformBufVert);
    assert(result == VK_SUCCESS);
    VkMemoryRequirements mem_reqs;
    vk::vkGetBufferMemoryRequirements(device, uniformBufVert, &mem_reqs);
    ...
    result = vk::vkAllocateMemory(device, &alloc_info, NULL, &memUniformBufVert);
    assert(result == VK_SUCCESS);
    result = vk::vkBindBufferMemory(device, uniformBufVert, memUniformBufVert, 0);
    assert(result == VK_SUCCESS);
    uniformBufferInfoVert.buffer = uniformBufVert;
    uniformBufferInfoVert.offset = 0;
    uniformBufferInfoVert.range = bufferByteCountVert;
}
void ShaderQueueSuit_CommonTexLight::destroy_uniform_buffer_vert(VkDevice &device)
{
    vk::vkDestroyBuffer(device, uniformBufVert, NULL);
    vk::vkFreeMemory(device, memUniformBufVert, NULL);
}
...
void ShaderQueueSuit_CommonTexLight::init_descriptor_set(VkDevice& device)
{
    ...
    type_count[2].descriptorCount = TextureManager::texNamesForCommonTexLight.size();
    type_count[3].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[3].descriptorCount = TextureManager::texNamesForCommonTexLight.size();
    ...
    writes[0].pBufferInfo = &uniformBufferInfoVert;
...
void ShaderQueueSuit_CommonTexLight::create_uniform_buffer_frag(VkDevice &device,VkPhysicalDeviceMemoryProperties &memoryroperties)
{
    bufferByteCountFrag=sizeof(float)*(18);
    VkBufferCreateInfo buf_info = {};
    buf_info.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
    buf_info.pNext = NULL;
    buf_info.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
...
void ShaderQueueSuit_CommonTexLight::destroy_uniform_buffer_frag(VkDevice &device)
{
