...
void ShaderQueueSuit_Common::create_pipeline_layout(VkDevice& device)
{
    ...
    assert(result == VK_SUCCESS);
    const unsigned push_constant_range_count = 3;
    VkPushConstantRange push_constant_ranges[push_constant_range_count] = {};
    push_constant_ranges[0].stageFlags = VK_SHADER_STAGE_GEOMETRY_BIT;
    push_constant_ranges[0].offset = 0;
    push_constant_ranges[0].size = sizeof(float)*20;
    push_constant_ranges[1].stageFlags = VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT;
    push_constant_ranges[1].offset = sizeof(float)*20;
    push_constant_ranges[1].size = sizeof(float)*16;
    push_constant_ranges[2].stageFlags = VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT;
    push_constant_ranges[2].offset =sizeof(float)*36;
    push_constant_ranges[2].size = sizeof(float)*4;
...
void ShaderQueueSuit_Common::create_shader(VkDevice& device)
{
    ...
    std::string tcsStr= FileUtil::loadAssetStr("shader/commonTexLight.tcs");
    std::string tesStr= FileUtil::loadAssetStr("shader/commonTexLight.tes");
    std::string geoStr= FileUtil::loadAssetStr("shader/commonTexLight.geo");
    ...
    std::vector<unsigned int> tcs_spv;
    retVal = GLSLtoSPV(VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT, tcsStr.c_str(), tcs_spv);
    assert(retVal);
    LOGE("曲面细分控制着色器脚本编译SPV成功！");
    moduleCreateInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
    moduleCreateInfo.pNext = NULL;
    moduleCreateInfo.flags = 0;
    moduleCreateInfo.codeSize = tcs_spv.size() * sizeof(unsigned int);
    moduleCreateInfo.pCode = tcs_spv.data();
    result = vk::vkCreateShaderModule(device, &moduleCreateInfo, NULL, &shaderStages[1].module);
    assert(result == VK_SUCCESS);
    shaderStages[2].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    shaderStages[2].pNext = NULL;
    ...
    std::vector<unsigned int> tes_spv;
    retVal = GLSLtoSPV(VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT, tesStr.c_str(), tes_spv);
    assert(retVal);
    LOGE("曲面细分执行着色器脚本编译SPV成功！");
    moduleCreateInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
    moduleCreateInfo.pNext = NULL;
    moduleCreateInfo.flags = 0;
    moduleCreateInfo.codeSize = tes_spv.size() * sizeof(unsigned int);
    moduleCreateInfo.pCode = tes_spv.data();
    result = vk::vkCreateShaderModule(device, &moduleCreateInfo, NULL, &shaderStages[2].module);
    assert(result == VK_SUCCESS);
    shaderStages[3].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    shaderStages[3].pNext = NULL;
    shaderStages[3].pSpecializationInfo = NULL;
    shaderStages[3].flags = 0;
    shaderStages[3].stage = VK_SHADER_STAGE_GEOMETRY_BIT;
    shaderStages[3].pName = "main";
    std::vector<unsigned int> geo_spv;
    retVal = GLSLtoSPV(VK_SHADER_STAGE_GEOMETRY_BIT, geoStr.c_str(), geo_spv);
    assert(retVal);
    LOGE("几何着色器脚本编译SPV成功！");
    moduleCreateInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
    moduleCreateInfo.pNext = NULL;
    moduleCreateInfo.flags = 0;
    moduleCreateInfo.codeSize = geo_spv.size() * sizeof(unsigned int);
    moduleCreateInfo.pCode = geo_spv.data();
    result = vk::vkCreateShaderModule(device, &moduleCreateInfo, NULL, &shaderStages[3].module);
    assert(result == VK_SUCCESS);
    shaderStages[4].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    shaderStages[4].pNext = NULL;
    shaderStages[4].pSpecializationInfo = NULL;
    shaderStages[4].flags = 0;
    shaderStages[4].stage = VK_SHADER_STAGE_FRAGMENT_BIT;
    ...
    result = vk::vkCreateShaderModule(device, &moduleCreateInfo, NULL, &shaderStages[4].module);
    assert(result == VK_SUCCESS);
}
