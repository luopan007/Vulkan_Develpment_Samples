...
ObjObject*  MyVulkanManager::KLForDraw;
ObjObject*  MyVulkanManager::ballForDraw;
int MyVulkanManager::needsNormal=0;
...
{
    KLForDraw=LoadUtil::loadFromFile("model/konglong.obj",device, memoryroperties);
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        for(int i=1;i<=4;i++)
        {
            ...
            MatrixState3D::translate(startX+i*20,0,0);
            MatrixState3D::rotate(-90,0,1,0);
            KLForDraw->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                                &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndex("texture/konglong.bntex")]),
                                i,i,i,i,needsNormal);
            ...
            ballForDraw->drawSelf(cmdBuffer,sqsCTL->pipelineLayout,sqsCTL->pipeline,
                                &(sqsCTL->descSet[TextureManager::getVkDescriptorSetIndex("texture/rock.bntex")]),
                                i,i,i,i,needsNormal);
            MatrixState3D::popMatrix();
        }
