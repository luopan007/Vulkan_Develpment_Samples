...
ShaderQueueSuit_Discard* MyVulkanManager::sqsD;
ObjObject*  MyVulkanManager::chuang;
ObjObject*  MyVulkanManager::pm;
ObjObject*  MyVulkanManager::zhuotui;
ObjObject*  MyVulkanManager::zhuomian;
bool MyVulkanManager::ifDiscard=true;
...
void MyVulkanManager::createDrawableObject() {
    chuang = LoadUtil::loadFromFile("model/chuang.obj", device, memoryroperties);
    pm = LoadUtil::loadFromFile("model/pm.obj", device, memoryroperties);
    zhuomian = LoadUtil::loadFromFile("model/zhuomian.obj", device, memoryroperties);
    zhuotui = LoadUtil::loadFromFile("model/zhuotui.obj", device, memoryroperties);
}
...
void MyVulkanManager::initMatrix() {
    ...
    MatrixState3D::setProjectFrustum(-ratio,ratio,-1,1,2.8f,1000);
...
void MyVulkanManager::flushUniformBuffer() {
    ...
    uint8_t *pDataD;
    result = vk::vkMapMemory(device, sqsD->memUniformBuf,0, sqsD->bufferByteCount, 0, (void **)&pDataD);
    assert(result==VK_SUCCESS);
    memcpy(pDataD, vertexUniformData, sqsD->bufferByteCount);
    vk::vkUnmapMemory(device,sqsD->memUniformBuf);
}
void MyVulkanManager::flushTexToDesSet() {
    ...
    for(int i=0;i<TextureManager::texNames.size();i++)
    {
        sqsD->writes[0].dstSet = sqsD->descSet[i];
        sqsD->writes[1].dstSet = sqsD->descSet[i];
        sqsD->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsD->writes, 0, NULL);
    }
}
void MyVulkanManager::drawObject() {
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        MatrixState3D::translate(0, -7, 0);
        MatrixState3D::pushMatrix();
        chuang->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                         &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/chuang.bntex")]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::scale(0.8, 0.8, 0.8);
        pm->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                     &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/dm.bntex")]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(0, 3, -10);
        MatrixState3D::scale(0.5, 0.5, 0.5);
        zhuomian->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                           &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/zhuomian.bntex")]));
        zhuotui->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                          &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/zhuotui.bntex")]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(0,7.5,-1);
        MatrixState3D::rotate(-90, 1, 0, 0);
        MatrixState3D::scale( 0.11, 1, 0.12);
        if(ifDiscard)
        {
            pm->drawSelf(cmdBuffer, sqsD->pipelineLayout, sqsD->pipeline,
                         &(sqsD->descSet[TextureManager::getVkDescriptorSetIndex("texture/sha.bntex")]));
        }
        else
        {
            pm->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                         &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/sha.bntex")]));
        }
        MatrixState3D::popMatrix();
        MatrixState3D::popMatrix();
...
void MyVulkanManager::initPipeline()
{
    ...
    sqsD=new ShaderQueueSuit_Discard(&device,renderPass,memoryroperties);
}
