...
void LoadUtil::setAAssetManager(AAssetManager* aamIn)//初始化AAssetManager对象
{
...
ObjObject* LoadUtil::loadFromFile(const string& vname,VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
    ...
    {
        vdataIn[indexTemp++]=alvResult[i*3+0];//顶点x坐标
        vdataIn[indexTemp++]=alvResult[i*3+1];//顶点y坐标
        vdataIn[indexTemp++]=alvResult[i*3+2];//顶点z坐标

        vdataIn[indexTemp++]=altResult[i*2+0];//纹理s坐标
        vdataIn[indexTemp++]=altResult[i*2+1];//纹理s坐标

        vdataIn[indexTemp++]=alnResult[i*3+0];//法向量x分量
        vdataIn[indexTemp++]=alnResult[i*3+1];//法向量y分量
        vdataIn[indexTemp++]=alnResult[i*3+2];//法向量z分量
    }
