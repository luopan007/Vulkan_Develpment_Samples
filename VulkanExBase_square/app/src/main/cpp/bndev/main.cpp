...
static int32_t engine_handle_input(struct android_app *app, AInputEvent *event) {
    ...
    if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION) {
        ...
        if (AInputEvent_getSource(event) == AINPUT_SOURCE_TOUCHSCREEN) {
            ...
            if (ZFTManager::life && ZFTManager::Box_pos < TREE_NUMBER - 1) {
                switch (id) {
                    //触摸按下消息
                    case AMOTION_EVENT_ACTION_DOWN: {
                        ...
                        if (!Touch2D::Button_start) {
                            ...
                            switch (My3DLayer::flagGo[ZFTManager::Box_pos]) {
                                case 0://右拐
                                    LOGI("右拐\n");
                            ...
                            if (!Touch2D::Button_mute) {
                                PlaySound::playSound("sound/dingdong.wav");
                            }
                            My3DLayer::mainBox->setPosition3D(My3DLayer::mainBox->x, 100,
                                                              My3DLayer::mainBox->z);
                        }
                    }
                        break;
                ...
                switch (id) {
                    ...
                    case AMOTION_EVENT_ACTION_DOWN: {
                        if (!ZFTManager::life) {
                            Touch2D::CheckButton_resume(x, y);//是否点击重新开始按钮
                            Touch2D::CheckButton_back(x, y);//是否点击重新开始按钮
                            if (Touch2D::Button_resume) {
                                My3DLayer::RepeatLevel();//重置关卡
                            }
                        } else if (ZFTManager::Box_pos == TREE_NUMBER - 1) {
                            My3DLayer::NextLevel();
                        }
                    }
                        break;
