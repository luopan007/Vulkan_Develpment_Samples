...
//创建Unifirm Buffer
void ShaderQueueSuit_Dashboard2D::create_uniform_buffer(VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
//销毁Unifirm Buffer相关
void ShaderQueueSuit_Dashboard2D::destroy_uniform_buffer(VkDevice& device)
{
...
//创建管线layout
void ShaderQueueSuit_Dashboard2D::create_pipeline_layout(VkDevice& device)
{
    //设置DescriptorSet数量
    ...
    //服务于数据uniform的片元着色器中的“ binding = 0”
...
//销毁管线layout
void ShaderQueueSuit_Dashboard2D::destroy_pipeline_layout(VkDevice& device)
{
...
//初始化descriptor set
void ShaderQueueSuit_Dashboard2D::init_descriptor_set(VkDevice& device)
{
    //创建VkDescriptorPoolSize数组
    VkDescriptorPoolSize type_count[2];
    type_count[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    type_count[0].descriptorCount = TextureManager::texNamesForDashboard2D.size();
    type_count[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    type_count[1].descriptorCount = TextureManager::texNamesForDashboard2D.size();

    //创建VkDescriptorPoolCreateInfo实例
    VkDescriptorPoolCreateInfo descriptor_pool = {};
    descriptor_pool.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    descriptor_pool.pNext = NULL;
    descriptor_pool.maxSets = TextureManager::texNamesForDashboard2D.size();
    ...
    alloc_info[0].descriptorSetCount = TextureManager::texNamesForDashboard2D.size();
    alloc_info[0].pSetLayouts = layouts.data();

    //调整尺寸
    descSet.resize(TextureManager::texNamesForDashboard2D.size());
    //分配DescriptorSets
...
void ShaderQueueSuit_Dashboard2D::create_shader(VkDevice& device)
{
    std::string vertStr= FileUtil::loadAssetStr("shader/dashboard2D.vert");
    std::string fragStr= FileUtil::loadAssetStr("shader/dashboard2D.frag");
...
void ShaderQueueSuit_Dashboard2D::destroy_shader(VkDevice& device)
{
...
void ShaderQueueSuit_Dashboard2D::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
...
void ShaderQueueSuit_Dashboard2D::destroy_pipe_line(VkDevice& device)
{
    //销毁管线
...
ShaderQueueSuit_Dashboard2D::ShaderQueueSuit_Dashboard2D(VkDevice* deviceIn,VkRenderPass& renderPass,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
