...
std::vector<PlanePosition*> MyVulkanManager::planeVector;
...
void MyVulkanManager::createDrawableObject()
{
    ...
    planePosition0=new PlanePosition(0);
    objObject1=LoadUtil::loadFromFile("model/plane02.obj",device,memoryroperties);
    planePosition1=new PlanePosition(1);
    objObject2=LoadUtil::loadFromFile("model/plane03.obj",device,memoryroperties);
    planePosition2=new PlanePosition(2);
    objObject3=LoadUtil::loadFromFile("model/plane04.obj",device,memoryroperties);
    planePosition3=new PlanePosition(3);
    planeVector.push_back(planePosition0);planeVector.push_back(planePosition1);
    planeVector.push_back(planePosition2);planeVector.push_back(planePosition3);
}
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        for(auto planeP:planeVector){
            planeP->update();
            for(int i=0;i<planeP->count;i++){
                MatrixState3D::pushMatrix();
                MatrixState3D::translate(planeP->curPosition[i*4+0], planeP->curPosition[i*4+1], planeP->curPosition[i*4+2]);
                MatrixState3D::scale(0.2f,0.2f,0.2f);
                switch(planeP->id){
                    case 0: objObject->drawSelf(cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline,
                                                &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/plane01.bntex")]));
                        break;
                    case 1: objObject1->drawSelf(cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline,
                                                &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/plane02.bntex")]));
                        break;
                    case 2: objObject2->drawSelf(cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline,
                                                &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/plane03.bntex")]));
                        break;
                    case 3: objObject3->drawSelf(cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline,
                                                &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/plane04.bntex")]));
                        break;
                }
