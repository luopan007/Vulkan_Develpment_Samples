...
void ShaderQueueSuit_Common::initVertexAttributeInfo()
{
    ...
    vertexAttribs[0].binding = 0;//顶点坐标输入属性的绑定点
    vertexAttribs[0].location = 0;//顶点坐标输入属性的位置索引
    vertexAttribs[0].format = VK_FORMAT_R32G32B32_SFLOAT;//顶点坐标输入属性的数据格式
    vertexAttribs[0].offset = 0;//顶点坐标输入属性的偏移量
