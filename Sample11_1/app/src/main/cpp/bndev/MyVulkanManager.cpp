...
ShaderQueueSuit_CommonTex_X* MyVulkanManager::sqsCT;
ShaderQueueSuit_CommonTex_XY* MyVulkanManager::sqsXY;
...
float MyVulkanManager::uWidthSpan = 12.0f;
float MyVulkanManager::uStartAngle = 0.0f;
int MyVulkanManager::isSwap = 0;
...
void MyVulkanManager::flushUniformBuffer()
{
    ...
    uniformData[0] = uWidthSpan;
    uniformData[1] = uStartAngle;
    uint8_t *pData;
    VkResult result = vk::vkMapMemory(device, sqsCT->memUniformBuf, 0, sqsCT->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, uniformData, sqsCT->bufferByteCount);
    vk::vkUnmapMemory(device, sqsCT->memUniformBuf);

    result = vk::vkMapMemory(device, sqsXY->memUniformBuf, 0, sqsXY->bufferByteCount, 0, (void **)&pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, uniformData, sqsXY->bufferByteCount);
    vk::vkUnmapMemory(device, sqsXY->memUniformBuf);

    result = vk::vkMapMemory(device, sqsXie->memUniformBuf, 0, sqsXie->bufferByteCount, 0, (void **)&pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, uniformData, sqsXie->bufferByteCount);
...
void MyVulkanManager::flushTexToDesSet()
{
    ...
    for (int i = 0; i<TextureManager::texNames.size(); i++)
    {
        sqsXY->writes[0].dstSet = sqsXY->descSet[i];
        sqsXY->writes[1].dstSet = sqsXY->descSet[i];
        sqsXY->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsXY->writes, 0, NULL);
    }
    for(int i=0;i<TextureManager::texNames.size();i++)
    {
        sqsXie->writes[0].dstSet = sqsXie->descSet[i];
        sqsXie->writes[1].dstSet = sqsXie->descSet[i];
        sqsXie->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i]]);
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        if (isSwap == 0) {
            texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/vulkan.bntex")]));
        }else if(isSwap == 1) {
            texTri->drawSelf(cmdBuffer, sqsXY->pipelineLayout, sqsXY->pipeline, &(sqsXY->descSet[TextureManager::getVkDescriptorSetIndex("texture/vulkan.bntex")]));
        }else if (isSwap == 2) {
            texTri->drawSelf(cmdBuffer, sqsXie->pipelineLayout, sqsXie->pipeline, &(sqsXie->descSet[TextureManager::getVkDescriptorSetIndex("texture/vulkan.bntex")]));
        }
...
{
    sqsCT=new ShaderQueueSuit_CommonTex_X(&device,renderPass,memoryroperties);
    sqsXY=new ShaderQueueSuit_CommonTex_XY(&device, renderPass, memoryroperties);
    sqsXie=new ShaderQueueSuit_CommonTex_Xie(&device, renderPass, memoryroperties);
}
