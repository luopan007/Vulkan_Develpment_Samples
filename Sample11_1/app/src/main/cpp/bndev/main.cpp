			...
			if(AInputEvent_getSource(event)==AINPUT_SOURCE_TOUCHSCREEN)
			{
				...
				switch(id)
				{
					...
					case AMOTION_EVENT_ACTION_MOVE:
						...
						if(!isClick){
							MyVulkanManager::yAngle=MyVulkanManager::yAngle+xDis*180.0/600;
							MyVulkanManager::zAngle=MyVulkanManager::zAngle+yDis*180.0/600;
						}
						xPre=x;
						yPre=y;
					break;
					case AMOTION_EVENT_ACTION_UP:
						if(isClick){
							if (xPre>0&& xPre<200&& yPre>0&& yPre<200) {
								MyVulkanManager::isSwap = (++MyVulkanManager::isSwap) % 3;
							}
						}
					break;
				}
