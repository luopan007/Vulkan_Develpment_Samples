#include "ShaderQueueSuit_CommonTex_X.h"
#include <assert.h>
...
void ShaderQueueSuit_CommonTex_X::create_uniform_buffer(VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
void ShaderQueueSuit_CommonTex_X::destroy_uniform_buffer(VkDevice& device)
{
    vk::vkDestroyBuffer(device, uniformBuf, NULL);
    vk::vkFreeMemory(device, memUniformBuf, NULL);
}
void ShaderQueueSuit_CommonTex_X::create_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonTex_X::destroy_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonTex_X::init_descriptor_set(VkDevice& device)
{
...
void ShaderQueueSuit_CommonTex_X::create_shader(VkDevice& device)
{
    std::string vertStr= FileUtil::loadAssetStr("shader/commonTexX.vert");
...
void ShaderQueueSuit_CommonTex_X::destroy_shader(VkDevice& device)
{
    vk::vkDestroyShaderModule(device, shaderStages[0].module, NULL);
    vk::vkDestroyShaderModule(device, shaderStages[1].module, NULL);
}
void ShaderQueueSuit_CommonTex_X::initVertexAttributeInfo()
{
...
void ShaderQueueSuit_CommonTex_X::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
...
void ShaderQueueSuit_CommonTex_X::destroy_pipe_line(VkDevice& device)
{
    vk::vkDestroyPipeline(device, pipeline, NULL);
    vk::vkDestroyPipelineCache(device, pipelineCache, NULL);
}
ShaderQueueSuit_CommonTex_X::ShaderQueueSuit_CommonTex_X(VkDevice* deviceIn,VkRenderPass& renderPass,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ShaderQueueSuit_CommonTex_X::~ShaderQueueSuit_CommonTex_X()
{
