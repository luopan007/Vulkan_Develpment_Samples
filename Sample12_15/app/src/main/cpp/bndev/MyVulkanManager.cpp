...
#include <algorithm>
#include "../util/ParticleForDraw.h"
...
ShaderQueueSuit_CommonTexParticle* MyVulkanManager::sqsParticle;
ShaderQueueSuit_CommonTexParticleTwo* MyVulkanManager::sqsParticleTwo;
ShaderQueueSuit_CommonTexParticleThree* MyVulkanManager::sqsParticleThree;
ShaderQueueSuit_CommonTexParticleFour* MyVulkanManager::sqsParticleFour;
ObjObject* MyVulkanManager::LYobjB;
ObjObject* MyVulkanManager::LYobjDiban;
ParticleForDraw* MyVulkanManager::LYobjParticle[4];
std::vector<ParticleSystem*> MyVulkanManager::particleVector;
std::mutex MyVulkanManager::myLock;//资源锁
...
{
    LYobjB = LoadUtil::loadFromFileVertexAndTex("model/brazier.obj", device, memoryroperties);
    FlatData::genVertexData();
    LYobjDiban = new ObjObject(FlatData::vdata, FlatData::dataByteCount, FlatData::vCount, device, memoryroperties);
    for(int i=0;i<4;i++){
        particleVector.push_back(new ParticleSystem(COUNT[i],ParticlePositon[i][0],ParticlePositon[i][1],i));
        particleVector[i]->genVertexData();
        LYobjParticle[i]=new ParticleForDraw(particleVector[i]->vdata, particleVector[i]->dataByteCount, particleVector[i]->vCount, device, memoryroperties);
    }
}
    ...
    float uniformData[6]=
            {
                   5.0f,
                   RADIS[0],
                   CameraUtil::cx,CameraUtil::cy,CameraUtil::cz,1.0f,
            };
    uint8_t *pData;
    VkResult result = vk::vkMapMemory(device, sqsParticle->memUniformBuf, 0, sqsParticle->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, uniformData, sqsParticle->bufferByteCount);
    vk::vkUnmapMemory(device, sqsParticle->memUniformBuf);

    uniformData[1]=RADIS[1];
    result = vk::vkMapMemory(device, sqsParticleTwo->memUniformBuf, 0, sqsParticleTwo->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, uniformData, sqsParticleTwo->bufferByteCount);
    vk::vkUnmapMemory(device, sqsParticleTwo->memUniformBuf);

    uniformData[1]=RADIS[2];
    result = vk::vkMapMemory(device, sqsParticleThree->memUniformBuf, 0, sqsParticleThree->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, uniformData, sqsParticleThree->bufferByteCount);
    vk::vkUnmapMemory(device, sqsParticleThree->memUniformBuf);

    uniformData[1]=RADIS[3];
    result = vk::vkMapMemory(device, sqsParticleFour->memUniformBuf, 0, sqsParticleFour->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, uniformData, sqsParticleFour->bufferByteCount);
    vk::vkUnmapMemory(device, sqsParticleFour->memUniformBuf);
}
void MyVulkanManager::updateParticleSystem()
{
    while (MyVulkanManager::loopDrawFlag)
    {
        //更新每一个粒子系统
        for (auto oneP : particleVector)
        {
            oneP->update();
        }
        //获取资源锁
...
void MyVulkanManager::flushTexToDesSet()
{
    ...
    for(int i=0;i<TextureManager::texNames.size();i++)
    {
        sqsParticle->writes[0].dstSet = sqsParticle->descSet[i];
        sqsParticle->writes[1].dstSet = sqsParticle->descSet[i];
        sqsParticle->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsParticle->writes, 0, NULL);

        sqsParticleTwo->writes[0].dstSet = sqsParticleTwo->descSet[i];
        sqsParticleTwo->writes[1].dstSet = sqsParticleTwo->descSet[i];
        sqsParticleTwo->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsParticleTwo->writes, 0, NULL);

        sqsParticleThree->writes[0].dstSet = sqsParticleThree->descSet[i];
        sqsParticleThree->writes[1].dstSet = sqsParticleThree->descSet[i];
        sqsParticleThree->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsParticleThree->writes, 0, NULL);

        sqsParticleFour->writes[0].dstSet = sqsParticleFour->descSet[i];
        sqsParticleFour->writes[1].dstSet = sqsParticleFour->descSet[i];
        sqsParticleFour->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsParticleFour->writes, 0, NULL);
    }
}
void MyVulkanManager::drawObject()
{
    //开辟一个线程 负责不断更新粒子系统
    thread t1(&MyVulkanManager::updateParticleSystem);
    t1.detach();
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        LYobjDiban->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(("texture/wall.bntex"))]));
        MatrixState3D::popMatrix();
        const float  penDistance=7.0f;
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(penDistance,0,penDistance);
        LYobjB->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(("texture/brazier.bntex"))]));
        MatrixState3D::popMatrix();
        ...
        //获取资源锁
        myLock.lock();
        //将最新的粒子系统数据从过渡区送入绘制类对象
        for (auto oneP : particleVector)
        {
            //更新Buffer中的数据
            LYobjParticle[oneP->id]->updateVertexData(oneP->vdata, oneP->dataByteCount, device);
        }
        //释放资源锁
        myLock.unlock();
        calculateDistance();
        for (auto oneP : particleVector) {
            MatrixState3D::pushMatrix();
            MatrixState3D::translate(oneP->positionX, 3.5f, oneP->positionZ);
            MatrixState3D::rotate(oneP->yAngle, 0, 1, 0);
            switch (oneP->id) {
                case 0:LYobjParticle[oneP->id]->drawSelf(cmdBuffer, sqsParticle->pipelineLayout, sqsParticle->pipeline, &(sqsParticle->descSet[TextureManager::getVkDescriptorSetIndex(("texture/fire.bntex"))]), oneP->id);
                    break;
                case 1:LYobjParticle[oneP->id]->drawSelf(cmdBuffer, sqsParticleTwo->pipelineLayout, sqsParticleTwo->pipeline, &(sqsParticleTwo->descSet[TextureManager::getVkDescriptorSetIndex(("texture/fire.bntex"))]), oneP->id);
                    break;
                case 2:LYobjParticle[oneP->id]->drawSelf(cmdBuffer, sqsParticleThree->pipelineLayout, sqsParticleThree->pipeline, &(sqsParticleThree->descSet[TextureManager::getVkDescriptorSetIndex(("texture/fire.bntex"))]), oneP->id);
                    break;
                case 3:LYobjParticle[oneP->id]->drawSelf(cmdBuffer, sqsParticleFour->pipelineLayout, sqsParticleFour->pipeline, &(sqsParticleFour->descSet[TextureManager::getVkDescriptorSetIndex(("texture/fire.bntex"))]), oneP->id);
                    break;
            }
            MatrixState3D::popMatrix();
        }
...
void MyVulkanManager::initPipeline()
{
    ...
    sqsParticle=new ShaderQueueSuit_CommonTexParticle(&device,renderPass,memoryroperties);
    sqsParticleTwo=new ShaderQueueSuit_CommonTexParticleTwo(&device,renderPass,memoryroperties);
    sqsParticleThree=new ShaderQueueSuit_CommonTexParticleThree(&device,renderPass,memoryroperties);
    sqsParticleFour=new ShaderQueueSuit_CommonTexParticleFour(&device,renderPass,memoryroperties);
}
void MyVulkanManager::destroyPipeline()
{
    delete sqsCT;
    delete sqsParticle;
    delete sqsParticleTwo;
    delete sqsParticleThree;
    delete sqsParticleFour;
}
void MyVulkanManager::calculateDistance(){
    sort(particleVector.begin(),particleVector.end(),compare);
}
bool MyVulkanManager::compare(ParticleSystem *pa,ParticleSystem *another){
    float xs=pa->positionX-CameraUtil::cx;
    float zs=pa->positionZ-CameraUtil::cz;
    float xo=another->positionX-CameraUtil::cx;
    float zo=another->positionZ-CameraUtil::cz;
