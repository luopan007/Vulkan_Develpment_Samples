...
DrawableObjectCommon * MyVulkanManager::colorRectR;
...
void MyVulkanManager::createDrawableObject()
{
    ...
    colorRectG=new DrawableObjectCommon(ColorRect::vdataG,ColorRect::dataByteCount,ColorRect::vCount,device,memoryroperties);
    colorRectR=new DrawableObjectCommon(ColorRect::vdataR,ColorRect::dataByteCount,ColorRect::vCount,device,memoryroperties);
}
...
void MyVulkanManager::initMatrix(){
    ...
    //MatrixState3D::setProjectFrustum(-ratio*225.0f, ratio*225.0f, -225.0f, 225.0f, 900.0f, 10000.0f);//效果更明显
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        switch (depthOffsetFlag){ //根据索引设置黄色矩形深度偏移参数
            case 0:break;
            case 1: vk::vkCmdSetDepthBias(cmdBuffer,-1.0,-3.0,-2.0); break; //黄色矩形深度值减小
            case 2: vk::vkCmdSetDepthBias(cmdBuffer,1.0,3.0,2.0); break;} //黄色矩形深度值增大
        MatrixState3D::pushMatrix(); //保护现场
