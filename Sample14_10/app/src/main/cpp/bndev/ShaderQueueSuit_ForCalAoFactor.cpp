...
void ShaderQueueSuit_ForCalAoFactor::create_uniform_buffer(VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
    bufferByteCount=sizeof(float)*(4+16);
...
void ShaderQueueSuit_ForCalAoFactor::destroy_uniform_buffer(VkDevice& device)
{
    vk::vkDestroyBuffer(device, uniformBuf, NULL);
    vk::vkFreeMemory(device, memUniformBuf, NULL);
}
void ShaderQueueSuit_ForCalAoFactor::create_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_ForCalAoFactor::destroy_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_ForCalAoFactor::init_descriptor_set(VkDevice& device)
{
...
void ShaderQueueSuit_ForCalAoFactor::create_shader(VkDevice& device)
{
    ...
    SpvData spvVertData=FileUtil::loadSPV("shader/calAoFactor.vert.spv");
    ...
    SpvData spvFragData=FileUtil::loadSPV("shader/calAoFactor.frag.spv");
    moduleCreateInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
    moduleCreateInfo.pNext = NULL;
    moduleCreateInfo.flags = 0;
...
void ShaderQueueSuit_ForCalAoFactor::destroy_shader(VkDevice& device)
{
    vk::vkDestroyShaderModule(device, shaderStages[0].module, NULL);
    vk::vkDestroyShaderModule(device, shaderStages[1].module, NULL);
}
void ShaderQueueSuit_ForCalAoFactor::initVertexAttributeInfo()
{
...
void ShaderQueueSuit_ForCalAoFactor::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
...
void ShaderQueueSuit_ForCalAoFactor::destroy_pipe_line(VkDevice& device)
{
    vk::vkDestroyPipeline(device, pipeline, NULL);
    vk::vkDestroyPipelineCache(device, pipelineCache, NULL);
}
ShaderQueueSuit_ForCalAoFactor::ShaderQueueSuit_ForCalAoFactor(VkDevice* deviceIn,VkRenderPass& renderPass,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ShaderQueueSuit_ForCalAoFactor::~ShaderQueueSuit_ForCalAoFactor()
{
