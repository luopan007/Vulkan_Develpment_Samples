...
std::vector<const char *> MyVulkanManager::exceptedLayerNames;
BNDeviceLayerAndExtension MyVulkanManager::bdlae;
...
void MyVulkanManager::init_vulkan_instance() {
    ...
    if (exceptedLayerNames.size() > 0)//若能够启动的验证Layer数量大于0
    {
        BNValidateUtil::createDebugReportCallbackSelf(instance);//创建调试报告回调
    }
}
...
void MyVulkanManager::enumerate_vulkan_phy_devices() {
    ...
    LOGE("[Vulkan硬件设备数量丿%d个]", gpuCount);
...
void MyVulkanManager::create_vulkan_devices() {
    ...
    for (unsigned int i = 0; i < queueFamilyCount; i++) {
        if (queueFamilyprops[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) {
            ...
            LOGE("[支持GRAPHICS工作的一个队列家族的索引丿%d]", i);
    ...
    std::vector<std::string *> needsDeviceExtensions = BNValidateUtil::getLayerDeviceExtension(
            gpus[0], bdlae.layerNames); //获取验证Layer所需设备扩展
...
void MyVulkanManager::create_vulkan_swapChain() {
    ...
    if (fpCreateAndroidSurfaceKHR == NULL) {
        LOGE("找不到vkCreateAndroidSurfaceKHR扩展函数＿");
    }
    ...
    for (uint32_t i = 0; i < queueFamilyCount; i++) {
        ...
        LOGE("队列家族索引=%d %s显示", i, (pSupportsPresent[i] == 1 ? "支持" : "不支挿"));
    }
    ...
    if (queueGraphicsFamilyIndex == UINT32_MAX || queuePresentFamilyIndex == UINT32_MAX) {
        LOGE("没有找到支持Graphis（图形）或Present（显示）工作的队列家旿");
        assert(false);
    }
    ...
    result = vk::vkGetPhysicalDeviceSurfacePresentModesKHR(gpus[0], surface, &presentModeCount,
                                                           NULL);
    assert(result == VK_SUCCESS);
    LOGE("显示模式数量丿%d", presentModeCount);
    presentModes.resize(presentModeCount);
    result = vk::vkGetPhysicalDeviceSurfacePresentModesKHR(gpus[0], surface, &presentModeCount,
                                                           presentModes.data());
    for (int i = 0; i < presentModeCount; i++) {
        LOGE("显示模式[%d]编号丿%d", i, presentModes[i]);
    }
    ...
    if (surfCapabilities.currentExtent.width == 0xFFFFFFFF) {
        ...
        LOGE("使用自己设置皿 宽度 %d 高度 %d", swapchainExtent.width, swapchainExtent.height);
    } else {
    ...
    assert(result == VK_SUCCESS);
    LOGE("[SwapChain中的Image数量丿%d]", swapchainImageCount);
...
void MyVulkanManager::destroy_vulkan_swapChain() {
    for (uint32_t i = 0; i < swapchainImageCount; i++) {
        ...
        LOGE("[锿毁SwapChain ImageView %d 成功]", i);
    }
...
void MyVulkanManager::create_vulkan_DepthBuffer() {
    ...
    if (depthFormatProps.linearTilingFeatures & VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) {
        ...
        LOGE("tiling为VK_IMAGE_TILING_LINEAR＿");
    } else if (depthFormatProps.optimalTilingFeatures &
               VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) {
        image_info.tiling = VK_IMAGE_TILING_OPTIMAL;
        LOGE("tiling为VK_IMAGE_TILING_OPTIMAL＿");
    } else {
        LOGE("不支持VK_FORMAT_D16_UNORM＿");
    }
    ...
    assert(flag);
    LOGE("确定内存类型成功 类型索引丿%d", mem_alloc.memoryTypeIndex);
...
void MyVulkanManager::createDrawableObject() {
    ...
    triForDraw = new DrawableObjectCommonLight(TriangleData::vdata, TriangleData::dataByteCount,
                                               TriangleData::vCount, device, memoryroperties);
}
