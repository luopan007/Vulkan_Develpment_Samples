...
ShaderQueueSuit_DoubleTex* MyVulkanManager::sqsDT;
...
{
    LandData* land = FileUtil::loadHdtData("hdt/land.bnhdt"); 
    landForDraw = new TexDrawableObject(land->vData, land->vCount * 5 * 4, land->vCount, device, memoryroperties);
...
{
    CameraUtil::calCamera(-10, 0);
    MatrixState3D::setInitStack();
    float ratio = (float)screenWidth / (float)screenHeight;
    MatrixState3D::setProjectFrustum(-ratio, ratio, -1, 1, 1.5f, 4000);
}
void MyVulkanManager::flushUniformBuffer()
{
    float fragmentUniformData[3]=
            {
                    0.9,120,90
            };
    uint8_t *pData;
    VkResult result = vk::vkMapMemory(device, sqsDT->memUniformBuf, 0, sqsDT->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, fragmentUniformData, sqsDT->bufferByteCount);
    vk::vkUnmapMemory(device,sqsDT->memUniformBuf);
}
void MyVulkanManager::flushTexToDesSet()
{
    for (int i = 0; i<TextureManager::texNames.size() / 2; i++)
    {
        sqsDT->writes[0].dstSet = sqsDT->descSet[i];
        sqsDT->writes[1].dstSet = sqsDT->descSet[i];
        sqsDT->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i * 2 + 0]]);
        sqsDT->writes[2].dstSet = sqsDT->descSet[i];
        sqsDT->writes[2].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i * 2 + 1]]);
        vk::vkUpdateDescriptorSets(device, 3, sqsDT->writes, 0, NULL);
    }
    for(int i=0;i<TextureManager::texNameSingle.size();i++)
    {
        sqsCT->writes[0].dstSet = sqsCT->descSet[i];
        sqsCT->writes[1].dstSet = sqsCT->descSet[i];
        sqsCT->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNameSingle[i]]);
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        landForDraw->drawSelf(cmdBuffer, sqsDT->pipelineLayout, sqsDT->pipeline, &(sqsDT->descSet[0]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        skyForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline, &(sqsCT->descSet[0]));
        MatrixState3D::popMatrix();
...
{
    sqsDT = new ShaderQueueSuit_DoubleTex(&device, renderPass, memoryroperties);
    LOGE("创建 sqsDT 成功");
