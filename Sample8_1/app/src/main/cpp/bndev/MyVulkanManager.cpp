...
TexDrawableObject* MyVulkanManager::texRect;
float MyVulkanManager::rectX = 0;
float MyVulkanManager::rectY = 0;
int MyVulkanManager::rectState = 0;
float MyVulkanManager::moveSpan = 0.1;
bool MyVulkanManager::moveFlag;
int MyVulkanManager::stop = 0;
int MyVulkanManager::up = 1;
int MyVulkanManager::down = 2;
int MyVulkanManager::left = 3;
int MyVulkanManager::right = 4;
ShaderQueueSuit_Common* MyVulkanManager::sqsCTL;
ObjObject*  MyVulkanManager::CHForDraw;
ObjObject*  MyVulkanManager::CFTForDraw;
ObjObject*  MyVulkanManager::QTForDraw;
ObjObject*  MyVulkanManager::YHForDraw;
...
{
    RectData::genVertexData();
    texRect = new TexDrawableObject(RectData::vdata,RectData::dataByteCount,RectData::vCount,device,memoryroperties);
    CHForDraw = LoadUtil::loadFromFile("model/ch.obj",device, memoryroperties);
    CFTForDraw = LoadUtil::loadFromFile("model/cft.obj",device, memoryroperties);
    QTForDraw = LoadUtil::loadFromFile("model/qt.obj",device, memoryroperties);
    YHForDraw = LoadUtil::loadFromFile("model/yh.obj",device, memoryroperties);
...
{
    MatrixState3D::setCamera(0, 0, 50, 0, 0, 0, 0, 1, 0);
    ...
    LightManager::setlightAmbient(0.4f,0.4f,0.4f,1.0f);
...
void MyVulkanManager::flushTexToDesSet()
{
    ...
    sqsCTL->writes[0].dstSet = sqsCTL->descSet[0];
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        if(moveFlag)
        {
            if(rectState == up) 
            {
                rectY += moveSpan;
                LOGE("moveFlag up");
            }else if(rectState == down) 
            {
                rectY -= moveSpan;
                LOGE("moveFlag down");
            }else if(rectState == left) 
            {
                rectX -= moveSpan;
                LOGE("moveFlag left");
            }else if(rectState == right)    
            {
                rectX += moveSpan;
                LOGE("moveFlag right");
            }
        ...
        MatrixState3D::pushMatrix();        
        MatrixState3D::rotate(-25, 1, 0, 0); 
        PMForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, &(sqsCTL->descSet[0]));
        MatrixState3D::pushMatrix();        
        MatrixState3D::scale(1.5, 1.5, 1.5);    
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(-10, 0, 0);
        CFTForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, &(sqsCTL->descSet[0]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(10, 0, 0);
        QTForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, &(sqsCTL->descSet[0]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(0, 0, -10);
        YHForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, &(sqsCTL->descSet[0]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(0, 0, 10);
        CHForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline, &(sqsCTL->descSet[0]));
        MatrixState3D::popMatrix();
        MatrixState3D::popMatrix();         
        MatrixState3D::popMatrix();         
        MatrixState3D::pushMatrix();
        MatrixState3D::scale(0.85, 0.85, 0.85);
        MatrixState3D::translate(rectX, rectY, 30);
        texRect->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline,
                         &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/lgq.bntex")]));
        MatrixState3D::popMatrix();
        MatrixState3D::popMatrix();         
...
void MyVulkanManager::initPipeline()
{
    ...
    sqsCTL=new ShaderQueueSuit_Common(&device,renderPass,memoryroperties);
    LOGE("管线创建成功！");
}
