...
float MyVulkanManager::angleSpan = 2.0f;
float MyVulkanManager::yStart = -1.5f;
float MyVulkanManager::ySpan = 3.0f;
float MyVulkanManager::angleStep = 2.0f;
...
void MyVulkanManager::createDrawableObject()
{
    ...
    texTri = new TexDrawableObject(ObjectData::vdata, ObjectData::dataByteCount, ObjectData::vCount, device, memoryroperties);
}
...
{
    MatrixState3D::setCamera(0,0,4,0,0,0,0,1,0);
...
void MyVulkanManager::flushUniformBuffer()
{
    ...
    if(toDegrees(angleSpan)>90)
    {
        angleStep=-2.0f;
    } else if(toDegrees(angleSpan)<-90)
    {
        angleStep=2.0f;
    }
    float* uniformData=new float[3]{angleSpan,yStart,ySpan};
    uint8_t *pData;
