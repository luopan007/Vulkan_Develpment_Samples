...
ObjObject*  MyVulkanManager::homeObjForDraw;
ObjObject*  MyVulkanManager::bedObjForDraw;
ObjObject*  MyVulkanManager::dengObjForDraw;
ObjObject*  MyVulkanManager::book0ObjForDraw;
ObjObject*  MyVulkanManager::book1ObjForDraw;
ObjObject*  MyVulkanManager::chairObjForDraw;
ObjObject*  MyVulkanManager::borderObjForDraw;
TextureRect* MyVulkanManager::tr;
...
{
    homeObjForDraw = LoadUtil::loadFromFile("model/skybox.obj", device, memoryroperties);
    bedObjForDraw = LoadUtil::loadFromFile("model/chuang.obj", device, memoryroperties);
    dengObjForDraw = LoadUtil::loadFromFile("model/deng.obj", device, memoryroperties);
    book0ObjForDraw = LoadUtil::loadFromFile("model/shu0.obj", device, memoryroperties);
    book1ObjForDraw = LoadUtil::loadFromFile("model/shu1.obj", device, memoryroperties);
    chairObjForDraw = LoadUtil::loadFromFile("model/yizi0.obj", device, memoryroperties);
    borderObjForDraw = LoadUtil::loadFromFile("model/mirror.obj", device, memoryroperties);
    /*创建镜面*/
    float UNIT_SIZE = 15.0f;
    float* vdataIn = new float[18]
            {
                    -UNIT_SIZE,-UNIT_SIZE,0,	UNIT_SIZE,UNIT_SIZE,0,		-UNIT_SIZE,UNIT_SIZE,0,
                    -UNIT_SIZE,-UNIT_SIZE,0,	UNIT_SIZE,-UNIT_SIZE,0,		UNIT_SIZE,UNIT_SIZE,0
            };
    tr = new TextureRect(vdataIn, 6 * 3 * sizeof(float), 6, device, memoryroperties);
}
void MyVulkanManager::destroyDrawableObject()
{
    delete homeObjForDraw;
    delete bedObjForDraw;
    delete dengObjForDraw;
    delete book0ObjForDraw;
    delete book1ObjForDraw;
    delete chairObjForDraw;
    delete borderObjForDraw;
    delete tr;
}
...
void MyVulkanManager::initMatrix()
{
    ...
    MatrixState3D::setProjectFrustum(-ratio, ratio, -1, 1, 1.0f, 500);
    LightManager::setLightPosition(0, 80, 7);
...
void MyVulkanManager::flushUniformBufferForToScreen()
{
    ...
    assert(result == VK_SUCCESS);
    memcpy(pData, fragUniformData, sqsM->bufferByteCount);
    vk::vkUnmapMemory(device, sqsM->memUniformBuf);
}
void MyVulkanManager::flushTexToDesSetForToScreen()
{
    sqsM->writes[0].dstSet = sqsM->descSet[0];
    sqsM->writes[1].dstSet = sqsM->descSet[0];
    sqsM->writes[1].pImageInfo = &colorImageInfo;
    vk::vkUpdateDescriptorSets(device, 2, sqsM->writes, 0, NULL);
}
void MyVulkanManager::drawSceneToTex() //将场景镜像绘制到纹理的方法
{
    CameraUtil::flushMirrorCameraToMatrix(); //根据镜像摄像机参数更新摄像机矩阵
    mirrorMatrix = MatrixState3D::getViewProjMatrix(); //获取镜像摄像机投影与观察组合矩阵
    rp_begin.framebuffer = selfTexFramebuffer;//将镜像绘制专用帧缓冲设置为渲染通道的当前帧缓冲
...
void MyVulkanManager::drawSceneToScreen(){ //将场景绘制到屏幕的方法
    CameraUtil::flushMainCameraToMatrix(); //根据主摄像机的参数更新摄像机矩阵
    VkResult result = vk::vkAcquireNextImageKHR(device, swapChain,
                                                UINT64_MAX, imageAcquiredSemaphore, VK_NULL_HANDLE, &currentBuffer);
    rp_begin.framebuffer = framebuffers[currentBuffer]; //指定渲染通道使用的帧缓冲
    vk::vkResetCommandBuffer(cmdBuffer, 0); //重置命令缓冲到初始状态
    ...
    MatrixState3D::translate(2.2f, 12.0f, CameraUtil::tz - 2.0f); //执行平移变换
    borderObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,//绘制镜子边框
                               &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/mirror.bntex")]));
    MatrixState3D::popMatrix(); //恢复现场
    MatrixState3D::pushMatrix(); //保护现场
    MatrixState3D::translate(2.0f, 12.0f, CameraUtil::tz - 1.7f); //执行平移
    tr->drawSelf(cmdBuffer, sqsM->pipelineLayout, sqsM->pipeline,&(sqsM->descSet[0]));//绘制镜面
    MatrixState3D::popMatrix(); //恢复现场
    vk::vkCmdEndRenderPass(cmdBuffer);
    result = vk::vkEndCommandBuffer(cmdBuffer);
...
void MyVulkanManager::drawThings()
{
    ...
    MatrixState3D::translate(0, -10, 7);
    homeObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                             &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/skybox.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(40, 0, -12);
    MatrixState3D::rotate(180, 0, 1, 0);
    dengObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                             &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/deng.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(14, -7, -18);
    book1ObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                              &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/shu1.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(22, 0, 8);
    MatrixState3D::rotate(180, 0, 1, 0);
    bedObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                            &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/chuang.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-40, 30, 10);
    book0ObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                              &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/shu0.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-40, 20, 20);
    book0ObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                              &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/shu0.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(-15, 0, -32);
    chairObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                              &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/yizi.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(40, 0, 34);
    MatrixState3D::rotate(180, 0, 1, 0);
    dengObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                             &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/deng.bntex")]));
    MatrixState3D::popMatrix();
