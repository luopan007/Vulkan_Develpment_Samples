...
VkDeviceMemory MyVulkanManager::shotMem;
VkBuffer MyVulkanManager::shotBuffer;
bool MyVulkanManager::shotFlag=false;
int MyVulkanManager::shotBytesCount;
...
void MyVulkanManager::create_vulkan_SelfColorBuffer(){ 
    ...
    vk::vkGetPhysicalDeviceFormatProperties
            (gpus[0], colorFormat, &colorFormatProps);
    if (colorFormatProps.linearTilingFeatures &
            VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT){
        image_info.tiling = VK_IMAGE_TILING_LINEAR;
        LOGE("tiling为VK_IMAGE_TILING_LINEAR！");
    }
    else if (colorFormatProps.optimalTilingFeatures &
            VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT){
        image_info.tiling = VK_IMAGE_TILING_OPTIMAL;
        LOGE("tiling为VK_IMAGE_TILING_OPTIMAL！");
    }
    else{LOGE("不支持VK_FORMAT_R8G8B8A8_UNORM！");}
    image_info.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
    image_info.pNext = NULL;
    image_info.imageType = VK_IMAGE_TYPE_2D;
    image_info.extent.width = screenWidth;
    ...
    mem_alloc.allocationSize = mem_reqs.size;
    VkFlags requirements_mask = 0;
    bool flag = memoryTypeFromProperties
    ...
    buf_info.pNext = NULL;
    buf_info.size = mem_reqs.size;
    buf_info.queueFamilyIndexCount = 0;
    buf_info.pQueueFamilyIndices = NULL;
    buf_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    buf_info.flags = 0;
    result = vk::vkCreateBuffer(device, &buf_info, NULL, &shotBuffer);
    assert(result == VK_SUCCESS);
    VkMemoryRequirements mem_reqsShot;
    vk::vkGetBufferMemoryRequirements(device, shotBuffer, &mem_reqsShot);
    assert(mem_reqs.size <= mem_reqsShot.size);
    shotBytesCount = mem_reqsShot.size;
    VkMemoryAllocateInfo alloc_info = {};
    alloc_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    alloc_info.pNext = NULL;
    alloc_info.memoryTypeIndex = 0;
    alloc_info.allocationSize = mem_reqsShot.size;
    requirements_mask = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | //设备内存掩码
                        VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
    flag = memoryTypeFromProperties(memoryroperties, mem_reqsShot.memoryTypeBits, requirements_mask, &alloc_info.memoryTypeIndex);
    if (flag)
    {
        printf("\nShot确定内存类型成功 类型索引为%d\n", alloc_info.memoryTypeIndex);
    }
    else
    {
        printf("\nShot确定内存类型失败!\n");
    }
    result = vk::vkAllocateMemory(device, &alloc_info, NULL, &shotMem);
    assert(result == VK_SUCCESS);
    result = vk::vkBindBufferMemory(device, shotBuffer, shotMem, 0);
    assert(result == VK_SUCCESS);
}
...
void MyVulkanManager::create_frame_buffer() {
    ...
    VkResult result = vk::vkCreateFramebuffer(device, 
                                              &fb_info, NULL, &selfTexFramebuffer);
    assert(result == VK_SUCCESS); 
}
...
void MyVulkanManager::initMatrix()
{
    ...
    LightManager::setLightPosition(0, 100, 150);
...
{
    float fragmentUniformData[1] = { 0.9f };
    uint8_t *pData;
...
void MyVulkanManager::drawSceneToTex() { 
    ...
    objObject->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, 
                        &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex(
                                "texture/SpaceStation512.bntex")]));
...
void MyVulkanManager::drawObject()
{
    ...
    while (MyVulkanManager::loopDrawFlag) {
        ...
        if(shotFlag){ //执行截屏操作
            ...
            vk::vkResetCommandBuffer(cmdBuffer, 0);
            VkResult result = vk::vkBeginCommandBuffer(cmdBuffer, &cmd_buf_info);
            vk::vkCmdCopyImageToBuffer(cmdBuffer,swapchainImages[currentBuffer],//执行数据拷贝
                                       VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
                                       shotBuffer,1,&bufferCopyRegion);
            result = vk::vkEndCommandBuffer(cmdBuffer);
            submit_info[0].waitSemaphoreCount = 0;
            submit_info[0].pWaitSemaphores = NULL;
            result = vk::vkQueueSubmit(queueGraphics, 1, submit_info, taskFinishFence);
            ...
            result = vk::vkMapMemory(device, shotMem, 0, shotBytesCount, 0, (void **)&pData);
            memcpy(shotData,pData,shotBytesCount); //将图像数据从显存送入内存
            vk::vkUnmapMemory(device, shotMem); //解除内存映射
            FileUtil::storeData("/storage/emulated/0/shot.bmp", //将数据写入到手机存储(闪存)中
                                (const char *)shotData, shotBytesCount, screenWidth,screenHeight);
            delete[] shotData; //释放图像数据内存
            LOGE("Shot! bc %d w %d h %d\n", shotBytesCount,screenWidth,screenHeight);//打印信息
