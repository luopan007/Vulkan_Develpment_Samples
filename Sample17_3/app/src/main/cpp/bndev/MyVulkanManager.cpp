...
ShaderQueueSuit_CommonQiu* MyVulkanManager::sqsQiu;
ShaderQueueSuit_CommonQiuTwo* MyVulkanManager::sqsQiuTwo;
ShaderQueueSuit_CommonQiuThree* MyVulkanManager::sqsQiuThree;
ObjObject*  MyVulkanManager::objObject;
ObjObjectQiu* MyVulkanManager::objQiu;
ObjObjectQiu* MyVulkanManager::objQiuTwo;
ObjObjectQiu* MyVulkanManager::objQiuThree;
vector<Planet*> MyVulkanManager::planetVector;
...
{
    objObject=LoadUtil::loadFromFile("model/qiu.obj",device,memoryroperties);
    objQiu=LoadUtil::loadFromFileQiu("model/stone.obj",device,memoryroperties);
    objQiuTwo=LoadUtil::loadFromFileQiu("model/stone2.obj",device,memoryroperties);
    objQiuThree=LoadUtil::loadFromFileQiu("model/stone3.obj",device,memoryroperties);
    planet1=new Planet(1,false);planet2=new Planet(2,false);planet3=new Planet(3,false);
    planet4=new Planet(4,true);planet5=new Planet(5,true);planet6=new Planet(6,true);
    planet1->initInstanceData();planet2->initInstanceData();planet3->initInstanceData();
    planet4->initInstanceData();planet5->initInstanceData();planet6->initInstanceData();
    planetVector.push_back(planet1);
    planetVector.push_back(planet2);planetVector.push_back(planet3);
    planetVector.push_back(planet4);planetVector.push_back(planet5);planetVector.push_back(planet6);
}
...
void MyVulkanManager::initMatrix()
{
    ...
    MatrixState3D::setCamera(-20,-40,80,0,0,0,1,0,0);
    LightManager::setLightPosition(-20,-20,30);
...
void MyVulkanManager::flushUniformBuffer()
{
    ...
    result = vk::vkMapMemory(device, sqsQiu->memUniformBuf,0, sqsQiu->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsQiu->bufferByteCount);
    vk::vkUnmapMemory(device,sqsQiu->memUniformBuf);
...
void MyVulkanManager::flushTexToDesSet()
{
    ...
    for(int i=0;i<TextureManager::texNames.size();i++)
    {
        sqsQiu->writes[0].dstSet = sqsQiu->descSet[i];
        sqsQiu->writes[1].dstSet = sqsQiu->descSet[i];
        sqsQiu->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i]]);
        vk::vkUpdateDescriptorSets(device, 2, sqsQiu->writes, 0, NULL);
    }
//    for(int i=0;i<TextureManager::texNames.size();i++)
//    {
//        sqsQiuTwo->writes[0].dstSet = sqsQiuTwo->descSet[i];
//        sqsQiuTwo->writes[1].dstSet = sqsQiuTwo->descSet[i];
//        sqsQiuTwo->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i]]);
//        vk::vkUpdateDescriptorSets(device, 2, sqsQiuTwo->writes, 0, NULL);
//    }
//    for(int i=0;i<TextureManager::texNames.size();i++)
//    {
//        sqsQiuThree->writes[0].dstSet = sqsQiuThree->descSet[i];
//        sqsQiuThree->writes[1].dstSet = sqsQiuThree->descSet[i];
//        sqsQiuThree->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i]]);
//        vk::vkUpdateDescriptorSets(device, 2, sqsQiuThree->writes, 0, NULL);
//    }
}
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        objObject->drawSelf(cmdBuffer,sqsCL->pipelineLayout,sqsCL->pipeline,
                  &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/qiu.bntex")]));
        MatrixState3D::popMatrix();

        PlanetConstant::perAngle+=0.8f;
        for(auto planet:planetVector){
            planet->update();
            for(int i=0;i<planet->paiNumber;i++){
                MatrixState3D::pushMatrix();
                MatrixState3D::translate(planet->attributeQiu[i*4+2],0,planet->attributeQiu[i*4+3]);
                MatrixState3D::rotate(PlanetConstant::perAngle,1,0,0);
                switch (planet->id){
                    case 1:objQiu->drawSelf(cmdBuffer,sqsQiu->pipelineLayout,sqsQiu->pipeline,
                                            &(sqsQiu->descSet[TextureManager::getVkDescriptorSetIndex("texture/stone.bntex")]));
                        break;
                    case 2:objQiu->drawSelf(cmdBuffer,sqsQiu->pipelineLayout,sqsQiu->pipeline,
                                            &(sqsQiu->descSet[TextureManager::getVkDescriptorSetIndex("texture/stone2.bntex")]));
                        break;
                    case 3:objQiu->drawSelf(cmdBuffer,sqsQiu->pipelineLayout,sqsQiu->pipeline,
                                            &(sqsQiu->descSet[TextureManager::getVkDescriptorSetIndex("texture/stone3.bntex")]));
                        break;
                    case 4:objQiu->drawSelf(cmdBuffer,sqsQiu->pipelineLayout,sqsQiu->pipeline,
                                            &(sqsQiu->descSet[TextureManager::getVkDescriptorSetIndex("texture/stone.bntex")]));
                        break;
                    case 5:objQiu->drawSelf(cmdBuffer,sqsQiu->pipelineLayout,sqsQiu->pipeline,
                                            &(sqsQiu->descSet[TextureManager::getVkDescriptorSetIndex("texture/stone2.bntex")]));
                        break;
                    case 6:objQiu->drawSelf(cmdBuffer,sqsQiu->pipelineLayout,sqsQiu->pipeline,
                                            &(sqsQiu->descSet[TextureManager::getVkDescriptorSetIndex("texture/stone3.bntex")]));
                        break;
                }
                MatrixState3D::popMatrix();
...
void MyVulkanManager::initPipeline()
{
    ...
    sqsQiu=new ShaderQueueSuit_CommonQiu(&device,renderPass,memoryroperties);
}
