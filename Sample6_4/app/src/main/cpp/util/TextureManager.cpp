...
std::vector<std::string> TextureManager::texNames={"texture/32Nearest.bntex","texture/32Linear.bntex","texture/256Nearest.bntex","texture/256Linear.bntex"};
...
void TextureManager::initSampler(VkDevice& device, VkPhysicalDevice& gpu)
{
    ...
    for(int i=0;i<SAMPLER_COUNT;i++){//循环设置不同的采样方式
        ...
        VkResult result = vk::vkCreateSampler(device, &samplerCreateInfo, NULL, &samplerTexture);//创建采样器
...
void TextureManager::initTextures(VkDevice& device,VkPhysicalDevice& gpu,VkPhysicalDeviceMemoryProperties& memoryroperties,VkCommandBuffer& cmdBuffer, VkQueue& queueGraphics){			//加载所有纹理的方法
    ...
    for (int i = 0; i<texNames.size(); i++){//遍历纹理文件名称列表
        ...
        init_SPEC_2D_Textures(texNames[i], device, gpu, memoryroperties, //加载2D纹理
