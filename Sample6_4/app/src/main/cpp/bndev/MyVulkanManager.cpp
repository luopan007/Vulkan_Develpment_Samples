...
int MyVulkanManager::triangleCount=0;
...
void MyVulkanManager::createDrawableObject()
{
    ...
    texTri=new TexDrawableObject(vdataIn,30*4,6,device, memoryroperties);
    ...
    texTri1=new TexDrawableObject(vdataIn1,30*4,6,device, memoryroperties);
}
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        MatrixState3D::translate(0,10,0); //沿 y 轴移动
        MatrixState3D::rotate(-30,0,0,1); //绕 z 轴旋转
        if(smallType==0){ //若 smallType 为 0（采用最近点采样）
            texTri->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, //绘制小纹理矩形
                             &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/256Nearest.bntex")]));
        }else{ //若 smallType 为 1（采用线性采样）
            texTri->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, //绘制小纹理矩形
                             &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/256Linear.bntex")]));
        }
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(0,-5,0); //沿 y 轴移动
        MatrixState3D::rotate(-30,0,0,1); //绕 z 轴旋转
        if(bigType==0){ //若 bigType 为 0（采用最近点采样）
            texTri1->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, //绘制大纹理矩形
                              &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/32Nearest.bntex")]));
        }else{ //若 bigType 为 1（采用线性采样）
            texTri1->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, //绘制大纹理矩形
                              &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/32Linear.bntex")]));
        }
        MatrixState3D::popMatrix(); //恢复现场
