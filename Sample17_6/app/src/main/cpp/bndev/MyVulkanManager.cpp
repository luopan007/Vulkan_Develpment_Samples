...
void MyVulkanManager::flushUniformBufferForBD()
{
    ...
    {      
            50,150,32,0.8,(float)qsj1,
            10,40,24,1,(float)qsj2,
            200,200,60,2,(float)qsj3,
            0
    };
...
void MyVulkanManager::flushTexToDesSetForDraw()
{
    ...
    for(int i=0;i<TextureManager::texNames.size();i++)
    {
        ...
        sqsSTL->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i]]);
...
void MyVulkanManager::flushTexToDesSetForBD(){ //更新计算波动描述集的方法
    sqsBD->writes[0].dstSet = sqsBD->descSet[0]; //更新描述集对应的写入属性0
    sqsBD->writes[1].dstSet = sqsBD->descSet[0]; //更新描述集对应的写入属性1
    sqsBD->writes[2].dstSet = sqsBD->descSet[0]; //更新描述集对应的写入属性2
    sqsBD->writes[0].pBufferInfo=&(waterForDraw->vertexDataBufferInfo);//绑定原始顶点数据缓冲
    sqsBD->writes[1].pBufferInfo=&(waterForDraw->vertexDataBufferInfoCompute);//绑定计算用数据缓冲
    vk::vkUpdateDescriptorSets(device, 3, sqsBD->writes, 0, NULL); //更新描述集
}
