...
VkFramebuffer MyVulkanManager::selfTexFramebuffer[2];
ShaderQueueSuit_Common* MyVulkanManager::sqsCL;
ShaderQueueSuit_SSZ* MyVulkanManager::sqsCLSSZ;
ShaderQueueSuit_Dashboard2D* MyVulkanManager::sqsD2D;
CommonObject*  MyVulkanManager::objObject;
...
void MyVulkanManager::create_vulkan_SelfColorBuffer()
{
    for (int i = 0; i<2; i++)
    {
        create_vulkan_SelfColorBufferSpec(i);
    }
}
void MyVulkanManager::create_vulkan_SelfColorBufferSpec(int index)
{
...
{
    LFTData::getVData();
    objObject=new CommonObject(LFTData::vdata, LFTData::dataByteCount, LFTData::vCount, device, memoryroperties);
...
void MyVulkanManager::initMatrix()
{
    ...
    float scaleView = 0.6;
    MatrixState3D::setProjectFrustum(-ratio*scaleView, ratio*scaleView, -1 * scaleView, 1 * scaleView, 4.5f, 1000);
...
void MyVulkanManager::flushUniformBufferForToTex()
{
    ...
    result = vk::vkMapMemory(device, sqsCLSSZ->memUniformBuf, 0, sqsCLSSZ->bufferByteCount, 0, (void **)&pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCLSSZ->bufferByteCount);
    vk::vkUnmapMemory(device, sqsCLSSZ->memUniformBuf);
}
void MyVulkanManager::flushTexToDesSetForToTex()
{
    ...
    sqsCLSSZ->writes[0].dstSet = sqsCLSSZ->descSet[0];
    vk::vkUpdateDescriptorSets(device, 1, sqsCLSSZ->writes, 0, NULL);
}
void MyVulkanManager::flushTexToDesSetForToScreen(){
    ...
    sqsD2D->writes[2].pImageInfo = &TextureManager::texImageInfoList[TextureManager::texNames3D[0]];
    sqsD2D->writes[3].dstSet = sqsD2D->descSet[0];
    sqsD2D->writes[3].pImageInfo = &TextureManager::texImageInfoList[TextureManager::texNames2D[0]];
    vk::vkUpdateDescriptorSets(device, 4, sqsD2D->writes, 0, NULL);
}
void MyVulkanManager::drawSceneToTexSpec(VkFramebuffer framebuffer, VkPipelineLayout& pipelineLayout, VkPipeline& pipeline, VkDescriptorSet* desSetPointer)
{
    ...
    objObject->drawSelf(cmdBuffer, pipelineLayout, pipeline, desSetPointer);
...
void MyVulkanManager::drawSceneToTex()
{
    ...
    drawSceneToTexSpec(selfTexFramebuffer[0], sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]));
    drawSceneToTexSpec(selfTexFramebuffer[1], sqsCLSSZ->pipelineLayout, sqsCLSSZ->pipeline, &(sqsCLSSZ->descSet[0]));
}
...
void MyVulkanManager::drawObject()
{
    ...
    while (MyVulkanManager::loopDrawFlag) {
        FPSUtil::calFPS();
        FPSUtil::before();
        FactorManager::setFactorValue(0.0f,1.0f,0.0f,1.0f,0.0f,1.0f,1.0f,2.0f);/*设置控制因子值（start、end）XYZ，detailDepth，LFactor*/
...
void MyVulkanManager::initPipeline()
{
    ...
    sqsCLSSZ = new ShaderQueueSuit_SSZ(&device, renderPass, memoryroperties);
