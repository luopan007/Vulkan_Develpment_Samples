...
int parseInt(const char *token) {
 ...
 CommonObject* LoadUtil::loadFromFile(const string& vname,VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties){
    CommonObject * lo;
    vector<float> alv;
    ...
    for(int i=0;i<lines.size();i++)
    {
     ...
     lo=new CommonObject(vdataIn,dataByteCount,vCount,device, memoryroperties);
     return lo; 
 }
