...
std::vector<std::string> TextureManager::texNames={"texture/Head_256x256x256.bntex","texture/Head_256x256x256.bn3dtex"};
std::vector<std::string> TextureManager::texNames2D = { "texture/Head_256x256x256.bntex" };
std::vector<std::string> TextureManager::texNames3D = { "texture/Head_256x256x256.bn3dtex" };
...
void TextureManager::init_SPEC_2D_Textures(std::string texName, VkDevice& device, VkPhysicalDevice& gpu, VkPhysicalDeviceMemoryProperties& memoryroperties, VkCommandBuffer& cmdBuffer, VkQueue& queueGraphics,VkFormat format, TexDataObject* ctdo)
{
    ...
    texImageInfo.sampler = samplerList[1];
    texImageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
    texImageInfoList[texName] = texImageInfo;
    delete ctdo;
}
void TextureManager::init_SPEC_3D_Textures(std::string texName, VkDevice& device, VkPhysicalDevice& gpu, VkPhysicalDeviceMemoryProperties& memoryroperties, VkCommandBuffer& cmdBuffer, VkQueue& queueGraphics, VkFormat format, ThreeDTexDataObject* ctdo)
{
    ...
    image_create_info.extent.width = ctdo->width; 
    image_create_info.extent.height = ctdo->height; 
    image_create_info.extent.depth = ctdo->depth; 
    image_create_info.mipLevels = 1;
    image_create_info.arrayLayers = 1;
    image_create_info.samples = VK_SAMPLE_COUNT_1_BIT;
    image_create_info.tiling = VK_IMAGE_TILING_OPTIMAL;
    ...
    bufferCopyRegion.imageExtent.width = ctdo->width; 
    bufferCopyRegion.imageExtent.height = ctdo->height; 
    bufferCopyRegion.imageExtent.depth = ctdo->depth; 
    VkCommandBufferBeginInfo cmd_buf_info = {};
    cmd_buf_info.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
    cmd_buf_info.pNext = NULL;
    cmd_buf_info.flags = 0;
...
void TextureManager::initTextures(VkDevice& device, VkPhysicalDevice& gpu, VkPhysicalDeviceMemoryProperties& memoryroperties, VkCommandBuffer& cmdBuffer, VkQueue& queueGraphics)
{
    ...
    for (int i = 0; i<texNames.size(); i++)
    {
        ...
        else if (sa[1].compare("bn3dtex") == 0)
        {
            ThreeDTexDataObject* ctdo = FileUtil::load3DTexData(texNames[i]);
            init_SPEC_3D_Textures(texName, device, gpu, memoryroperties, cmdBuffer, queueGraphics, VK_FORMAT_R16_UNORM, ctdo);
        }
    }
}
