...
#include <string.h>
#include <mylog.h>
Dashboard2DObject::Dashboard2DObject(float* vdataIn, int dataByteCount, int vCountIn, VkDevice& device, VkPhysicalDeviceMemoryProperties& memoryroperties)
{
	pushConstantData = new float[24];
...
void Dashboard2DObject::drawSelf(VkCommandBuffer& cmd, VkPipelineLayout& pipelineLayout, VkPipeline& pipeline, VkDescriptorSet* desSetPointer)
{
	...
	FactorManager::setFactorToConstant(&pushConstantData[16]);
	vk::vkCmdPushConstants(cmd, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(float) * 16, pushConstantData);
	vk::vkCmdPushConstants(cmd, pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(float) * 16, sizeof(float) * 8, pushConstantData+16);
