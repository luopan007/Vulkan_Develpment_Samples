...
void ShaderQueueSuit_Common::create_uniform_buffer(VkDevice &device,
    ...
    if (flag) { LOGE("确定内存类型成功 类型索引丿%d", alloc_info.memoryTypeIndex); }
    else { LOGE("确定内存类型失败!"); }
...
void ShaderQueueSuit_Common::create_shader(VkDevice &device) {
    SpvData spvVertData = FileUtil::loadSPV("shader/commonTexLight.vert.spv");//加载顶点着色器数据
    SpvData spvFragData = FileUtil::loadSPV("shader/commonTexLight.frag.spv");//加载片元着色器数据
