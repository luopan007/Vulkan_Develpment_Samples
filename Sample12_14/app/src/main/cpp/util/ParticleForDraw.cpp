...
void ParticleForDraw::drawSelf(VkCommandBuffer& cmd,VkPipelineLayout& pipelineLayout,VkPipeline& pipeline,VkDescriptorSet* desSetPointer,int id)
{
    ...
    memcpy(pushConstantData+16, START_COLOR[id], sizeof(float)*4);
    memcpy(pushConstantData+20, END_COLOR[id], sizeof(float)*4);
    memcpy(pushConstantData+24, RADIS+id, sizeof(float)*1);

    vk::vkCmdPushConstants(cmd, pipelineLayout,VK_SHADER_STAGE_FRAGMENT_BIT, 64, sizeof(float)*9,pushConstantData+16);
