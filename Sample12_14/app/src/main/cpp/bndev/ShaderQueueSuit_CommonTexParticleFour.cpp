#include "ShaderQueueSuit_CommonTexParticleFour.h"
#include <assert.h>
...
void ShaderQueueSuit_CommonTexParticleFour::create_uniform_buffer(VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
    bufferByteCount=1*sizeof(float);
...
void ShaderQueueSuit_CommonTexParticleFour::destroy_uniform_buffer(VkDevice& device)
{
    vk::vkDestroyBuffer(device, uniformBuf, NULL);
    vk::vkFreeMemory(device, memUniformBuf, NULL);
}
void ShaderQueueSuit_CommonTexParticleFour::create_pipeline_layout(VkDevice& device)
{
    ...
    push_constant_ranges[1].offset = 64;
    push_constant_ranges[1].size = sizeof(float)*9;
...
void ShaderQueueSuit_CommonTexParticleFour::destroy_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonTexParticleFour::init_descriptor_set(VkDevice& device)
{
    ...
    type_count[1].descriptorCount = TextureManager::texNames.size();
...
void ShaderQueueSuit_CommonTexParticleFour::create_shader(VkDevice& device)
{
    std::string vertStr= FileUtil::loadAssetStr("shader/commonParticle.vert");
    std::string fragStr= FileUtil::loadAssetStr("shader/commonParticle.frag");
...
void ShaderQueueSuit_CommonTexParticleFour::destroy_shader(VkDevice& device)
{
    vk::vkDestroyShaderModule(device, shaderStages[0].module, NULL);
    vk::vkDestroyShaderModule(device, shaderStages[1].module, NULL);
}
void ShaderQueueSuit_CommonTexParticleFour::initVertexAttributeInfo()
{
    ...
    vertexAttribs[1].format = VK_FORMAT_R32_SFLOAT;
    vertexAttribs[1].offset = 8;

    vertexAttribs[2].binding = 0;
    vertexAttribs[2].location = 2;
    vertexAttribs[2].format = VK_FORMAT_R32_SFLOAT;
    vertexAttribs[2].offset = 12;

    vertexAttribs[3].binding = 0;
    vertexAttribs[3].location = 3;
    vertexAttribs[3].format = VK_FORMAT_R32G32_SFLOAT;
    vertexAttribs[3].offset = 16;
}
void ShaderQueueSuit_CommonTexParticleFour::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
    ...
    att_state[0].alphaBlendOp = VK_BLEND_OP_REVERSE_SUBTRACT;//设置Alpha 通道混合方式
    att_state[0].colorBlendOp = VK_BLEND_OP_REVERSE_SUBTRACT;//设置RGB 通道混合方式
    ...
    att_state[0].srcAlphaBlendFactor = VK_BLEND_FACTOR_ONE;//设置源Alpha 混合因子
    att_state[0].dstAlphaBlendFactor = VK_BLEND_FACTOR_ONE;//设置目标Alpha 混合因子
...
void ShaderQueueSuit_CommonTexParticleFour::destroy_pipe_line(VkDevice& device)
{
    vk::vkDestroyPipeline(device, pipeline, NULL);
    vk::vkDestroyPipelineCache(device, pipelineCache, NULL);
}
ShaderQueueSuit_CommonTexParticleFour::ShaderQueueSuit_CommonTexParticleFour(VkDevice* deviceIn,VkRenderPass& renderPass,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ShaderQueueSuit_CommonTexParticleFour::~ShaderQueueSuit_CommonTexParticleFour()
{
