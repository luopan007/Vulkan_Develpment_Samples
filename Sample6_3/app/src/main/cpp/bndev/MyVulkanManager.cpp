...
TexDrawableObject* MyVulkanManager::texTri1;
TexDrawableObject* MyVulkanManager::texTri2;
...
void MyVulkanManager::createDrawableObject(){//创建绘制用物体的方法
    float* vdataIn=new float[30]{ //顶点数据数组
            9,9,0, 4,0,   -9,9,0, 0,0,   -9,-9,0, 0,4,//第1个三角形的数据
            9,9,0, 4,0,   -9,-9,0, 0,4,  9,-9,0, 4,4//第2个三角形的数据
    };
    texTri=new TexDrawableObject(vdataIn,30*4,6,device, memoryroperties); //创建绘制物体1
    vdataIn=new float[30]{ //顶点数据数组
            9,9,0, 4,0,   -9,9,0, 0,0,   -9,-9,0, 0,2,	//第1个三角形的数据
            9,9,0, 4,0,   -9,-9,0, 0,2,  9,-9,0, 4,2//第2个三角形的数据
    };
    texTri1=new TexDrawableObject(vdataIn,30*4,6,device, memoryroperties);//创建绘制物体2
    vdataIn=new float[30]{ //顶点数据数组
            9,9,0, 1,0,   -9,9,0, 0,0,   -9,-9,0, 0,1,	//第1个三角形的数据
            9,9,0, 1,0,   -9,-9,0, 0,1,  9,-9,0, 1,1//第2个三角形的数据
    };
    texTri2=new TexDrawableObject(vdataIn,30*4,6,device, memoryroperties); //创建绘制物体3
}
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        MatrixState3D::rotate(yAngle,0,1,0); //绕 y 轴旋转
        MatrixState3D::rotate(zAngle,0,0,1); //绕 z 轴旋转
        string textureName; //当前纹理名称
        switch(samplerType){ //根据 samplerType 的值分支
            case 0:textureName="texture/robot0.bntex";break; //设置纹理名称 0
            case 1:textureName="texture/robot1.bntex";break; //设置纹理名称 1
            case 2:textureName="texture/robot2.bntex";break; //设置纹理名称 2
            case 3:textureName="texture/robot3.bntex";break; //设置纹理名称 3
        }
        if(texType==0){ //若 texType 为 0(采用 4×4 纹理坐标范围)
            texTri->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, //绘制物体 0
                             &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(textureName)]));
        }else if(texType==1){ //若 texType 为 1(采用 4×2 纹理坐标范围)
            texTri1->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, //绘制物体 1
                              &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(textureName)]));
        }else if(texType==2){ //若 texType 为 2(采用 1×1 纹理坐标范围)
            texTri2->drawSelf(cmdBuffer,sqsCT->pipelineLayout,sqsCT->pipeline, //绘制物体 2
                              &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(textureName)]));
        }
