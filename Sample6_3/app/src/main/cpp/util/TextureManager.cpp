...
std::vector<std::string> TextureManager::texNames={"texture/robot0.bntex","texture/robot1.bntex","texture/robot2.bntex","texture/robot3.bntex"};
...
void TextureManager::initSampler(VkDevice& device, VkPhysicalDevice& gpu){
    ...
    for(int i=0;i<SAMPLER_COUNT;i++){ //循环设置各种拉伸方式
        ...
        VkResult result = vk::vkCreateSampler(device, //创建采样器
