...
VkClearValue MyVulkanManager::clear_values_self[4];
...
void MyVulkanManager::create_render_pass_self() {
    ...
    clear_values_self[3].depthStencil.depth = 1.0f;
    clear_values_self[3].depthStencil.stencil = 0;
    ...
    rp_begin_self.clearValueCount = 4;
...
void MyVulkanManager::initMatrixAndLight() {
    ...
    for (int i = 0;i < LIGHTS_COUNT;i++){ //遍历所有的光源
        LightManager::setLightRange(i,350+80*i); //设置光源的范围
        LightManager::setLightPosition(i, 0, 100, 100); //设置光源的位置
        LightManager::setlightAmbient(i, 0.025f, 0.025f, 0.025f, 0.025f); //设置光源的环境光强度
        LightManager::setlightDiffuse(i, 0.4f, 0.4f, 0.4f, 0.4f); //设置光源的散射光强度
        LightManager::setlightSpecular(i, 0.4f, 0.4f, 0.4f, 0.4f); //设置光源的镜面光强度
    }}
...
void MyVulkanManager::flushUniformBufferForToScreen() {//将摄像机及光照数据送入一致变量缓冲的方法
    float fragmentUniformData[68]= { //一致变量缓冲数据数组
            CameraUtil::camera9Para[0],CameraUtil::camera9Para[1],CameraUtil::camera9Para[2],1.0};
    memcpy(&fragmentUniformData[4],LightManager::lightArray, sizeof(float)*64);//拷贝四个光源的数据
    uint8_t *pData;
...
void MyVulkanManager::create_vulkan_SelfColorBufferSpec(VkFormat colorFormat, int index) {
    VkFormatProperties colorFormatProps; //物理设备支持的颜色格式属性
    ...
    image_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    image_info.flags = 0;
    VkResult result = vk::vkCreateImage(device, &image_info, NULL, &(colorImage[index])); //创建图像
    ...
    view_info.viewType = VK_IMAGE_VIEW_TYPE_2D;
    view_info.flags = 0;
   view_info.image = colorImage[index]; //指定图像
   result = vk::vkCreateImageView(device, &view_info, NULL, &(colorImageView[index])); //创建图像视图
   assert(result == VK_SUCCESS); //检查是否创建成功
}
void MyVulkanManager::create_vulkan_SelfColorBuffer() {
    VkFormat colorFormat[3] = { VK_FORMAT_R8G8B8A8_UNORM, //格式数组
                                VK_FORMAT_R32G32B32A32_SFLOAT,VK_FORMAT_R32G32B32A32_SFLOAT};
    for(int i=0;i<3;i++){ //遍历所有颜色附件
