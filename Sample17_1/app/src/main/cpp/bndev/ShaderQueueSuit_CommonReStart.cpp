...
//
#include "ShaderQueueSuit_CommonReStart.h"
#include <assert.h>
...
void ShaderQueueSuit_CommonReStart::create_uniform_buffer(VkDevice& device,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
void ShaderQueueSuit_CommonReStart::destroy_uniform_buffer(VkDevice& device)
{
...
void ShaderQueueSuit_CommonReStart::create_pipeline_layout(VkDevice& device)
{
    ...
    VkPipelineLayoutCreateInfo pPipelineLayoutCreateInfo = {};	//构建管线布局创建信息
    pPipelineLayoutCreateInfo.sType = 						//指定结构体类型
            VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    pPipelineLayoutCreateInfo.pNext = NULL;					//自定义数据指针
    pPipelineLayoutCreateInfo.pushConstantRangeCount=push_constant_range_count;
    pPipelineLayoutCreateInfo.pPushConstantRanges = push_constant_ranges;   //推送常量范围列表
...
void ShaderQueueSuit_CommonReStart::destroy_pipeline_layout(VkDevice& device)
{
...
void ShaderQueueSuit_CommonReStart::init_descriptor_set(VkDevice& device)
{
...
void ShaderQueueSuit_CommonReStart::create_shader(VkDevice& device)
{
...
void ShaderQueueSuit_CommonReStart::destroy_shader(VkDevice& device)
{
    vk::vkDestroyShaderModule(device, shaderStages[0].module, NULL);
    vk::vkDestroyShaderModule(device, shaderStages[1].module, NULL);
}

void ShaderQueueSuit_CommonReStart::initVertexAttributeInfo()
{
...
void ShaderQueueSuit_CommonReStart::create_pipe_line(VkDevice& device,VkRenderPass& renderPass)
{
    ...
    ia.pNext = NULL;
    ia.flags = 0;
    ia.primitiveRestartEnable = VK_TRUE;//开启图元重启
...
void ShaderQueueSuit_CommonReStart::destroy_pipe_line(VkDevice& device)
{
    vk::vkDestroyPipeline(device, pipeline, NULL);
    vk::vkDestroyPipelineCache(device, pipelineCache, NULL);
}

ShaderQueueSuit_CommonReStart::ShaderQueueSuit_CommonReStart(VkDevice* deviceIn,VkRenderPass& renderPass,VkPhysicalDeviceMemoryProperties& memoryroperties)
{
...
ShaderQueueSuit_CommonReStart::~ShaderQueueSuit_CommonReStart(){
