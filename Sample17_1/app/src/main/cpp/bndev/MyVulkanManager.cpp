...
ShaderQueueSuit_CommonReStart* MyVulkanManager::sqsCRS;
DrawableObjectCommonLight*  MyVulkanManager::open;
DrawableObjectCommonLight*  MyVulkanManager::close;
...
{
    float* vdataIn=new float[48]//顶点坐标数据
            {
                    -45,5,0,  1,1,0,
                    -45,-5,0, 1,1,1,
                    -15,5,0, 1,1,0,
    ...
    uint16_t* dataIndex=new uint16_t[9]//有效使用图元重启时的索引数据
            {
                    0,1,2,3,
                    0xFFFF,//特殊索引值
                    4,5,6,7
            };
    open=new DrawableObjectCommonLight(vdataIn,48*4,8,dataIndex,9*sizeof(uint16_t),9,device,memoryroperties);//创建有效使用图元重启时的绘制物体
    dataIndex=new uint16_t[8]//未有效使用图元重启时的索引数据
            {
                    0,1,2,3,
                    4,5,6,7
            };
    close=new DrawableObjectCommonLight(vdataIn,48*4,8,dataIndex,8*sizeof(uint16_t),8,device,memoryroperties);//创建未有效使用图元重启时的绘制物体
}
...
void MyVulkanManager::flushUniformBuffer()
{
    ...
    VkResult  result = vk::vkMapMemory(device, sqsCRS->memUniformBuf,0, sqsCRS->bufferByteCount, 0, (void **)&pData);
    assert(result==VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCRS->bufferByteCount);
    vk::vkUnmapMemory(device,sqsCRS->memUniformBuf);
...
void MyVulkanManager::drawObject()
{
    ...
    while(MyVulkanManager::loopDrawFlag){
        ...
        MatrixState3D::translate(0,10,0); //沿Y 轴正方向平移50
        close->drawSelf(cmdBuffer,sqsCRS->pipelineLayout, //绘制正十边形
                        sqsCRS->pipeline,&(sqsCRS->descSet[0]),0,8);
        MatrixState3D::popMatrix(); //恢复现场

        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(0,-10,0); //沿Y 轴正方向平移50
        open->drawSelf(cmdBuffer,sqsCRS->pipelineLayout, //绘制正十边形
                       sqsCRS->pipeline,&(sqsCRS->descSet[0]),0,9);
        MatrixState3D::popMatrix(); //恢复现场
...
{
    sqsCRS=new ShaderQueueSuit_CommonReStart(&device,renderPass,memoryroperties);
}
