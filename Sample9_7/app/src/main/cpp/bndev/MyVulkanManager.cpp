...
float MyVulkanManager::UNIT_SIZE = 10;
...
void MyVulkanManager::drawObject() {
    ...
    while(MyVulkanManager::loopDrawFlag)
    {
        ...
        MatrixState3D::translate(0, 0, UNIT_SIZE); //平移变换
        MatrixState3D::rotate(180, 0, 1, 0); //旋转变换
        texForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,//绘制天空盒的前面
                             &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/skycubemap_front.bntex")]));
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(0, 0, -UNIT_SIZE); //平移变换
        texForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,//绘制天空盒的后面
                             &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/skycubemap_back.bntex")]));
        MatrixState3D::popMatrix(); //恢复现场
        MatrixState3D::pushMatrix(); //保护现场
        MatrixState3D::translate(UNIT_SIZE, 0, 0); //平移变换
        MatrixState3D::rotate(90, 0, 1, 0); //旋转变换
        texForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,//绘制天空盒的左面
                             &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/skycubemap_left.bntex")]));
        MatrixState3D::popMatrix(); //恢复现场
        ...
        texForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/skycubemap_right.bntex")]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(0, UNIT_SIZE, 0);
        MatrixState3D::rotate(90, 1, 0, 0);
        texForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/skycubemap_up.bntex")]));
        MatrixState3D::popMatrix();
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(0, -UNIT_SIZE, 0);
        MatrixState3D::rotate(90, 1, 0, 0);
        texForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("texture/skycubemap_down.bntex")]));
        MatrixState3D::popMatrix();
