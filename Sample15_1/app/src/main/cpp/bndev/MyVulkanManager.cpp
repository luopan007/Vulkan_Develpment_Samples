...
VkImage MyVulkanManager::colorImage[5];
VkDeviceMemory MyVulkanManager::memColor[5];
VkImageView MyVulkanManager::colorImageView[5];
VkDescriptorImageInfo MyVulkanManager::colorImageInfo[5];
VkFramebuffer*  MyVulkanManager::framebuffers;
VkFramebuffer MyVulkanManager::selfTexFramebuffer[5];
...
float MyVulkanManager::SPAN=0.5f;
float MyVulkanManager::TIMESPAN=0.75f;
...
void MyVulkanManager::create_vulkan_SelfColorBuffer(){ 
    ...
    for(int i=0;i<5;i++)
    {
        VkResult result = vk::vkCreateImage(device, &image_info, NULL, &(colorImage[i]));
        assert(result == VK_SUCCESS);
        VkMemoryAllocateInfo mem_alloc = {};
        mem_alloc.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
        mem_alloc.pNext = NULL;
        ...
        view_info.viewType = VK_IMAGE_VIEW_TYPE_2D;
        view_info.flags = 0;
        view_info.image = colorImage[i];
        result = vk::vkCreateImageView(device, &view_info, NULL, &(colorImageView[i]));
        assert(result == VK_SUCCESS);
    }
}
...
void MyVulkanManager::create_frame_buffer() {
    ...
    for(int i=0;i<5;i++){
        attachments[0] = colorImageView[i]; 
        VkResult result = vk::vkCreateFramebuffer(device,&fb_info, NULL, &selfTexFramebuffer[i]); 
        assert(result == VK_SUCCESS); 
    }
}
void MyVulkanManager::destroy_frame_buffer()
{
    ...
    free(framebuffers);
    for(int i=0;i<5;i++){
        vk::vkDestroyFramebuffer(device, selfTexFramebuffer[i], NULL);
    }
    LOGE("销毁帧缓冲成功！");
}
...
void MyVulkanManager::initMatrix()
{
    ...
    MatrixState3D::setProjectFrustum(-ratio, ratio, -1, 1, 1.0f, 1000);
...
void MyVulkanManager::flushUniformBufferForToTex()
{
    ...
    float fragmentUniformData[1] = { 1.0f };
...
void MyVulkanManager::flushTexToDesSetForToScreen(){
    ...
    sqsD2D->writes[3].pImageInfo = &colorImageInfo[2]; 
    sqsD2D->writes[4].dstSet = sqsD2D->descSet[0]; 
    sqsD2D->writes[4].pImageInfo = &colorImageInfo[3]; 
    sqsD2D->writes[5].dstSet = sqsD2D->descSet[0]; 
    sqsD2D->writes[5].pImageInfo = &colorImageInfo[4]; 
    vk::vkUpdateDescriptorSets(device, 6, sqsD2D->writes, 0, NULL); 
}
void MyVulkanManager::drawSceneToTex() { //将场景绘制到5 幅纹理的方法
    float czCurrTemp=cz;//将摄像机位置的z 坐标赋值给临时变量
    float targetzCurrTemp=targetz;//将摄像机目标点的z 坐标赋值给临时变量
    for(int i=0;i<5;i++){//循环绘制到5 幅纹理中
        rp_begin.framebuffer = selfTexFramebuffer[i];//设置当前帧缓冲
        ...
        MatrixState3D::setCamera(0, 0, czCurrTemp-TIMESPAN*i, 0, 0, targetzCurrTemp-TIMESPAN*i, 0, 1, 0);//设置摄像机(摄像机Z 坐标每幅纹理不同)
        ...
        MatrixState3D::translate(0,0,45);//沿y 轴平移
        MatrixState3D::scale(0.1,-0.1,0.1);//绕y 轴旋转90°
        landForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                              &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForSky("texture/grass.bntex")]));//绘制云天空
...
void MyVulkanManager::drawSceneToScreen()//将5 幅纹理叠加绘制到屏幕的方法
{
    ...
    float ratio = (float)screenWidth / (float)screenHeight;
    MatrixState3D::setProjectOrtho(-ratio, ratio, -1, 1, 2, 100);//设置正交投影
    MatrixState3D::setCamera(0,0,3,0,0,0,0,1.0f,0.0f);//设置摄像机参数
...
void MyVulkanManager::drawTrees(int treeIndex,float transX,float transY,float transZ)
{
    if(treeIndex==1)
    {
        ...
        tree1->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, 
    ...
    }else
    {
        MatrixState3D::pushMatrix();
        MatrixState3D::translate(transX, transY, transZ);
        tree2->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, 
                            &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("texture/tree1.bntex")]));
        MatrixState3D::popMatrix();
    }
}
...
void MyVulkanManager::init_texture(){ 
    TextureManager::initTextures 
            (device,gpus[0],memoryroperties,cmdBuffer,queueGraphics);
    for(int i=0;i<5;i++){
