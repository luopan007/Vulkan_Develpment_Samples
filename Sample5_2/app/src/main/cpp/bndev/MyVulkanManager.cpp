...
void MyVulkanManager::flushUniformBuffer()
{
    ...
    VkResult result = vk::vkMapMemory(device, sqsCL->memUniformBuf,0, sqsCL->bufferByteCount, 0, (void **)&pData);//将设备内存映射为CPU可访问
    assert(result==VK_SUCCESS);
    memcpy(pData, vertexUniformData, sqsCL->bufferByteCount);//将数据拷贝进设备内存
